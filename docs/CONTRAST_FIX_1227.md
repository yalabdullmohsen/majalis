...
2. **لون حقيقي — `.pts-dates`:** `PrayerTimesView` يضع التاريخ في `.pts-lobby-body` خارج `.pts-screen`. القاعدة `.pts-dates { color: #D5E0DA }` كانت للنص على زمرد. الخلفية الفعلية `#F2F4F3` (`--mj-bg`) → 1.23:1. ملاحظة: `#66766F` (`--mj-muted`) على `#F2F4F3` = **4.33:1** دون 4.5، لذلك الوصف الثانوي لا يستخدمه.
3. **`.pts-hero__name` NOT_FOUND:** اسم الصلاة انتقل إلى `primary` في `FeaturedSectionCard` (`.card__label`).
4. **شرائح نشطة ليلاً:** `#fff` على `--mj-brand` الليلي `#4FB48B` = 2.55:1. الإصلاح: `var(--mj-on-brand)` (`#06231A` → 6.52:1).
...
## أزواج `@theme` المعتمدة (محسوبة بنفس معادلة WCAG 2 للبوابة)
...
| `--brand-on-white` | `#146C4E` | `#F2F4F3` | 5.79:1 | 4.5 |
| `--chip-fg` / `--chip-bg` | `#0F4A36` / `#E4F0EA` | الزوج | 8.74:1 | 4.5 |
| `--on-brand` | `#FFFFFF` | `#1F7A5A` | 5.26:1 | 4.5 |
| `--on-brand-muted` | `#E8F3EE` | `#1F7A5A` | 4.63:1 | 4.5 |

هوية العلامة: نفس عائلة الأخضر؛ النص على الفاتح أغمق ليُقرأ (`#146C4E` بدل `#1F7A5A` للنص)، لا لون جديد.

يتحقق `src/lib/__tests__/theme-contrast-pairs.test.ts` من هذه الأزواج ويفشل عند إدخال زوج دون العتبة.
...
| 7 | `#D5E0DA` على `#F2F4F3` = 1.23:1 | `--on-surface-muted` على سطح اللوبي = 6.60:1؛ على الزمرد يبقى `--on-brand-muted` = 4.63:1 |
