...
## 4) ما الأصول التي يملكها / لا يملكها «المجلس العلمي» في هذا المسار؟

المقارنة هنا بين ما **موجود داخل `mushafi`** وما **يمتلكه/يشغّله منتج المجلس العلمي (`artifacts/majalis`)** — من واقع المستودع، وليس فتوى ترخيص قانونية نهائية.
...
| **صور مصحف المدينة / QPC V2 / مضلعات** | غير موجودة في mushafi (مسار المجلس العلمي في majalis مختلف) |
| **حسابات/جلسات سحابية Supabase للمجلس** | غير مستخدمة هنا |
| **مراجعة معلّم أو صفوف** | غير موجودة |

### علاقة مع أصول المجلس العلمي (majalis)
...
ازدواجية معروفة: محاذاة/أخطاء موجودة في Python (`alignment_utils.py`) **وفي** Dart (`mistake_detection_engine.dart`) — عند الدمج مع المجلس العلمي يُفضَّل مصدر حقيقة واحد للخوارزمية.
