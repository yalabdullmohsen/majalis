...
export function generateLessonTitle(row) {
  const fromPoster = pick(row, "title", "generated_title", "poster_title", "lesson_title");
  if (fromPoster) return fromPoster.slice(0, 200);

  const sheikh = pick(row, "speaker_name", "sheikh_name");
  if (sheikh) return `درس للشيخ ${sheikh}`.slice(0, 200);

  const mosque = pick(row, "mosque", "location");
  if (mosque) {
    const label = /^مسجد/i.test(mosque) ? mosque : `مسجد ${mosque}`;
    return `درس في ${label}`.slice(0, 200);
  }
