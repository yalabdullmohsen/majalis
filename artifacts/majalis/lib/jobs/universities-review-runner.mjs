/**
 * Weekly universities review — creates human review reminders (no auto data mutation).
 */
 ...
 */
export async function runUniversitiesReview(opts = {}) {
  const { signal } = opts;
  if (signal?.aborted) {
  ...
  if (!admin) return { ok: false, error: "supabase_admin_missing", reminders_created: 0 };
  ...
  for (const u of staleUnivs || []) {
    ...
    if (!existing) {
      ...
      const { error } = await admin.from("review_reminders").insert({
        university_id: u.id,
