...
> ⚠️ **تنبيه تعارض (2026-07-24):** لوحة الألوان أدناه (`#1F6E54` أخضر
> زمردي + `#B08D2E` ذهبي برّاق) تعود لهوية v2 وسبق استبدالها فعليًا في
> الكود بهوية v3 (`#173D35` أخضر داكن رمادي، راجع
> `src/styles/modern-2026.css` و`docs/design-tokens.md` المُحدَّث).
> أسماء المتغيّرات نفسها (`--majalis-emerald` وغيرها) لا تزال حيّة في
> CSS لكن قيمها الفعلية أُعيد توجيهها لـv3 — **القيم الموثّقة هنا خاطئة
> حاليًا**. بقية هذا الملف (المكوّنات، التباعد، الظلال، الزخارف
> الإسلامية) لم تُراجَع بعد سطرًا بسطر مقابل الكود الحي — لا تفترض
> صحتها؛ تحقق من كل قسم قبل الاعتماد عليه (مُسجَّل في
> `docs/ui-audit/backlog.json`).

## لوحة الألوان (v2 — قديمة، انظر التنبيه أعلاه)
