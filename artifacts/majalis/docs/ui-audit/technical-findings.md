...
**تصحيح لتقرير سابق (كان خاطئًا):** ذكرتُ سابقًا "23 ملفًا فيها
console.log متبقٍّ في كود الإنتاج" كبند تنظيف مقترَح — **هذا غير دقيق**.
تحقّقت فعليًا: كل الـ23 ملفًا بلا استثناء داخل مجلدات `__tests__/`
حصرًا (سكربتات اختبار المشروع الذاتية تعتمد `console.log` كآلية طباعة
✓/✗ النتائج بتصميمها، تُشغَّل عبر `node` مباشرة، لا تُحزَم في تطبيق
الإنتاج إطلاقًا). **صفر console.log خارج `__tests__/`** (تحقّقت). لا
تنظيف لازم — كان هذا اقتراحًا خاطئًا مني في تقرير سابق، صححته هنا قبل
تنفيذه.

**اكتُشف (لم يُنفَّذ — يحتاج دفعة مخصَّصة):** `eslint-plugin-jsx-a11y`
مُثبَّت في `package.json` (تبعية موجودة فعليًا) لكن **غير مُفعَّل إطلاقًا**
في `eslint.config.js` — لا استيراد، لا plugin، لا rules. يعني هذا أن
`pnpm run lint` لا يفحص أي قاعدة وصول (aria-label، keyboard handlers،
alt text...) رغم توفّر الأداة، تمامًا كنمط "أدوات فحص جاهزة غير موصولة
بأي gate" الموثَّق سابقًا في الذاكرة. **اختبرتُ فعليًا** (محليًا، لم
أُثبِّت): تفعيل `jsxA11y.flatConfigs.recommended` على `src/**/*.{tsx,jsx}`
يُظهر **85 خطأً حقيقيًا** (لا تحذيرات — أخطاء تُفشل `pnpm run lint`
فورًا) عبر عشرات الملفات، أغلبها نمط واحد متكرر:
`jsx-a11y/click-events-have-key-events` +
`jsx-a11y/no-static-element-interactions` (عناصر `<div>`/غير-تفاعلية
بمعالج `onClick` بلا معالج لوحة مفاتيح مرافق ولا `role`/`tabIndex`
مناسبين) — مرصودة في `MushafPage.tsx`, `MushafPageView.tsx`,
`NotificationSettingsPage.tsx`, `VaultPage.tsx`, وعدة صفحات admin.
**رجعتُ عن تفعيل الإعداد** (لا commit) — تفعيله الآن سيُفشل بوابة lint
فورًا لكل من النافذتين حتى تُصلَح الـ85 حالة، وإصلاحها الآمن يحتاج حكمًا
فرديًا لكل عنصر (onKeyDown حقيقي أو تحويل لعنصر HTML دلالي `<button>`،
لا auto-fix آلي بلا فهم السلوك المقصود). **الخطوات التالية لدفعة
مخصَّصة:** فعّل نفس التعديل (أضف `import jsxA11y from
"eslint-plugin-jsx-a11y"` + كتلة `files: ["src/**/*.{tsx,jsx}"]` بقواعد
`jsxA11y.flatConfigs.recommended.rules`)، أصلح الـ85 حالة ملفًا فملف
مع تحقّق بصري، ثم فعِّلها بشكل دائم فقط بعد نظافة كاملة.
