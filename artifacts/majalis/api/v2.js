...
export default async function handler(req, res) {
  ...
  if (req.query?.format === "html" || req.query?.action === "docs") {
    if (req.query?.format === "json") {
      if (typeof res.status === "function") return res.status(200).json(generateOpenApiSpec("v2"));
      res.statusCode = 200;
      res.setHeader("Content-Type", "application/json");
      return res.end(JSON.stringify(generateOpenApiSpec("v2")));
    }
    res.statusCode = 200;
    res.setHeader("Content-Type", "text/html; charset=utf-8");
    return res.end(generateDocsHtml("v2"));
  }

  return handleOpenPlatformRequest(req, res, "v2");
}
