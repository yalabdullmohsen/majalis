#!/usr/bin/env node
/**
 * Round 61 — lesson bodies ≥360, pages meta ≥290, nations ≥290,
 * updates ≥190, circles/mutashabihat ≥220, mind-maps/amr ≥230, latin corruption scan.
 * Usage: node scripts/enrich-round61.mjs [--apply] [--verify]
 */
...
function lessonBodyReport(verifyMode = false) {
  ...
  const out = execSync(`node scripts/enrich-r61-lesson-bodies.mjs ${flag}`.trim(), {
    cwd: ROOT,
    encoding: "utf8",
  });
...
if (apply) {
  execSync("node scripts/enrich-r61-lesson-bodies.mjs --apply", { cwd: ROOT, stdio: "inherit" });
}
