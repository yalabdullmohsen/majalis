#!/usr/bin/env node
/**
 * Round 60 bulk content — quiz/QA/fawaid/stories/PM + scholars/sheikhs/prophets raises.
 * Usage: node scripts/enrich-round60-content.mjs [--apply] [--verify]
 */
import fs from "node:fs";
import path from "node:path";
import { fileURLToPath } from "node:url";
import { QUIZ_ITEMS, QA_ITEMS, FAWAID_ITEMS } from "./r60-content-data.mjs";
...
const SCHOLAR_BIO_MIN = 460;
const SHEIKH_BIO_MIN = 290;
const PROPHET_BIO_MIN = 420;
const PROPHET_LESSON_MIN = 140;

const PM_R60_IDS = [
  "avoid-overeating-sunnah",
  "cool-water-drinking-adab",
  "blessed-fig-dates-pair",
  "siwak-perfume-teeth",
  "salt-moderation-food",
];
...
function raiseQuizExplanations(apply) {
  ...
  for (const q of quiz) {
    ...
    if (Number.isFinite(n) && n >= 1815 && (q.explanation || "").length < QUIZ_EXPL_MIN) {
...
function readStoriesBlock() {
  return fs.readFileSync(path.join(__dirname, "r60-original-stories.ts"), "utf8").trim();
}

function readPmBlock() {
  return fs.readFileSync(path.join(__dirname, "r60-prophetic-medicine.ts"), "utf8").trim();
}

function addSeeds(apply) {
  const stats = { quiz: 0, qa: 0, fawaid: 0, stories: 0, pm: 0 };
  const marker = "جولة ٦٠";

  const quizPath = path.join(LIB, "quiz-seed.ts");
  let quizContent = fs.readFileSync(quizPath, "utf8");
  if (!quizContent.includes(marker)) {
    const block = `  /* ───────── ${marker}: أقسام أضعف (1815-1864) ───────── */\n` +
  ...
  if (!qaContent.includes("seed-qa-1030")) {
  ...
  if (!fawaidContent.includes("إضافات جولة ٦٠")) {
    const block = `  /* ── إضافات جولة ٦٠ ── */\n` + FAWAID_ITEMS.map(renderFawaidItem).join(",\n");
    if (apply) {
  ...
  if (!storiesContent.includes("id: 170,")) {
  ...
  if (!pmContent.includes("avoid-overeating-sunnah")) {
...
async function verifyCounts() {
  ...
  const round60Idx = fawaidSrc.indexOf("إضافات جولة ٦٠");
  const fawaidRound60Block =
    round60Idx >= 0 ? fawaidSrc.slice(round60Idx).split("{ text:").length - 1 : 0;
  ...
  return {
    ...
    quizRound60ShortExpl: quiz.filter((q) => {
      const n = parseInt((q.id || "").replace("demo-quiz-", ""), 10);
      return n >= 1815 && (q.explanation || "").length < QUIZ_EXPL_MIN;
    }).length,
    quizRound60: quiz.filter((q) => {
      const n = parseInt((q.id || "").replace("demo-quiz-", ""), 10);
      return n >= 1815 && n <= 1864;
    }).length,
    qaRound60: qa.filter((q) => {
      const n = parseInt((q.id || "").replace("seed-qa-", ""), 10);
      return n >= 1030 && n <= 1069;
    }).length,
    storiesRound60: stories.filter((s) => s.id >= 170 && s.id <= 174).length,
    pmRound60: pm.filter((x) => PM_R60_IDS.includes(x.id)).length,
    fawaidRound60Block,
    fawaidRound60Short145: (() => {
      if (round60Idx < 0) return 999;
      const block = fawaidSrc.slice(round60Idx);
      const texts = [...block.matchAll(/text:\s*"((?:[^"\\]|\\.)*)"/g)].map((m) => m[1]);
      return texts.filter((t) => t.length < FAWAID_MIN).length;
    })(),
    scholarsShort460: scholars.filter((s) => s.bio.length < SCHOLAR_BIO_MIN).length,
    sheikhsShort290: sheikhsMod.SHEIKHS_SEED.filter((s) => (s.bio || "").length < SHEIKH_BIO_MIN).length,
    prophetsBriefShort420: prophets.filter((p) => (p.briefBio || "").length < PROPHET_BIO_MIN).length,
    prophetsLessonShort140: prophets.reduce(
...
if (verify) {
  ...
  const fail =
    ...
    (a.quizRound60ShortExpl ?? 1) > 0 ||
    (a.quizRound60 ?? 0) !== 50 ||
    (a.qaRound60 ?? 0) !== 40 ||
    (a.storiesRound60 ?? 0) !== 5 ||
    (a.pmRound60 ?? 0) !== 5 ||
    (a.fawaidRound60Block ?? 0) !== 25 ||
    (a.fawaidRound60Short145 ?? 1) > 0 ||
    (a.scholarsShort460 ?? 1) > 0 ||
    (a.sheikhsShort290 ?? 1) > 0 ||
    (a.prophetsBriefShort420 ?? 1) > 0 ||
    (a.prophetsLessonShort140 ?? 1) > 0;
