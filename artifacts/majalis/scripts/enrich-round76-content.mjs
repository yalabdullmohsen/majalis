#!/usr/bin/env node
/**
 * Round 76 bulk content — quiz/QA/fawaid/stories/PM + scholars/sheikhs/prophets raises.
 * Usage: node scripts/enrich-round76-content.mjs [--apply] [--verify]
 */
import fs from "node:fs";
import path from "node:path";
import { fileURLToPath } from "node:url";
import { QUIZ_ITEMS, QA_ITEMS, FAWAID_ITEMS } from "./r76-content-data.mjs";
...
const SCHOLAR_BIO_MIN = 620;
const SHEIKH_BIO_MIN = 370;
const PROPHET_BIO_MIN = 580;
const PROPHET_LESSON_MIN = 220;
...
function raiseQuizExplanations(apply) {
  ...
  for (const q of quiz) {
    ...
    if (Number.isFinite(n) && n >= 2615 && (q.explanation || "").length < QUIZ_EXPL_MIN) {
...
function readStoriesBlock() {
  return fs.readFileSync(path.join(__dirname, "r76-original-stories.ts"), "utf8").trim();
}

function readPmBlock() {
  return fs.readFileSync(path.join(__dirname, "r76-prophetic-medicine.ts"), "utf8").trim();
}

function addSeeds(apply) {
  const stats = { quiz: 0, qa: 0, fawaid: 0, stories: 0, pm: 0 };
  const marker = "جولة ٧٦";

  const quizPath = path.join(LIB, "quiz-seed.ts");
  let quizContent = fs.readFileSync(quizPath, "utf8");
  if (!quizContent.includes(marker)) {
    const block = `  /* ───────── ${marker}: أقسام أضعف (2615-2664) ───────── */\n` +
  ...
  if (!qaContent.includes("seed-qa-1670")) {
  ...
  if (!fawaidContent.includes("إضافات جولة ٧٦")) {
    const block = `  /* ── إضافات جولة ٧٦ ── */\n` + FAWAID_ITEMS.map(renderFawaidItem).join(",\n");
    if (apply) {
  ...
  if (!storiesContent.includes("id: 250,")) {
  ...
  if (!pmContent.includes("fenugreek-r76")) {
...
async function verifyCounts() {
  ...
  const round76Idx = fawaidSrc.indexOf("إضافات جولة ٧٦");
  const fawaidRound76Block =
    round76Idx >= 0 ? fawaidSrc.slice(round76Idx).split("{ text:").length - 1 : 0;
  ...
  return {
    ...
    quizRound76ShortExpl: quiz.filter((q) => {
      const n = parseInt((q.id || "").replace("demo-quiz-", ""), 10);
      return n >= 2615 && (q.explanation || "").length < QUIZ_EXPL_MIN;
    }).length,
    quizRound76: quiz.filter((q) => {
      const n = parseInt((q.id || "").replace("demo-quiz-", ""), 10);
      return n >= 2615 && n <= 2664;
    }).length,
    qaRound76: qa.filter((q) => {
      const n = parseInt((q.id || "").replace("seed-qa-", ""), 10);
      return n >= 1670 && n <= 1709;
    }).length,
    storiesRound76: stories.filter((s) => s.id >= 250 && s.id <= 254).length,
    pmRound76: pm.filter((x) => PM_R76_IDS.includes(x.id)).length,
    fawaidRound76Block,
    fawaidRound76Short145: (() => {
      if (round76Idx < 0) return 999;
      const block = fawaidSrc.slice(round76Idx);
      const texts = [...block.matchAll(/text:\s*"((?:[^"\\]|\\.)*)"/g)].map((m) => m[1]);
      return texts.filter((t) => t.length < FAWAID_MIN).length;
    })(),
    scholarsShort620: scholars.filter((s) => s.bio.length < SCHOLAR_BIO_MIN).length,
    sheikhsShort370: sheikhsMod.SHEIKHS_SEED.filter((s) => (s.bio || "").length < SHEIKH_BIO_MIN).length,
    prophetsBriefShort580: prophets.filter((p) => (p.briefBio || "").length < PROPHET_BIO_MIN).length,
    prophetsLessonShort220: prophets.reduce(
...
if (verify) {
  ...
  const fail =
    ...
    (a.quizRound76ShortExpl ?? 1) > 0 ||
    (a.quizRound76 ?? 0) !== 50 ||
    (a.qaRound76 ?? 0) !== 40 ||
    (a.storiesRound76 ?? 0) !== 5 ||
    (a.pmRound76 ?? 0) !== 5 ||
    (a.fawaidRound76Block ?? 0) !== 25 ||
    (a.fawaidRound76Short145 ?? 1) > 0 ||
    (a.scholarsShort620 ?? 1) > 0 ||
    (a.sheikhsShort370 ?? 1) > 0 ||
    (a.prophetsBriefShort580 ?? 1) > 0 ||
    (a.prophetsLessonShort220 ?? 1) > 0;
