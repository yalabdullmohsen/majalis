#!/usr/bin/env node
/**
 * Round 75 — lesson bodies ≥500, pages meta ≥430 (educational), nations ≥430,
 * updates≥260, circles/mutashabihat≥290, mind-maps/amr≥300, latin corruption scan.
 * Usage: node scripts/enrich-round75.mjs [--apply] [--verify]
 */
...
const PAGE_MIN = 430;
const NATIONS_MIN = 430;
const UPDATE_SUMMARY_MIN = 260;
const QURAN_CIRCLE_MIN = 290;
const MUTA_MIN = 290;
const MIND_MAP_MIN = 300;
const AMR_EXPL_MIN = 300;
...
function lessonBodyReport(verifyMode = false) {
  ...
  const out = execSync(`node scripts/enrich-r75-lesson-bodies.mjs ${flag}`.trim(), {
    cwd: ROOT,
    encoding: "utf8",
  });
...
if (apply) {
  execSync("node scripts/enrich-r75-lesson-bodies.mjs --apply", { cwd: ROOT, stdio: "inherit" });
}
