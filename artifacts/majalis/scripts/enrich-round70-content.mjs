#!/usr/bin/env node
/**
 * Round 70 bulk content — quiz/QA/fawaid/stories/PM + scholars/sheikhs/prophets raises.
 * Usage: node scripts/enrich-round70-content.mjs [--apply] [--verify]
 */
import fs from "node:fs";
import path from "node:path";
import { fileURLToPath } from "node:url";
import { QUIZ_ITEMS, QA_ITEMS, FAWAID_ITEMS } from "./r70-content-data.mjs";
...
const SCHOLAR_BIO_MIN = 560;
const SHEIKH_BIO_MIN = 340;
const PROPHET_BIO_MIN = 520;
const PROPHET_LESSON_MIN = 190;

const PM_R70_IDS = [
  "honey-nutrition-r70",
  "olive-oil-sunnah-r70",
  "black-seed-wellness-r70",
  "siwak-hygiene-r70",
  "water-moderation-r70",
];
...
function raiseQuizExplanations(apply) {
  ...
  for (const q of quiz) {
    ...
    if (Number.isFinite(n) && n >= 2315 && (q.explanation || "").length < QUIZ_EXPL_MIN) {
...
function readStoriesBlock() {
  return fs.readFileSync(path.join(__dirname, "r70-original-stories.ts"), "utf8").trim();
}

function readPmBlock() {
  return fs.readFileSync(path.join(__dirname, "r70-prophetic-medicine.ts"), "utf8").trim();
}

function addSeeds(apply) {
  const stats = { quiz: 0, qa: 0, fawaid: 0, stories: 0, pm: 0 };
  const marker = "جولة ٧٠";

  const quizPath = path.join(LIB, "quiz-seed.ts");
  let quizContent = fs.readFileSync(quizPath, "utf8");
  if (!quizContent.includes(marker)) {
    const block = `  /* ───────── ${marker}: أقسام أضعف (2315-2364) ───────── */\n` +
  ...
  if (!qaContent.includes("seed-qa-1430")) {
  ...
  if (!fawaidContent.includes("إضافات جولة ٧٠")) {
    const block = `  /* ── إضافات جولة ٧٠ ── */\n` + FAWAID_ITEMS.map(renderFawaidItem).join(",\n");
    if (apply) {
  ...
  if (!storiesContent.includes("id: 220,")) {
  ...
  if (!pmContent.includes("honey-nutrition-r70")) {
...
async function verifyCounts() {
  ...
  const round70Idx = fawaidSrc.indexOf("إضافات جولة ٧٠");
  const fawaidRound70Block =
    round70Idx >= 0 ? fawaidSrc.slice(round70Idx).split("{ text:").length - 1 : 0;
  ...
  return {
    ...
    quizRound70ShortExpl: quiz.filter((q) => {
      const n = parseInt((q.id || "").replace("demo-quiz-", ""), 10);
      return n >= 2315 && (q.explanation || "").length < QUIZ_EXPL_MIN;
    }).length,
    quizRound70: quiz.filter((q) => {
      const n = parseInt((q.id || "").replace("demo-quiz-", ""), 10);
      return n >= 2315 && n <= 2364;
    }).length,
    qaRound70: qa.filter((q) => {
      const n = parseInt((q.id || "").replace("seed-qa-", ""), 10);
      return n >= 1430 && n <= 1469;
    }).length,
    storiesRound70: stories.filter((s) => s.id >= 220 && s.id <= 224).length,
    pmRound70: pm.filter((x) => PM_R70_IDS.includes(x.id)).length,
    fawaidRound70Block,
    fawaidRound70Short145: (() => {
      if (round70Idx < 0) return 999;
      const block = fawaidSrc.slice(round70Idx);
      const texts = [...block.matchAll(/text:\s*"((?:[^"\\]|\\.)*)"/g)].map((m) => m[1]);
      return texts.filter((t) => t.length < FAWAID_MIN).length;
    })(),
    scholarsShort560: scholars.filter((s) => s.bio.length < SCHOLAR_BIO_MIN).length,
    sheikhsShort340: sheikhsMod.SHEIKHS_SEED.filter((s) => (s.bio || "").length < SHEIKH_BIO_MIN).length,
    prophetsBriefShort520: prophets.filter((p) => (p.briefBio || "").length < PROPHET_BIO_MIN).length,
    prophetsLessonShort190: prophets.reduce(
...
if (verify) {
  ...
  const fail =
    ...
    (a.quizRound70ShortExpl ?? 1) > 0 ||
    (a.quizRound70 ?? 0) !== 50 ||
    (a.qaRound70 ?? 0) !== 40 ||
    (a.storiesRound70 ?? 0) !== 5 ||
    (a.pmRound70 ?? 0) !== 5 ||
    (a.fawaidRound70Block ?? 0) !== 25 ||
    (a.fawaidRound70Short145 ?? 1) > 0 ||
    (a.scholarsShort560 ?? 1) > 0 ||
    (a.sheikhsShort340 ?? 1) > 0 ||
    (a.prophetsBriefShort520 ?? 1) > 0 ||
    (a.prophetsLessonShort190 ?? 1) > 0;
