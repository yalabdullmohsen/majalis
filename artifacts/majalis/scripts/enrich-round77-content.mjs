#!/usr/bin/env node
/**
 * Round 77 bulk content — quiz/QA/fawaid/stories.
 * Usage: node scripts/enrich-round77-content.mjs [--apply] [--verify]
 */
import fs from "node:fs";
import path from "node:path";
import { fileURLToPath } from "node:url";
import { QUIZ_ITEMS, QA_ITEMS, FAWAID_ITEMS } from "./r77-content-data.mjs";
...
function raiseQuizExplanations(apply) {
  ...
  for (const q of quiz) {
    ...
    if (Number.isFinite(n) && n >= 2665 && (q.explanation || "").length < QUIZ_EXPL_MIN) {
...
function addSeeds(apply) {
  ...
  if (!qaContent.includes("seed-qa-1710")) {
  ...
  if (!fawaidContent.includes("إضافات جولة ٧٧")) {
    const block = `  /* ── إضافات جولة ٧٧ ── */\n` + FAWAID_ITEMS.map(renderFawaidItem).join(",\n");
    if (apply) {
  ...
  if (!storiesContent.includes("id: 255,")) {
...
async function verifyCounts() {
  ...
  const round77Idx = fawaidSrc.indexOf("إضافات جولة ٧٧");
  const fawaidRound77Block =
    round77Idx >= 0 ? fawaidSrc.slice(round77Idx).split("{ text:").length - 1 : 0;
  ...
  return {
    ...
    quizRound77ShortExpl: quiz.filter((q) => {
      const n = parseInt((q.id || "").replace("demo-quiz-", ""), 10);
      return n >= 2665 && (q.explanation || "").length < QUIZ_EXPL_MIN;
    }).length,
    quizRound77: quiz.filter((q) => {
      const n = parseInt((q.id || "").replace("demo-quiz-", ""), 10);
      return n >= 2665 && n <= 2714;
    }).length,
    qaRound77: qa.filter((q) => {
      const n = parseInt((q.id || "").replace("seed-qa-", ""), 10);
      return n >= 1710 && n <= 1749;
    }).length,
    storiesRound77: stories.filter((s) => s.id >= 255 && s.id <= 259).length,
    fawaidRound77Block,
    fawaidRound77Short145: (() => {
      if (round77Idx < 0) return 999;
      const block = fawaidSrc.slice(round77Idx);
...
if (verify) {
  ...
  const fail =
    ...
    (a.quizRound77ShortExpl ?? 1) > 0 ||
    (a.quizRound77 ?? 0) !== 50 ||
    (a.qaRound77 ?? 0) !== 40 ||
    (a.storiesRound77 ?? 0) !== 5 ||
    (a.fawaidRound77Block ?? 0) !== 25 ||
    (a.fawaidRound77Short145 ?? 1) > 0;
