#!/usr/bin/env node
/**
 * Round 80 bulk content — quiz/QA/fawaid/stories/PM.
 * Usage: node scripts/enrich-round80-content.mjs [--apply] [--verify]
 */
import fs from "node:fs";
import path from "node:path";
import { fileURLToPath } from "node:url";
import { QUIZ_ITEMS, QA_ITEMS, FAWAID_ITEMS } from "./r80-content-data.mjs";
...
const QUIZ_START = 2805;
const QUIZ_END = 2854;
const QA_START = 1820;
const QA_END = 1859;
const STORY_SLUGS = ["story-r80-1", "story-r80-2", "story-r80-3", "story-r80-4", "story-r80-5"];
const PM_R80_IDS = [
  "contagion-separation-r80",
  "dog-vessel-washing-r80",
  "breathing-outside-vessel-r80",
  "camel-milk-context-r80",
  "eating-moderation-r80",
];
...
function assertSourceData() {
  ...
  if (problems.length) throw new Error(`Invalid r80 source data: ${problems.join(", ")}`);
}

function addSeeds(apply) {
  assertSourceData();
  const stats = { quiz: 0, qa: 0, fawaid: 0, stories: 0, pm: 0 };
  const marker = "جولة ٨٠";

  const quizPath = path.join(LIB, "quiz-seed.ts");
  let quizContent = fs.readFileSync(quizPath, "utf8");
  if (!quizContent.includes("demo-quiz-2805")) {
    const block =
      `  /* ───────── ${marker}: محتوى أفقي (2805-2854) ───────── */\n` +
  ...
  if (!qaContent.includes("seed-qa-1820")) {
  ...
  if (!fawaidContent.includes("إضافات جولة ٨٠")) {
    const block = `  /* ── إضافات جولة ٨٠ ── */\n` + FAWAID_ITEMS.map(renderFawaidItem).join(",\n");
    if (apply) {
  ...
  if (!storiesContent.includes("story-r80-1")) {
    const block = readBlock("r80-original-stories.ts");
    if (apply) {
  ...
  if (!pmContent.includes("contagion-separation-r80")) {
    const block = readBlock("r80-prophetic-medicine.ts");
    if (apply) {
      pmContent = insertBeforeClosing(pmContent, "\n];", ",\n" + block + "\n");
      fs.writeFileSync(pmPath, pmContent, "utf8");
    }
    stats.pm = PM_R80_IDS.length;
  }
...
async function verifyCounts() {
  ...
  const round80Idx = fawaidSrc.indexOf("إضافات جولة ٨٠");
  const fawaidRound80Block =
    round80Idx >= 0 ? fawaidSrc.slice(round80Idx).split("{ text:").length - 1 : 0;
  const fawaidRound80Texts =
    round80Idx >= 0
      ? [...fawaidSrc.slice(round80Idx).matchAll(/text:\s*"((?:[^"\\]|\\.)*)"/g)].map((m) => m[1])
      : [];

  const quizRound80 = quiz.filter((q) => {
    const n = parseInt((q.id || "").replace("demo-quiz-", ""), 10);
    return n >= QUIZ_START && n <= QUIZ_END;
  });
  const qaRound80 = qa.filter((q) => {
    const n = parseInt((q.id || "").replace("seed-qa-", ""), 10);
    return n >= QA_START && n <= QA_END;
  });
  const storiesRound80 = stories.filter((s) => STORY_SLUGS.includes(s.slug));
  const pmRound80 = pm.filter((x) => PM_R80_IDS.includes(x.id));

  return {
    quizRound80: quizRound80.length,
    quizRound80ShortAnswers: quizRound80.filter((q) => (q.answer || "").length < QUIZ_ANSWER_MIN).length,
    quizRound80ShortExpl: quizRound80.filter((q) => (q.explanation || "").length < QUIZ_EXPL_MIN).length,
    qaRound80: qaRound80.length,
    qaRound80ShortAnswers: qaRound80.filter((q) => (q.answer || "").length < QA_ANSWER_MIN).length,
    fawaidRound80Block,
    fawaidRound80Short145: fawaidRound80Texts.filter((t) => t.length < FAWAID_MIN).length,
    storiesRound80: storiesRound80.length,
    storiesRound80ShortContent: storiesRound80.filter((s) => (s.full_content || "").length < STORY_CONTENT_MIN).length,
    pmRound80: pmRound80.length,
    pmRound80WeakAsFixed: pmRound80.filter((x) => /ضعيف/.test(x.hadithSource || "") && !/لا يثبت|ضعف|ضعيف/.test(`${x.body || ""} ${x.benefits?.join(" ") || ""}`)).length,
    ...
    pmRound80Ids: pmRound80.map((x) => x.id),
  };
}
...
if (verify) {
  ...
  const fail =
    (a.quizRound80 ?? 0) !== 50 ||
    (a.quizRound80ShortAnswers ?? 1) > 0 ||
    (a.quizRound80ShortExpl ?? 1) > 0 ||
    (a.qaRound80 ?? 0) !== 40 ||
    (a.qaRound80ShortAnswers ?? 1) > 0 ||
    (a.fawaidRound80Block ?? 0) !== 25 ||
    (a.fawaidRound80Short145 ?? 1) > 0 ||
    (a.storiesRound80 ?? 0) !== 5 ||
    (a.storiesRound80ShortContent ?? 1) > 0 ||
    (a.pmRound80 ?? 0) !== 5 ||
    (a.pmRound80WeakAsFixed ?? 1) > 0 ||
    a.lastQuizId !== "demo-quiz-2854" ||
    a.lastQaId !== "seed-qa-1859";
