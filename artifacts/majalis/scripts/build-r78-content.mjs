...
const OUT = path.join(__dirname, "r78-content-data.mjs");
...
function pickQuiz(existing, need) {
  ...
  const recentQs = new Set(
    existing.filter((q) => parseInt((q.id || "").replace("demo-quiz-", ""), 10) >= 2665).map((q) => q.question),
  );
...
const quizPicked = pickQuiz(existingQuiz, 40);
const qaPicked = pickQa(existingQa, 30, 1750);
const fawaidPicked = pickFawaid(curated, 20, 765);

if (quizPicked.length < 40) throw new Error(`Only ${quizPicked.length} quiz`);
if (qaPicked.length < 30) throw new Error(`Only ${qaPicked.length} qa`);
if (fawaidPicked.length < 20) throw new Error(`Only ${fawaidPicked.length} fawaid`);

const quizLines = quizPicked.map((q, i) => {
  const id = String(2715 + i);
