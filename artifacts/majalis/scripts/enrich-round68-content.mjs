#!/usr/bin/env node
/**
 * Round 68 bulk content — quiz/QA/fawaid/stories/PM + scholars/sheikhs/prophets raises.
 * Usage: node scripts/enrich-round68-content.mjs [--apply] [--verify]
 */
import fs from "node:fs";
import path from "node:path";
import { fileURLToPath } from "node:url";
import { QUIZ_ITEMS, QA_ITEMS, FAWAID_ITEMS } from "./r68-content-data.mjs";
...
const SCHOLAR_BIO_MIN = 540;
const SHEIKH_BIO_MIN = 330;
const PROPHET_BIO_MIN = 500;
const PROPHET_LESSON_MIN = 180;

const PM_R68_IDS = [
  "dates-nutrition-r68",
  "milk-sunnah-r68",
  "talbina-comfort-r68",
  "vinegar-digestion-r68",
  "fasting-wellness-r68",
];
...
function raiseQuizExplanations(apply) {
  ...
  for (const q of quiz) {
    ...
    if (Number.isFinite(n) && n >= 2215 && (q.explanation || "").length < QUIZ_EXPL_MIN) {
...
function readStoriesBlock() {
  return fs.readFileSync(path.join(__dirname, "r68-original-stories.ts"), "utf8").trim();
}

function readPmBlock() {
  return fs.readFileSync(path.join(__dirname, "r68-prophetic-medicine.ts"), "utf8").trim();
}

function addSeeds(apply) {
  const stats = { quiz: 0, qa: 0, fawaid: 0, stories: 0, pm: 0 };
  const marker = "جولة ٦٨";

  const quizPath = path.join(LIB, "quiz-seed.ts");
  let quizContent = fs.readFileSync(quizPath, "utf8");
  if (!quizContent.includes(marker)) {
    const block = `  /* ───────── ${marker}: أقسام أضعف (2215-2264) ───────── */\n` +
  ...
  if (!qaContent.includes("seed-qa-1350")) {
  ...
  if (!fawaidContent.includes("إضافات جولة ٦٨")) {
    const block = `  /* ── إضافات جولة ٦٨ ── */\n` + FAWAID_ITEMS.map(renderFawaidItem).join(",\n");
    if (apply) {
  ...
  if (!storiesContent.includes("id: 210,")) {
  ...
  if (!pmContent.includes("dates-nutrition-r68")) {
...
async function verifyCounts() {
  ...
  const round68Idx = fawaidSrc.indexOf("إضافات جولة ٦٨");
  const fawaidRound68Block =
    round68Idx >= 0 ? fawaidSrc.slice(round68Idx).split("{ text:").length - 1 : 0;
  ...
  return {
    ...
    quizRound68ShortExpl: quiz.filter((q) => {
      const n = parseInt((q.id || "").replace("demo-quiz-", ""), 10);
      return n >= 2215 && (q.explanation || "").length < QUIZ_EXPL_MIN;
    }).length,
    quizRound68: quiz.filter((q) => {
      const n = parseInt((q.id || "").replace("demo-quiz-", ""), 10);
      return n >= 2215 && n <= 2264;
    }).length,
    qaRound68: qa.filter((q) => {
      const n = parseInt((q.id || "").replace("seed-qa-", ""), 10);
      return n >= 1350 && n <= 1389;
    }).length,
    storiesRound68: stories.filter((s) => s.id >= 210 && s.id <= 214).length,
    pmRound68: pm.filter((x) => PM_R68_IDS.includes(x.id)).length,
    fawaidRound68Block,
    fawaidRound68Short145: (() => {
      if (round68Idx < 0) return 999;
      const block = fawaidSrc.slice(round68Idx);
      const texts = [...block.matchAll(/text:\s*"((?:[^"\\]|\\.)*)"/g)].map((m) => m[1]);
      return texts.filter((t) => t.length < FAWAID_MIN).length;
    })(),
    scholarsShort540: scholars.filter((s) => s.bio.length < SCHOLAR_BIO_MIN).length,
    sheikhsShort330: sheikhsMod.SHEIKHS_SEED.filter((s) => (s.bio || "").length < SHEIKH_BIO_MIN).length,
    prophetsBriefShort500: prophets.filter((p) => (p.briefBio || "").length < PROPHET_BIO_MIN).length,
    prophetsLessonShort180: prophets.reduce(
...
if (verify) {
  ...
  const fail =
    ...
    (a.quizRound68ShortExpl ?? 1) > 0 ||
    (a.quizRound68 ?? 0) !== 50 ||
    (a.qaRound68 ?? 0) !== 40 ||
    (a.storiesRound68 ?? 0) !== 5 ||
    (a.pmRound68 ?? 0) !== 5 ||
    (a.fawaidRound68Block ?? 0) !== 25 ||
    (a.fawaidRound68Short145 ?? 1) > 0 ||
    (a.scholarsShort540 ?? 1) > 0 ||
    (a.sheikhsShort330 ?? 1) > 0 ||
    (a.prophetsBriefShort500 ?? 1) > 0 ||
    (a.prophetsLessonShort180 ?? 1) > 0;
