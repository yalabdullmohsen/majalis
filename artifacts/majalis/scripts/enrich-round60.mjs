#!/usr/bin/env node
/**
 * Round 60 — lesson bodies ≥350, pages meta ≥280, nations ≥280,
 * fiqh-issues summary≥210 description≥240, landmarks desc≥400 significance≥260,
 * courses summary≥250, occasions≥240, latin corruption scan.
 * Usage: node scripts/enrich-round60.mjs [--apply] [--verify]
 */
...
const PAGE_MIN = 280;
const NATIONS_MIN = 280;
const FIQH_SUMMARY_MIN = 210;
const FIQH_DESC_MIN = 240;
const LANDMARK_DESC_MIN = 400;
const LANDMARK_SIG_MIN = 260;
const COURSE_SUMMARY_MIN = 250;
const OCCASION_SUMMARY_MIN = 240;
...
function lessonBodyReport(verifyMode = false) {
  ...
  const out = execSync(`node scripts/enrich-r60-lesson-bodies.mjs ${flag}`.trim(), {
    cwd: ROOT,
    encoding: "utf8",
  });
...
const before = {
  ...
  seeds: countShortCoursesLandmarksOccasions(),
...
if (apply) {
  execSync("node scripts/enrich-r60-lesson-bodies.mjs --apply", { cwd: ROOT, stdio: "inherit" });
}

results.pages = enrichPages(apply);
results.nationsRaised = enrichNations(apply);
results.fiqhRaised = await enrichFiqhIssues(apply);
results.seeds = raiseCoursesLandmarksOccasions(apply);
...
if (apply || verify) {
  ...
  results.raised = {
    ...
    courses: results.seeds.courses,
    landmarks: results.seeds.landmarks,
    occasions: results.seeds.occasions,
...
if (verify) {
  ...
  const fail =
    ...
    (a?.seeds?.coursesShort ?? 1) > 0 ||
    (a?.seeds?.landmarksDescShort ?? 1) > 0 ||
    (a?.seeds?.landmarksSigShort ?? 1) > 0 ||
    (a?.seeds?.occasionsShort ?? 1) > 0;
