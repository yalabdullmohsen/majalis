#!/usr/bin/env node
/**
 * Round 59 bulk content — quiz/QA/fawaid/stories.
 * Usage: node scripts/enrich-round59-content.mjs [--apply] [--verify]
 */
import fs from "node:fs";
import path from "node:path";
import { fileURLToPath } from "node:url";
import { QUIZ_ITEMS, QA_ITEMS, FAWAID_ITEMS } from "./r59-content-data.mjs";
...
function raiseQuizExplanations(apply) {
  ...
  for (const q of quiz) {
    ...
    if (Number.isFinite(n) && n >= 1765 && (q.explanation || "").length < QUIZ_EXPL_MIN) {
...
function addSeeds(apply) {
  ...
  if (!qaContent.includes("seed-qa-990")) {
  ...
  if (!fawaidContent.includes("إضافات جولة ٥٩")) {
    const block = `  /* ── إضافات جولة ٥٩ ── */\n` + FAWAID_ITEMS.map(renderFawaidItem).join(",\n");
    if (apply) {
  ...
  if (!storiesContent.includes("id: 165,")) {
...
async function verifyCounts() {
  ...
  const round59Idx = fawaidSrc.indexOf("إضافات جولة ٥٩");
  const fawaidRound59Block =
    round59Idx >= 0 ? fawaidSrc.slice(round59Idx).split("{ text:").length - 1 : 0;
  ...
  return {
    ...
    quizRound59ShortExpl: quiz.filter((q) => {
      const n = parseInt((q.id || "").replace("demo-quiz-", ""), 10);
      return n >= 1765 && (q.explanation || "").length < QUIZ_EXPL_MIN;
    }).length,
    quizRound59: quiz.filter((q) => {
      const n = parseInt((q.id || "").replace("demo-quiz-", ""), 10);
      return n >= 1765 && n <= 1814;
    }).length,
    qaRound59: qa.filter((q) => {
      const n = parseInt((q.id || "").replace("seed-qa-", ""), 10);
      return n >= 990 && n <= 1029;
    }).length,
    storiesRound59: stories.filter((s) => s.id >= 165 && s.id <= 169).length,
    fawaidRound59Block,
    fawaidRound59Short145: (() => {
      if (round59Idx < 0) return 999;
      const block = fawaidSrc.slice(round59Idx);
...
if (verify) {
  ...
  const fail =
    ...
    (a.quizRound59ShortExpl ?? 1) > 0 ||
    (a.quizRound59 ?? 0) !== 50 ||
    (a.qaRound59 ?? 0) !== 40 ||
    (a.storiesRound59 ?? 0) !== 5 ||
    (a.fawaidRound59Block ?? 0) !== 25 ||
    (a.fawaidRound59Short145 ?? 1) > 0;
