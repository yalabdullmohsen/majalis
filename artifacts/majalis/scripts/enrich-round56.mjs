#!/usr/bin/env node
/**
 * Round 56 — lesson bodies ≥310, pages meta ≥240, nations ≥240,
 * fiqh-issues summary≥190 description≥220, landmarks desc≥360 significance≥260,
 * courses summary≥230, occasions≥220, latin corruption scan.
 * Usage: node scripts/enrich-round56.mjs [--apply] [--verify]
 */
...
const PAGE_MIN = 240;
const NATIONS_MIN = 240;
const FIQH_SUMMARY_MIN = 190;
const FIQH_DESC_MIN = 220;
const LANDMARK_DESC_MIN = 360;
const LANDMARK_SIG_MIN = 260;
const COURSE_SUMMARY_MIN = 230;
const OCCASION_SUMMARY_MIN = 220;
...
function lessonBodyReport(verifyMode = false) {
  ...
  const out = execSync(`node scripts/enrich-r56-lesson-bodies.mjs ${flag}`.trim(), {
    cwd: ROOT,
    encoding: "utf8",
  });
...
if (apply) {
  execSync("node scripts/enrich-r56-lesson-bodies.mjs --apply", { cwd: ROOT, stdio: "inherit" });
}
