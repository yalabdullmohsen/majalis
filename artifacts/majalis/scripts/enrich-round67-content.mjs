#!/usr/bin/env node
/**
 * Round 67 bulk content — quiz/QA/fawaid/stories.
 * Usage: node scripts/enrich-round67-content.mjs [--apply] [--verify]
 */
import fs from "node:fs";
import path from "node:path";
import { fileURLToPath } from "node:url";
import { QUIZ_ITEMS, QA_ITEMS, FAWAID_ITEMS } from "./r67-content-data.mjs";
...
function raiseQuizExplanations(apply) {
  ...
  for (const q of quiz) {
    ...
    if (Number.isFinite(n) && n >= 2165 && (q.explanation || "").length < QUIZ_EXPL_MIN) {
...
function addSeeds(apply) {
  ...
  if (!qaContent.includes("seed-qa-1310")) {
  ...
  if (!fawaidContent.includes("إضافات جولة ٦٧")) {
    const block = `  /* ── إضافات جولة ٦٧ ── */\n` + FAWAID_ITEMS.map(renderFawaidItem).join(",\n");
    if (apply) {
  ...
  if (!storiesContent.includes("id: 205,")) {
...
async function verifyCounts() {
  ...
  const round67Idx = fawaidSrc.indexOf("إضافات جولة ٦٧");
  const fawaidRound67Block =
    round67Idx >= 0 ? fawaidSrc.slice(round67Idx).split("{ text:").length - 1 : 0;
  ...
  return {
    ...
    quizRound67ShortExpl: quiz.filter((q) => {
      const n = parseInt((q.id || "").replace("demo-quiz-", ""), 10);
      return n >= 2165 && (q.explanation || "").length < QUIZ_EXPL_MIN;
    }).length,
    quizRound67: quiz.filter((q) => {
      const n = parseInt((q.id || "").replace("demo-quiz-", ""), 10);
      return n >= 2165 && n <= 2214;
    }).length,
    qaRound67: qa.filter((q) => {
      const n = parseInt((q.id || "").replace("seed-qa-", ""), 10);
      return n >= 1310 && n <= 1349;
    }).length,
    storiesRound67: stories.filter((s) => s.id >= 205 && s.id <= 209).length,
    fawaidRound67Block,
    fawaidRound67Short145: (() => {
      if (round67Idx < 0) return 999;
      const block = fawaidSrc.slice(round67Idx);
...
if (verify) {
  ...
  const fail =
    ...
    (a.quizRound67ShortExpl ?? 1) > 0 ||
    (a.quizRound67 ?? 0) !== 50 ||
    (a.qaRound67 ?? 0) !== 40 ||
    (a.storiesRound67 ?? 0) !== 5 ||
    (a.fawaidRound67Block ?? 0) !== 25 ||
    (a.fawaidRound67Short145 ?? 1) > 0;
