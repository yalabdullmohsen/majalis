#!/usr/bin/env node
/**
 * Round 74 — lesson bodies ≥490, pages meta ≥420 (educational), nations ≥420,
 * fiqh summary≥280 description≥310, courses≥320 occasions≥310 landmarks≥540, latin corruption scan.
 * Usage: node scripts/enrich-round74.mjs [--apply] [--verify]
 */
...
const PAGE_MIN = 420;
const NATIONS_MIN = 420;
const FIQH_SUMMARY_MIN = 280;
const FIQH_DESC_MIN = 310;
const LANDMARK_DESC_MIN = 540;
const LANDMARK_SIG_MIN = 540;
const COURSE_SUMMARY_MIN = 320;
const OCCASION_SUMMARY_MIN = 310;
...
function lessonBodyReport(verifyMode = false) {
  ...
  const out = execSync(`node scripts/enrich-r74-lesson-bodies.mjs ${flag}`.trim(), {
    cwd: ROOT,
    encoding: "utf8",
  });
...
if (apply) {
  execSync("node scripts/enrich-r74-lesson-bodies.mjs --apply", { cwd: ROOT, stdio: "inherit" });
}
