#!/usr/bin/env node
/**
 * Round 146 — raise ALL live/scientific lesson bodies to ≥1570.
 * Meaningful scholarly expansion; bridge max frequency ≤35; no latin corruption.
 * Usage: node scripts/enrich-r138-lesson-bodies.mjs [--apply] [--verify]
 */
...
const TRACKED_PHRASES = (() => {
  const uniq = [...new Set(R53_BRIDGE_POOL)];
  return uniq.filter((p) => !uniq.some((q) => q !== p && q.startsWith(p)));
})();
// Keep full pool for insertion variety, but prefer non-prefix-colliding tracked set for caps.
const R53_BRIDGE_POOL_SAFE = TRACKED_PHRASES.length ? TRACKED_PHRASES : [...new Set(R53_BRIDGE_POOL)];
  ...
  for (const p of sorted) {
    ...
    while ((idx = text.indexOf(p, idx)) !== -1) {
      ...
      for (let i = idx; i < idx + p.length; i++) {
        if (occupied[i]) { blocked = true; break; }
      }
      if (!blocked) {
        counts[p]++;
        for (let i = idx; i < idx + p.length; i++) occupied[i] = true;
      }
      idx += p.length;
    }
  }
...
function cleanArtifacts(body) {
  ...
  // Strip stacked suffix remnants from prefix-bridge collisions
  const rem = String.raw`(?:سندًا|الحماسية|والعمل|الشرعي|ٌ\s*شرعي)`;
  for (let i = 0; i < 12; i++) {
    const next = out
      .replace(new RegExp(String.raw`(?:\s*${rem}){2,}`, "g"), "")
      .replace(/(?<!باب)ٌ\s*شرعي/g, "")
      .replace(/\s+(?:سندًا|الحماسية)(?=\s*\.|\s*تطبيق|\s*عملي|\s*ابدأ|\s*خطوة|\s*$)/g, "");
    if (next === out) break;
    out = next;
  }
  return out
...
function pickBridge(body, counts, idx) {
  const candidates = R53_BRIDGE_POOL_SAFE.filter((p) => !body.includes(p)).sort(
    (a, b) => (counts[a] || 0) - (counts[b] || 0),
  );
  const underCap = candidates.filter((p) => (counts[p] || 0) < MAX_BRIDGE_FREQ);
  const pool = underCap.length ? underCap : candidates.length ? candidates : R53_BRIDGE_POOL_SAFE;
...
function redistributeBridges(body, counts) {
  ...
  const over = TRACKED_PHRASES.filter((p) => (counts[p] || 0) > MAX_BRIDGE_FREQ).sort(
    (a, b) => b.length - a.length || (counts[b] || 0) - (counts[a] || 0),
  );
  for (const phrase of over) {
    while ((counts[phrase] || 0) > MAX_BRIDGE_FREQ && out.includes(phrase)) {
      const alt = pickReplacement(out, R53_BRIDGE_POOL_SAFE, counts);
      if (!alt || alt === phrase) break;
      counts[phrase]--;
...
function processBody(body, title, summary, minLen, counts, state) {
  let out = cleanArtifacts(stripLatinCorruption(body));
  out = expandBody(out, title, summary, minLen, counts, state);
  out = varyTatbiq(out, state);
  out = redistributeBridges(out, counts);
  out = cleanArtifacts(stripLatinCorruption(out));
