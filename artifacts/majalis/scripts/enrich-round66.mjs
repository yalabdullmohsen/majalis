#!/usr/bin/env node
/**
 * Round 66 — lesson bodies ≥410, pages meta ≥340 (educational), nations ≥340,
 * fiqh summary≥240 description≥270, courses≥280 occasions≥270 landmarks≥460, latin corruption scan.
 * Usage: node scripts/enrich-round66.mjs [--apply] [--verify]
 */
...
const PAGE_MIN = 340;
const NATIONS_MIN = 340;
const FIQH_SUMMARY_MIN = 240;
const FIQH_DESC_MIN = 270;
const LANDMARK_DESC_MIN = 460;
const LANDMARK_SIG_MIN = 460;
const COURSE_SUMMARY_MIN = 280;
const OCCASION_SUMMARY_MIN = 270;
...
function lessonBodyReport(verifyMode = false) {
  ...
  const out = execSync(`node scripts/enrich-r66-lesson-bodies.mjs ${flag}`.trim(), {
    cwd: ROOT,
    encoding: "utf8",
  });
...
if (apply) {
  execSync("node scripts/enrich-r66-lesson-bodies.mjs --apply", { cwd: ROOT, stdio: "inherit" });
}
