#!/usr/bin/env node
/**
 * Round 69 — lesson bodies ≥440, pages meta ≥370 (educational), nations ≥370,
 * updates≥230, circles/mutashabihat≥260, mind-maps/amr≥270, latin corruption scan.
 * Usage: node scripts/enrich-round69.mjs [--apply] [--verify]
 */
...
const PAGE_MIN = 370;
const NATIONS_MIN = 370;
const UPDATE_SUMMARY_MIN = 230;
const QURAN_CIRCLE_MIN = 260;
const MUTA_MIN = 260;
const MIND_MAP_MIN = 270;
const AMR_EXPL_MIN = 270;
...
function lessonBodyReport(verifyMode = false) {
  ...
  const out = execSync(`node scripts/enrich-r69-lesson-bodies.mjs ${flag}`.trim(), {
    cwd: ROOT,
    encoding: "utf8",
  });
...
if (apply) {
  execSync("node scripts/enrich-r69-lesson-bodies.mjs --apply", { cwd: ROOT, stdio: "inherit" });
}
