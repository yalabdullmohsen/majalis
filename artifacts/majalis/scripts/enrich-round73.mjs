#!/usr/bin/env node
/**
 * Round 73 — lesson bodies ≥480, pages meta ≥410 (educational), nations ≥410,
 * updates≥250, circles/mutashabihat≥280, mind-maps/amr≥290, latin corruption scan.
 * Usage: node scripts/enrich-round73.mjs [--apply] [--verify]
 */
...
const PAGE_MIN = 410;
const NATIONS_MIN = 410;
const UPDATE_SUMMARY_MIN = 250;
const QURAN_CIRCLE_MIN = 280;
const MUTA_MIN = 280;
const MIND_MAP_MIN = 290;
const AMR_EXPL_MIN = 290;
...
function lessonBodyReport(verifyMode = false) {
  ...
  const out = execSync(`node scripts/enrich-r73-lesson-bodies.mjs ${flag}`.trim(), {
    cwd: ROOT,
    encoding: "utf8",
  });
...
if (apply) {
  execSync("node scripts/enrich-r73-lesson-bodies.mjs --apply", { cwd: ROOT, stdio: "inherit" });
}
