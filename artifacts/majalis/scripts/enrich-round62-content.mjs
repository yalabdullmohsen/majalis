#!/usr/bin/env node
/**
 * Round 62 bulk content — quiz/QA/fawaid/stories/PM + scholars/sheikhs/prophets raises.
 * Usage: node scripts/enrich-round62-content.mjs [--apply] [--verify]
 */
import fs from "node:fs";
import path from "node:path";
import { fileURLToPath } from "node:url";
import { QUIZ_ITEMS, QA_ITEMS, FAWAID_ITEMS } from "./r62-content-data.mjs";
...
const SCHOLAR_BIO_MIN = 480;
const SHEIKH_BIO_MIN = 300;
const PROPHET_BIO_MIN = 440;
const PROPHET_LESSON_MIN = 150;

const PM_R62_IDS = [
  "trim-mustache-hygiene",
  "comb-hair-grooming",
  "removing-harm-pathway",
  "dua-entering-home",
  "blessed-vinegar-honey",
];
...
function raiseQuizExplanations(apply) {
  ...
  for (const q of quiz) {
    ...
    if (Number.isFinite(n) && n >= 1915 && (q.explanation || "").length < QUIZ_EXPL_MIN) {
...
function readStoriesBlock() {
  return fs.readFileSync(path.join(__dirname, "r62-original-stories.ts"), "utf8").trim();
}

function readPmBlock() {
  return fs.readFileSync(path.join(__dirname, "r62-prophetic-medicine.ts"), "utf8").trim();
}

function addSeeds(apply) {
  const stats = { quiz: 0, qa: 0, fawaid: 0, stories: 0, pm: 0 };
  const marker = "جولة ٦٢";

  const quizPath = path.join(LIB, "quiz-seed.ts");
  let quizContent = fs.readFileSync(quizPath, "utf8");
  if (!quizContent.includes(marker)) {
    const block = `  /* ───────── ${marker}: أقسام أضعف (1915-1964) ───────── */\n` +
  ...
  if (!qaContent.includes("seed-qa-1110")) {
  ...
  if (!fawaidContent.includes("إضافات جولة ٦٢")) {
    const block = `  /* ── إضافات جولة ٦٢ ── */\n` + FAWAID_ITEMS.map(renderFawaidItem).join(",\n");
    if (apply) {
  ...
  if (!storiesContent.includes("id: 180,")) {
  ...
  if (!pmContent.includes("trim-mustache-hygiene")) {
...
async function verifyCounts() {
  ...
  const round62Idx = fawaidSrc.indexOf("إضافات جولة ٦٢");
  const fawaidRound62Block =
    round62Idx >= 0 ? fawaidSrc.slice(round62Idx).split("{ text:").length - 1 : 0;
  ...
  return {
    ...
    quizRound62ShortExpl: quiz.filter((q) => {
      const n = parseInt((q.id || "").replace("demo-quiz-", ""), 10);
      return n >= 1915 && (q.explanation || "").length < QUIZ_EXPL_MIN;
    }).length,
    quizRound62: quiz.filter((q) => {
      const n = parseInt((q.id || "").replace("demo-quiz-", ""), 10);
      return n >= 1915 && n <= 1964;
    }).length,
    qaRound62: qa.filter((q) => {
      const n = parseInt((q.id || "").replace("seed-qa-", ""), 10);
      return n >= 1110 && n <= 1149;
    }).length,
    storiesRound62: stories.filter((s) => s.id >= 180 && s.id <= 184).length,
    pmRound62: pm.filter((x) => PM_R62_IDS.includes(x.id)).length,
    fawaidRound62Block,
    fawaidRound62Short145: (() => {
      if (round62Idx < 0) return 999;
      const block = fawaidSrc.slice(round62Idx);
      const texts = [...block.matchAll(/text:\s*"((?:[^"\\]|\\.)*)"/g)].map((m) => m[1]);
      return texts.filter((t) => t.length < FAWAID_MIN).length;
    })(),
    scholarsShort480: scholars.filter((s) => s.bio.length < SCHOLAR_BIO_MIN).length,
    sheikhsShort300: sheikhsMod.SHEIKHS_SEED.filter((s) => (s.bio || "").length < SHEIKH_BIO_MIN).length,
    prophetsBriefShort440: prophets.filter((p) => (p.briefBio || "").length < PROPHET_BIO_MIN).length,
    prophetsLessonShort150: prophets.reduce(
...
if (verify) {
  ...
  const fail =
    ...
    (a.quizRound62ShortExpl ?? 1) > 0 ||
    (a.quizRound62 ?? 0) !== 50 ||
    (a.qaRound62 ?? 0) !== 40 ||
    (a.storiesRound62 ?? 0) !== 5 ||
    (a.pmRound62 ?? 0) !== 5 ||
    (a.fawaidRound62Block ?? 0) !== 25 ||
    (a.fawaidRound62Short145 ?? 1) > 0 ||
    (a.scholarsShort480 ?? 1) > 0 ||
    (a.sheikhsShort300 ?? 1) > 0 ||
    (a.prophetsBriefShort440 ?? 1) > 0 ||
    (a.prophetsLessonShort150 ?? 1) > 0;
