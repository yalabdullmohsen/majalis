#!/usr/bin/env node
/**
 * Round 52 — lesson bodies ≥270, pages meta ≥200, nations ≥200,
 * sheikhs bio ≥250, scholars bio ≥380, glossary ≥190, latin corruption scan.
 * Usage: node scripts/enrich-round52.mjs [--apply] [--verify]
 */
...
const PAGE_MIN = 200;
const NATIONS_MIN = 200;
const SHEIKH_BIO_MIN = 250;
const SCHOLAR_BIO_MIN = 380;
const GLOSSARY_MIN = 190;
...
function lessonBodyReport(verifyMode = false) {
  ...
  const out = execSync(`node scripts/enrich-r52-lesson-bodies.mjs ${flag}`.trim(), {
    cwd: ROOT,
    encoding: "utf8",
  });
...
if (apply) {
  execSync("node scripts/enrich-r52-lesson-bodies.mjs --apply", { cwd: ROOT, stdio: "inherit" });
}
