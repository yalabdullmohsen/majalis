#!/usr/bin/env node
/**
 * Round 71 bulk content — quiz/QA/fawaid/stories.
 * Usage: node scripts/enrich-round71-content.mjs [--apply] [--verify]
 */
import fs from "node:fs";
import path from "node:path";
import { fileURLToPath } from "node:url";
import { QUIZ_ITEMS, QA_ITEMS, FAWAID_ITEMS } from "./r71-content-data.mjs";
...
function raiseQuizExplanations(apply) {
  ...
  for (const q of quiz) {
    ...
    if (Number.isFinite(n) && n >= 2365 && (q.explanation || "").length < QUIZ_EXPL_MIN) {
...
function addSeeds(apply) {
  ...
  if (!qaContent.includes("seed-qa-1470")) {
  ...
  if (!fawaidContent.includes("إضافات جولة ٧١")) {
    const block = `  /* ── إضافات جولة ٧١ ── */\n` + FAWAID_ITEMS.map(renderFawaidItem).join(",\n");
    if (apply) {
  ...
  if (!storiesContent.includes("id: 225,")) {
...
async function verifyCounts() {
  ...
  const round71Idx = fawaidSrc.indexOf("إضافات جولة ٧١");
  const fawaidRound71Block =
    round71Idx >= 0 ? fawaidSrc.slice(round71Idx).split("{ text:").length - 1 : 0;
  ...
  return {
    ...
    quizRound71ShortExpl: quiz.filter((q) => {
      const n = parseInt((q.id || "").replace("demo-quiz-", ""), 10);
      return n >= 2365 && (q.explanation || "").length < QUIZ_EXPL_MIN;
    }).length,
    quizRound71: quiz.filter((q) => {
      const n = parseInt((q.id || "").replace("demo-quiz-", ""), 10);
      return n >= 2365 && n <= 2414;
    }).length,
    qaRound71: qa.filter((q) => {
      const n = parseInt((q.id || "").replace("seed-qa-", ""), 10);
      return n >= 1470 && n <= 1509;
    }).length,
    storiesRound71: stories.filter((s) => s.id >= 225 && s.id <= 229).length,
    fawaidRound71Block,
    fawaidRound71Short145: (() => {
      if (round71Idx < 0) return 999;
      const block = fawaidSrc.slice(round71Idx);
...
if (verify) {
  ...
  const fail =
    ...
    (a.quizRound71ShortExpl ?? 1) > 0 ||
    (a.quizRound71 ?? 0) !== 50 ||
    (a.qaRound71 ?? 0) !== 40 ||
    (a.storiesRound71 ?? 0) !== 5 ||
    (a.fawaidRound71Block ?? 0) !== 25 ||
    (a.fawaidRound71Short145 ?? 1) > 0;
