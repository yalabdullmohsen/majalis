#!/usr/bin/env node
/** Round 97 — raise fiqh-issues, landmarks, occasions educational fields. */
import fs from "node:fs";
import path from "node:path";
import { fileURLToPath } from "node:url";
const __dirname = path.dirname(fileURLToPath(import.meta.url));
const LIB = path.join(__dirname, "../src/lib");
const FIQH_SUMMARY_MIN = 300;
const FIQH_DESC_MIN = 380;
const LANDMARK_MIN = 420;
const OCCASION_MIN = 380;
const CLAUSES = [
