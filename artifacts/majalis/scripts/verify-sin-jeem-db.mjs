#!/usr/bin/env node
/**
 * Verify Sin Jeem database schema + seeded data (production integration).
 * Skips gracefully when credentials missing; fails when tables missing with credentials present.
 */
import { createClient } from "@supabase/supabase-js";
import { verifySinJeemSchema } from "../lib/sin-jeem-migration.mjs";
import { SIN_JEEM_TABLES } from "../lib/sin-jeem-migration.mjs";

let pass = 0;
let fail = 0;
let skip = 0;

function ok(c, m) {
  if (c) {
    pass++;
    console.log(`✓ ${m}`);
  } else {
    fail++;
    console.error(`✗ ${m}`);
  }
}

function skipped(m) {
  skip++;
  console.log(`⊘ ${m}`);
}

async function main() {
  const url = process.env.VITE_SUPABASE_URL || process.env.SUPABASE_URL;
  const serviceKey = process.env.SUPABASE_SERVICE_ROLE_KEY;
  const hasDb = Boolean(process.env.DATABASE_URL || process.env.SUPABASE_ACCESS_TOKEN);

  if (hasDb) {
    const schema = await verifySinJeemSchema();
    ok(schema.ok, `Schema verify (${schema.tableCount}/${SIN_JEEM_TABLES.length} tables)`);
    if (schema.missing?.length) ok(false, `Missing tables: ${schema.missing.join(", ")}`);
    ok((schema.indexes?.length || 0) >= 5, `Indexes present (${schema.indexes?.length || 0})`);
    if (schema.missingFks?.length) ok(false, `Missing FKs: ${schema.missingFks.join(", ")}`);
    else ok(true, "Foreign keys verified");
  } else {
    skipped("DATABASE_URL not set — pg schema verify skipped");
  }

  if (!url) {
    skipped("SUPABASE_URL not set — REST checks skipped");
    console.log(`\nSin Jeem DB verify: ${pass} passed, ${fail} failed, ${skip} skipped`);
    process.exit(fail > 0 ? 1 : 0);
  }

  const key = serviceKey || process.env.VITE_SUPABASE_ANON_KEY;
  const sb = createClient(url, key);

  for (const table of SIN_JEEM_TABLES) {
    const { error } = await sb.from(table).select("id").limit(1);
    if (error?.code === "PGRST205" || error?.code === "42P01") {
      ok(false, `Table exists: ${table}`);
    } else if (error && !serviceKey) {
      skipped(`Table ${table}: ${error.message}`);
    } else {
      ok(!error, `Table reachable: ${table}`);
    }
  }

  if (serviceKey) {
    const admin = createClient(url, serviceKey, { auth: { persistSession: false } });
    const { count: qCount } = await admin
      .from("sin_jeem_questions")
      .select("*", { count: "exact", head: true })
      .eq("status", "published");
    ok((qCount || 0) >= 500, `Published questions >= 500 (${qCount || 0})`);

    const { count: cCount } = await admin
      .from("sin_jeem_categories")
      .select("*", { count: "exact", head: true });
    ok((cCount || 0) >= 20, `Categories >= 20 (${cCount || 0})`);
  } else {
    skipped("SERVICE_ROLE_KEY not set — row count checks skipped");
  }

  console.log(`\nSin Jeem DB verify: ${pass} passed, ${fail} failed, ${skip} skipped`);
  process.exit(fail > 0 ? 1 : 0);
}

main().catch((err) => {
  console.error(err);
  process.exit(1);
});
