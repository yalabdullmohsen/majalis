#!/usr/bin/env node
/**
 * Round 75 bulk content — quiz/QA/fawaid/stories.
 * Usage: node scripts/enrich-round75-content.mjs [--apply] [--verify]
 */
import fs from "node:fs";
import path from "node:path";
import { fileURLToPath } from "node:url";
import { QUIZ_ITEMS, QA_ITEMS, FAWAID_ITEMS } from "./r75-content-data.mjs";
...
function raiseQuizExplanations(apply) {
  ...
  for (const q of quiz) {
    ...
    if (Number.isFinite(n) && n >= 2565 && (q.explanation || "").length < QUIZ_EXPL_MIN) {
...
function addSeeds(apply) {
  ...
  if (!qaContent.includes("seed-qa-1630")) {
  ...
  if (!fawaidContent.includes("إضافات جولة ٧٥")) {
    const block = `  /* ── إضافات جولة ٧٥ ── */\n` + FAWAID_ITEMS.map(renderFawaidItem).join(",\n");
    if (apply) {
  ...
  if (!storiesContent.includes("id: 245,")) {
...
async function verifyCounts() {
  ...
  const round75Idx = fawaidSrc.indexOf("إضافات جولة ٧٥");
  const fawaidRound75Block =
    round75Idx >= 0 ? fawaidSrc.slice(round75Idx).split("{ text:").length - 1 : 0;
  ...
  return {
    ...
    quizRound75ShortExpl: quiz.filter((q) => {
      const n = parseInt((q.id || "").replace("demo-quiz-", ""), 10);
      return n >= 2565 && (q.explanation || "").length < QUIZ_EXPL_MIN;
    }).length,
    quizRound75: quiz.filter((q) => {
      const n = parseInt((q.id || "").replace("demo-quiz-", ""), 10);
      return n >= 2565 && n <= 2614;
    }).length,
    qaRound75: qa.filter((q) => {
      const n = parseInt((q.id || "").replace("seed-qa-", ""), 10);
      return n >= 1630 && n <= 1669;
    }).length,
    storiesRound75: stories.filter((s) => s.id >= 245 && s.id <= 249).length,
    fawaidRound75Block,
    fawaidRound75Short145: (() => {
      if (round75Idx < 0) return 999;
      const block = fawaidSrc.slice(round75Idx);
...
if (verify) {
  ...
  const fail =
    ...
    (a.quizRound75ShortExpl ?? 1) > 0 ||
    (a.quizRound75 ?? 0) !== 50 ||
    (a.qaRound75 ?? 0) !== 40 ||
    (a.storiesRound75 ?? 0) !== 5 ||
    (a.fawaidRound75Block ?? 0) !== 25 ||
    (a.fawaidRound75Short145 ?? 1) > 0;
