#!/usr/bin/env node
/**
 * Round 63 bulk content — quiz/QA/fawaid/stories.
 * Usage: node scripts/enrich-round63-content.mjs [--apply] [--verify]
 */
import fs from "node:fs";
import path from "node:path";
import { fileURLToPath } from "node:url";
import { QUIZ_ITEMS, QA_ITEMS, FAWAID_ITEMS } from "./r63-content-data.mjs";
...
function raiseQuizExplanations(apply) {
  ...
  for (const q of quiz) {
    ...
    if (Number.isFinite(n) && n >= 1965 && (q.explanation || "").length < QUIZ_EXPL_MIN) {
...
function addSeeds(apply) {
  ...
  if (!qaContent.includes("seed-qa-1150")) {
  ...
  if (!fawaidContent.includes("إضافات جولة ٦٣")) {
    const block = `  /* ── إضافات جولة ٦٣ ── */\n` + FAWAID_ITEMS.map(renderFawaidItem).join(",\n");
    if (apply) {
  ...
  if (!storiesContent.includes("id: 185,")) {
...
async function verifyCounts() {
  ...
  const round63Idx = fawaidSrc.indexOf("إضافات جولة ٦٣");
  const fawaidRound63Block =
    round63Idx >= 0 ? fawaidSrc.slice(round63Idx).split("{ text:").length - 1 : 0;
  ...
  return {
    ...
    quizRound63ShortExpl: quiz.filter((q) => {
      const n = parseInt((q.id || "").replace("demo-quiz-", ""), 10);
      return n >= 1965 && (q.explanation || "").length < QUIZ_EXPL_MIN;
    }).length,
    quizRound63: quiz.filter((q) => {
      const n = parseInt((q.id || "").replace("demo-quiz-", ""), 10);
      return n >= 1965 && n <= 2014;
    }).length,
    qaRound63: qa.filter((q) => {
      const n = parseInt((q.id || "").replace("seed-qa-", ""), 10);
      return n >= 1150 && n <= 1189;
    }).length,
    storiesRound63: stories.filter((s) => s.id >= 185 && s.id <= 189).length,
    fawaidRound63Block,
    fawaidRound63Short145: (() => {
      if (round63Idx < 0) return 999;
      const block = fawaidSrc.slice(round63Idx);
...
if (verify) {
  ...
  const fail =
    ...
    (a.quizRound63ShortExpl ?? 1) > 0 ||
    (a.quizRound63 ?? 0) !== 50 ||
    (a.qaRound63 ?? 0) !== 40 ||
    (a.storiesRound63 ?? 0) !== 5 ||
    (a.fawaidRound63Block ?? 0) !== 25 ||
    (a.fawaidRound63Short145 ?? 1) > 0;
