#!/usr/bin/env node
/**
 * Round 69 bulk content — quiz/QA/fawaid/stories.
 * Usage: node scripts/enrich-round67-content.mjs [--apply] [--verify]
 */
import fs from "node:fs";
import path from "node:path";
import { fileURLToPath } from "node:url";
import { QUIZ_ITEMS, QA_ITEMS, FAWAID_ITEMS } from "./r69-content-data.mjs";
...
function raiseQuizExplanations(apply) {
  ...
  for (const q of quiz) {
    ...
    if (Number.isFinite(n) && n >= 2265 && (q.explanation || "").length < QUIZ_EXPL_MIN) {
...
function addSeeds(apply) {
  ...
  if (!qaContent.includes("seed-qa-1390")) {
  ...
  if (!fawaidContent.includes("إضافات جولة ٦٩")) {
    const block = `  /* ── إضافات جولة ٦٩ ── */\n` + FAWAID_ITEMS.map(renderFawaidItem).join(",\n");
    if (apply) {
  ...
  if (!storiesContent.includes("id: 215,")) {
...
async function verifyCounts() {
  ...
  const round69Idx = fawaidSrc.indexOf("إضافات جولة ٦٩");
  const fawaidRound69Block =
    round69Idx >= 0 ? fawaidSrc.slice(round69Idx).split("{ text:").length - 1 : 0;
  ...
  return {
    ...
    quizRound69ShortExpl: quiz.filter((q) => {
      const n = parseInt((q.id || "").replace("demo-quiz-", ""), 10);
      return n >= 2265 && (q.explanation || "").length < QUIZ_EXPL_MIN;
    }).length,
    quizRound69: quiz.filter((q) => {
      const n = parseInt((q.id || "").replace("demo-quiz-", ""), 10);
      return n >= 2265 && n <= 2314;
    }).length,
    qaRound69: qa.filter((q) => {
      const n = parseInt((q.id || "").replace("seed-qa-", ""), 10);
      return n >= 1390 && n <= 1429;
    }).length,
    storiesRound69: stories.filter((s) => s.id >= 215 && s.id <= 219).length,
    fawaidRound69Block,
    fawaidRound69Short145: (() => {
      if (round69Idx < 0) return 999;
      const block = fawaidSrc.slice(round69Idx);
...
if (verify) {
  ...
  const fail =
    ...
    (a.quizRound69ShortExpl ?? 1) > 0 ||
    (a.quizRound69 ?? 0) !== 50 ||
    (a.qaRound69 ?? 0) !== 40 ||
    (a.storiesRound69 ?? 0) !== 5 ||
    (a.fawaidRound69Block ?? 0) !== 25 ||
    (a.fawaidRound69Short145 ?? 1) > 0;
