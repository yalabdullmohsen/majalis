#!/usr/bin/env node
/**
 * Round 67 — lesson bodies ≥420, pages meta ≥350 (educational), nations ≥350,
 * updates≥220, circles/mutashabihat≥250, mind-maps/amr≥260, latin corruption scan.
 * Usage: node scripts/enrich-round67.mjs [--apply] [--verify]
 */
...
const PAGE_MIN = 350;
const NATIONS_MIN = 350;
const UPDATE_SUMMARY_MIN = 220;
const QURAN_CIRCLE_MIN = 250;
const MUTA_MIN = 250;
const MIND_MAP_MIN = 260;
const AMR_EXPL_MIN = 260;
...
function lessonBodyReport(verifyMode = false) {
  ...
  const out = execSync(`node scripts/enrich-r67-lesson-bodies.mjs ${flag}`.trim(), {
    cwd: ROOT,
    encoding: "utf8",
  });
...
if (apply) {
  execSync("node scripts/enrich-r67-lesson-bodies.mjs --apply", { cwd: ROOT, stdio: "inherit" });
}
