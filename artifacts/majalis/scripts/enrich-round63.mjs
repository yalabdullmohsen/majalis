#!/usr/bin/env node
/**
 * Round 63 — lesson bodies ≥380, pages meta ≥310 (educational), nations ≥310,
 * updates≥200, circles/mutashabihat≥230, mind-maps/amr≥240, latin corruption scan.
 * Usage: node scripts/enrich-round63.mjs [--apply] [--verify]
 */
...
const PAGE_MIN = 310;
const NATIONS_MIN = 310;
const UPDATE_SUMMARY_MIN = 200;
const QURAN_CIRCLE_MIN = 230;
const MUTA_MIN = 230;
const MIND_MAP_MIN = 240;
const AMR_EXPL_MIN = 240;
...
function lessonBodyReport(verifyMode = false) {
  ...
  const out = execSync(`node scripts/enrich-r63-lesson-bodies.mjs ${flag}`.trim(), {
    cwd: ROOT,
    encoding: "utf8",
  });
...
if (apply) {
  execSync("node scripts/enrich-r63-lesson-bodies.mjs --apply", { cwd: ROOT, stdio: "inherit" });
}
