#!/usr/bin/env node
/**
 * Round 71 — lesson bodies ≥460, pages meta ≥390 (educational), nations ≥390,
 * updates≥240, circles/mutashabihat≥270, mind-maps/amr≥280, latin corruption scan.
 * Usage: node scripts/enrich-round71.mjs [--apply] [--verify]
 */
...
const PAGE_MIN = 390;
const NATIONS_MIN = 390;
const UPDATE_SUMMARY_MIN = 240;
const QURAN_CIRCLE_MIN = 270;
const MUTA_MIN = 270;
const MIND_MAP_MIN = 280;
const AMR_EXPL_MIN = 280;
...
function lessonBodyReport(verifyMode = false) {
  ...
  const out = execSync(`node scripts/enrich-r71-lesson-bodies.mjs ${flag}`.trim(), {
    cwd: ROOT,
    encoding: "utf8",
  });
...
if (apply) {
  execSync("node scripts/enrich-r71-lesson-bodies.mjs --apply", { cwd: ROOT, stdio: "inherit" });
}
