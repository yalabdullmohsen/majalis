#!/usr/bin/env node
/**
 * Round 73 bulk content — quiz/QA/fawaid/stories.
 * Usage: node scripts/enrich-round73-content.mjs [--apply] [--verify]
 */
import fs from "node:fs";
import path from "node:path";
import { fileURLToPath } from "node:url";
import { QUIZ_ITEMS, QA_ITEMS, FAWAID_ITEMS } from "./r73-content-data.mjs";
...
function raiseQuizExplanations(apply) {
  ...
  for (const q of quiz) {
    ...
    if (Number.isFinite(n) && n >= 2465 && (q.explanation || "").length < QUIZ_EXPL_MIN) {
...
function addSeeds(apply) {
  ...
  if (!qaContent.includes("seed-qa-1550")) {
  ...
  if (!fawaidContent.includes("إضافات جولة ٧٣")) {
    const block = `  /* ── إضافات جولة ٧٣ ── */\n` + FAWAID_ITEMS.map(renderFawaidItem).join(",\n");
    if (apply) {
  ...
  if (!storiesContent.includes("id: 235,")) {
...
async function verifyCounts() {
  ...
  const round73Idx = fawaidSrc.indexOf("إضافات جولة ٧٣");
  const fawaidRound73Block =
    round73Idx >= 0 ? fawaidSrc.slice(round73Idx).split("{ text:").length - 1 : 0;
  ...
  return {
    ...
    quizRound73ShortExpl: quiz.filter((q) => {
      const n = parseInt((q.id || "").replace("demo-quiz-", ""), 10);
      return n >= 2465 && (q.explanation || "").length < QUIZ_EXPL_MIN;
    }).length,
    quizRound73: quiz.filter((q) => {
      const n = parseInt((q.id || "").replace("demo-quiz-", ""), 10);
      return n >= 2465 && n <= 2514;
    }).length,
    qaRound73: qa.filter((q) => {
      const n = parseInt((q.id || "").replace("seed-qa-", ""), 10);
      return n >= 1550 && n <= 1589;
    }).length,
    storiesRound73: stories.filter((s) => s.id >= 235 && s.id <= 239).length,
    fawaidRound73Block,
    fawaidRound73Short145: (() => {
      if (round73Idx < 0) return 999;
      const block = fawaidSrc.slice(round73Idx);
...
if (verify) {
  ...
  const fail =
    ...
    (a.quizRound73ShortExpl ?? 1) > 0 ||
    (a.quizRound73 ?? 0) !== 50 ||
    (a.qaRound73 ?? 0) !== 40 ||
    (a.storiesRound73 ?? 0) !== 5 ||
    (a.fawaidRound73Block ?? 0) !== 25 ||
    (a.fawaidRound73Short145 ?? 1) > 0;
