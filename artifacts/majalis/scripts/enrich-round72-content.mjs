#!/usr/bin/env node
/**
 * Round 72 bulk content — quiz/QA/fawaid/stories/PM + scholars/sheikhs/prophets raises.
 * Usage: node scripts/enrich-round72-content.mjs [--apply] [--verify]
 */
import fs from "node:fs";
import path from "node:path";
import { fileURLToPath } from "node:url";
import { QUIZ_ITEMS, QA_ITEMS, FAWAID_ITEMS } from "./r72-content-data.mjs";
...
const SCHOLAR_BIO_MIN = 580;
const SHEIKH_BIO_MIN = 350;
const PROPHET_BIO_MIN = 540;
const PROPHET_LESSON_MIN = 200;

const PM_R72_IDS = [
  "figs-nutrition-r72",
  "barley-sunnah-r72",
  "pomegranate-wellness-r72",
  "henna-sunnah-r72",
  "ginger-wellness-r72",
];
...
function raiseQuizExplanations(apply) {
  ...
  for (const q of quiz) {
    ...
    if (Number.isFinite(n) && n >= 2415 && (q.explanation || "").length < QUIZ_EXPL_MIN) {
...
function readStoriesBlock() {
  return fs.readFileSync(path.join(__dirname, "r72-original-stories.ts"), "utf8").trim();
}

function readPmBlock() {
  return fs.readFileSync(path.join(__dirname, "r72-prophetic-medicine.ts"), "utf8").trim();
}

function addSeeds(apply) {
  const stats = { quiz: 0, qa: 0, fawaid: 0, stories: 0, pm: 0 };
  const marker = "جولة ٧٢";

  const quizPath = path.join(LIB, "quiz-seed.ts");
  let quizContent = fs.readFileSync(quizPath, "utf8");
  if (!quizContent.includes(marker)) {
    const block = `  /* ───────── ${marker}: أقسام أضعف (2415-2464) ───────── */\n` +
  ...
  if (!qaContent.includes("seed-qa-1510")) {
  ...
  if (!fawaidContent.includes("إضافات جولة ٧٢")) {
    const block = `  /* ── إضافات جولة ٧٢ ── */\n` + FAWAID_ITEMS.map(renderFawaidItem).join(",\n");
    if (apply) {
  ...
  if (!storiesContent.includes("id: 230,")) {
  ...
  if (!pmContent.includes("figs-nutrition-r72")) {
...
async function verifyCounts() {
  ...
  const round72Idx = fawaidSrc.indexOf("إضافات جولة ٧٢");
  const fawaidRound72Block =
    round72Idx >= 0 ? fawaidSrc.slice(round72Idx).split("{ text:").length - 1 : 0;
  ...
  return {
    ...
    quizRound72ShortExpl: quiz.filter((q) => {
      const n = parseInt((q.id || "").replace("demo-quiz-", ""), 10);
      return n >= 2415 && (q.explanation || "").length < QUIZ_EXPL_MIN;
    }).length,
    quizRound72: quiz.filter((q) => {
      const n = parseInt((q.id || "").replace("demo-quiz-", ""), 10);
      return n >= 2415 && n <= 2464;
    }).length,
    qaRound72: qa.filter((q) => {
      const n = parseInt((q.id || "").replace("seed-qa-", ""), 10);
      return n >= 1510 && n <= 1549;
    }).length,
    storiesRound72: stories.filter((s) => s.id >= 230 && s.id <= 234).length,
    pmRound72: pm.filter((x) => PM_R72_IDS.includes(x.id)).length,
    fawaidRound72Block,
    fawaidRound72Short145: (() => {
      if (round72Idx < 0) return 999;
      const block = fawaidSrc.slice(round72Idx);
      const texts = [...block.matchAll(/text:\s*"((?:[^"\\]|\\.)*)"/g)].map((m) => m[1]);
      return texts.filter((t) => t.length < FAWAID_MIN).length;
    })(),
    scholarsShort580: scholars.filter((s) => s.bio.length < SCHOLAR_BIO_MIN).length,
    sheikhsShort350: sheikhsMod.SHEIKHS_SEED.filter((s) => (s.bio || "").length < SHEIKH_BIO_MIN).length,
    prophetsBriefShort540: prophets.filter((p) => (p.briefBio || "").length < PROPHET_BIO_MIN).length,
    prophetsLessonShort200: prophets.reduce(
...
if (verify) {
  ...
  const fail =
    ...
    (a.quizRound72ShortExpl ?? 1) > 0 ||
    (a.quizRound72 ?? 0) !== 50 ||
    (a.qaRound72 ?? 0) !== 40 ||
    (a.storiesRound72 ?? 0) !== 5 ||
    (a.pmRound72 ?? 0) !== 5 ||
    (a.fawaidRound72Block ?? 0) !== 25 ||
    (a.fawaidRound72Short145 ?? 1) > 0 ||
    (a.scholarsShort580 ?? 1) > 0 ||
    (a.sheikhsShort350 ?? 1) > 0 ||
    (a.prophetsBriefShort540 ?? 1) > 0 ||
    (a.prophetsLessonShort200 ?? 1) > 0;
