#!/usr/bin/env node
/**
 * Round 57 bulk content — quiz/QA/fawaid/stories.
 * Usage: node scripts/enrich-round57-content.mjs [--apply] [--verify]
 */
import fs from "node:fs";
import path from "node:path";
import { fileURLToPath } from "node:url";
import { QUIZ_ITEMS, QA_ITEMS, FAWAID_ITEMS } from "./r57-content-data.mjs";
...
function raiseQuizExplanations(apply) {
  ...
  for (const q of quiz) {
    ...
    if (Number.isFinite(n) && n >= 1665 && (q.explanation || "").length < QUIZ_EXPL_MIN) {
...
function addSeeds(apply) {
  ...
  if (!qaContent.includes("seed-qa-910")) {
  ...
  if (!fawaidContent.includes("إضافات جولة ٥٧")) {
    const block = `  /* ── إضافات جولة ٥٧ ── */\n` + FAWAID_ITEMS.map(renderFawaidItem).join(",\n");
    if (apply) {
  ...
  if (!storiesContent.includes("id: 155,")) {
...
async function verifyCounts() {
  ...
  const round57Idx = fawaidSrc.indexOf("إضافات جولة ٥٧");
  const fawaidRound57Block =
    round57Idx >= 0 ? fawaidSrc.slice(round57Idx).split("{ text:").length - 1 : 0;
  ...
  return {
    ...
    quizRound57ShortExpl: quiz.filter((q) => {
      const n = parseInt((q.id || "").replace("demo-quiz-", ""), 10);
      return n >= 1665 && (q.explanation || "").length < QUIZ_EXPL_MIN;
    }).length,
    quizRound57: quiz.filter((q) => {
      const n = parseInt((q.id || "").replace("demo-quiz-", ""), 10);
      return n >= 1665 && n <= 1714;
    }).length,
    qaRound57: qa.filter((q) => {
      const n = parseInt((q.id || "").replace("seed-qa-", ""), 10);
      return n >= 910 && n <= 949;
    }).length,
    storiesRound57: stories.filter((s) => s.id >= 155 && s.id <= 159).length,
    fawaidRound57Block,
    fawaidRound57Short145: (() => {
      if (round57Idx < 0) return 999;
      const block = fawaidSrc.slice(round57Idx);
...
if (verify) {
  ...
  const fail =
    ...
    (a.quizRound57ShortExpl ?? 1) > 0 ||
    (a.quizRound57 ?? 0) !== 50 ||
    (a.qaRound57 ?? 0) !== 40 ||
    (a.storiesRound57 ?? 0) !== 5 ||
    (a.fawaidRound57Block ?? 0) !== 25 ||
    (a.fawaidRound57Short145 ?? 1) > 0;
