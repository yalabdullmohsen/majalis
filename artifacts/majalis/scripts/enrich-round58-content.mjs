#!/usr/bin/env node
/**
 * Round 58 bulk content — quiz/QA/fawaid/stories/PM + scholars/sheikhs/prophets raises.
 * Usage: node scripts/enrich-round58-content.mjs [--apply] [--verify]
 */
import fs from "node:fs";
import path from "node:path";
import { fileURLToPath } from "node:url";
import { QUIZ_ITEMS, QA_ITEMS, FAWAID_ITEMS } from "./r58-content-data.mjs";
...
const SCHOLAR_BIO_MIN = 440;
const SHEIKH_BIO_MIN = 280;
const PROPHET_BIO_MIN = 400;
const PROPHET_LESSON_MIN = 130;

const PM_R58_IDS = [
  "eat-from-near-side",
  "avoid-hot-blow-food",
  "wudu-before-sleep",
  "praising-food-sunnah",
  "early-sleep-sunnah",
];
...
function raiseQuizExplanations(apply) {
  ...
  for (const q of quiz) {
    ...
    if (Number.isFinite(n) && n >= 1715 && (q.explanation || "").length < QUIZ_EXPL_MIN) {
...
function readStoriesBlock() {
  return fs.readFileSync(path.join(__dirname, "r58-original-stories.ts"), "utf8").trim();
}

function readPmBlock() {
  return fs.readFileSync(path.join(__dirname, "r58-prophetic-medicine.ts"), "utf8").trim();
}

function addSeeds(apply) {
  const stats = { quiz: 0, qa: 0, fawaid: 0, stories: 0, pm: 0 };
  const marker = "جولة ٥٨";

  const quizPath = path.join(LIB, "quiz-seed.ts");
  let quizContent = fs.readFileSync(quizPath, "utf8");
  if (!quizContent.includes(marker)) {
    const block = `  /* ───────── ${marker}: أقسام أضعف (1715-1764) ───────── */\n` +
  ...
  if (!qaContent.includes("seed-qa-950")) {
  ...
  if (!fawaidContent.includes("إضافات جولة ٥٨")) {
    const block = `  /* ── إضافات جولة ٥٨ ── */\n` + FAWAID_ITEMS.map(renderFawaidItem).join(",\n");
    if (apply) {
  ...
  if (!storiesContent.includes("id: 160,")) {
  ...
  if (!pmContent.includes("eat-from-near-side")) {
...
async function verifyCounts() {
  ...
  const round58Idx = fawaidSrc.indexOf("إضافات جولة ٥٨");
  const fawaidRound58Block =
    round58Idx >= 0 ? fawaidSrc.slice(round58Idx).split("{ text:").length - 1 : 0;
  ...
  return {
    ...
    quizRound58ShortExpl: quiz.filter((q) => {
      const n = parseInt((q.id || "").replace("demo-quiz-", ""), 10);
      return n >= 1715 && (q.explanation || "").length < QUIZ_EXPL_MIN;
    }).length,
    quizRound58: quiz.filter((q) => {
      const n = parseInt((q.id || "").replace("demo-quiz-", ""), 10);
      return n >= 1715 && n <= 1764;
    }).length,
    qaRound58: qa.filter((q) => {
      const n = parseInt((q.id || "").replace("seed-qa-", ""), 10);
      return n >= 950 && n <= 989;
    }).length,
    storiesRound58: stories.filter((s) => s.id >= 160 && s.id <= 164).length,
    pmRound58: pm.filter((x) => PM_R58_IDS.includes(x.id)).length,
    fawaidRound58Block,
    fawaidRound58Short145: (() => {
      if (round58Idx < 0) return 999;
      const block = fawaidSrc.slice(round58Idx);
      const texts = [...block.matchAll(/text:\s*"((?:[^"\\]|\\.)*)"/g)].map((m) => m[1]);
      return texts.filter((t) => t.length < FAWAID_MIN).length;
    })(),
    scholarsShort440: scholars.filter((s) => s.bio.length < SCHOLAR_BIO_MIN).length,
    sheikhsShort280: sheikhsMod.SHEIKHS_SEED.filter((s) => (s.bio || "").length < SHEIKH_BIO_MIN).length,
    prophetsBriefShort400: prophets.filter((p) => (p.briefBio || "").length < PROPHET_BIO_MIN).length,
    prophetsLessonShort130: prophets.reduce(
...
if (verify) {
  ...
  const fail =
    ...
    (a.quizRound58ShortExpl ?? 1) > 0 ||
    (a.quizRound58 ?? 0) !== 50 ||
    (a.qaRound58 ?? 0) !== 40 ||
    (a.storiesRound58 ?? 0) !== 5 ||
    (a.pmRound58 ?? 0) !== 5 ||
    (a.fawaidRound58Block ?? 0) !== 25 ||
    (a.fawaidRound58Short145 ?? 1) > 0 ||
    (a.scholarsShort440 ?? 1) > 0 ||
    (a.sheikhsShort280 ?? 1) > 0 ||
    (a.prophetsBriefShort400 ?? 1) > 0 ||
    (a.prophetsLessonShort130 ?? 1) > 0;
