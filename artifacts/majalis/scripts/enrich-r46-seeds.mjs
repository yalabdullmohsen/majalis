#!/usr/bin/env node
/**
 * Round 46 — add quiz/QA/fawaid/stories + raise scholars≥320, sheikhs≥220, prophet lessons≥110.
 * Usage: node scripts/enrich-r46-seeds.mjs [--apply] [--verify]
 */
...
/* ── Quiz questions 1180-1204 ── */
const QUIZ_BLOCK = `  /* ───────── جولة ٤٦: أقسام أضعف (1180-1204) ───────── */
  {
    "id": "demo-quiz-1180",
    "section": "الطب النبوي",
...
/* ── QA 525-549 (abbreviated in script, full in file) ── */
function buildQaBlock() {
  const items = [
    ["525", "ما حكم صلاة الجمعة؟", "الجواب: صلاة الجمعة فرض على كل مسلم بالغ عاقل ذكر مقيم — قال ﷺ: «من ترك ثلاث جمع تهاوناً طبع الله على قلبه». رواه أبو داود (1052) وصححه الألباني. وهي ركعتان تُصلّى بعد الأذان الثاني.", "seed-cat-salah", "واجب", "صحيح أبي داود 1052"],
    ["526", "ما حكم صيام يوم عرفة؟", "الجواب: صيام يوم عرفة لغير الحاج مستحب؛ قال ﷺ: «صيام يوم عرفة أحتسب على الله أن يكفر السنة التي قبله والتي بعده». رواه مسلم (1162). والحاج لا يصومه على الراجح.", "seed-cat-sawm", "مستحب", "صحيح مسلم 1162"],
    ["527", "ما حكم الوصية لغير الوارث؟", "الجواب: الوصية لغير الوارث جائزة بثلث المال فقط — {مِن بَعْدِ وَصِيَّةٍ يُوصَىٰ بِهَا أَوْ دَيْنٍ} — النساء: 11. ولا تُجاوز الثلث إلا برضا الورثة.", "seed-cat-fiqh", "جائز", "سورة النساء: 11"],
    ["528", "ما حكم قراءة الفاتحة في الصلاة؟", "الجواب: قراءة الفاتحة ركن في كل ركعة عند جمهور العلماء؛ قال ﷺ: «لا صلاة لمن لم يقرأ بفاتحة الكتاب». رواه البخاري (756) ومسلم (394).", "seed-cat-salah", "واجب", "صحيح البخاري 756"],
    ["529", "ما حكم النية في الوضوء؟", "الجواب: النية شرط لصحة الوضوء عند جمهور العلماء — {وَمَا أُمِرُوا إِلَّا لِيَعْبُدُوا اللَّهَ مُخْلِصِينَ لَهُ الدِّينَ} — البينة: 5. ويُستحب تلفظها.", "seed-cat-tahara", "واجب", "سورة البينة: 5"],
    ["530", "ما حكم الزكاة في الذهب؟", "الجواب: زكاة الذهب واجبة إذا بلغ النصاب (85 جراماً) وحال عليه الحول — {وَآتُوا حَقَّهُ يَوْمَ حَصَادِهِ} — الأنعام: 141. ونسبتها 2.5%.", "seed-cat-zakat", "واجب", "سورة الأنعام: 141"],
    ["531", "ما حكم صلاة العيد؟", "الجواب: صلاة العيد سنة مؤكدة؛ قال ﷺ: «كان النبي ﷺ إذا كان يوم عيد أمر بإخراج الصبيان والنساء». رواه البخاري (912). وهي ركعتان.", "seed-cat-salah", "سنة", "صحيح البخاري 912"],
    ["532", "ما حكم التيمم؟", "الجواب: التيمم بديل عن الوضوء والغسل عند عدم الماء أو العجز — {فَتَيَمَّمُوا صَعِيدًا طَيِّبًا} — المائدة: 6. ويُبطل بوجود الماء.", "seed-cat-tahara", "جائز", "سورة المائدة: 6"],
    ["533", "ما حكم صيام يوم عاشوراء؟", "الجواب: صيام يوم عاشوراء سنة؛ قال ﷺ: «صيام يوم عاشوراء أحتسب على الله أن يكفر السنة التي قبله». رواه مسلم (1162). ويُستحب صيام التاسع معه.", "seed-cat-sawm", "مستحب", "صحيح مسلم 1162"],
    ["534", "ما حكم الحج على المسلم؟", "الجواب: الحج واجب مرة في العمر على المستطيع — {وَلِلَّهِ عَلَى النَّاسِ حِجُّ الْبَيْتِ} — آل عمران: 97. والاستطاعة تشمل المال والبدن والأمن.", "seed-cat-hajj", "واجب", "سورة آل عمران: 97"],
    ["535", "ما حكم قراءة القرآن في الركعة؟", "الجواب: قراءة القرآن في الصلاة واجبة في الركعة الأولى على الأقل عند جمهور العلماء. ويُستحب قراءة سورة بعد الفاتحة.", "seed-cat-salah", "واجب", "المغني — ابن قدامة"],
    ["536", "ما حكم الربا؟", "الجواب: الربا حرام بالاتفاق — {يَمْحَقُ اللَّهُ الرِّبَا وَيُرْبِي الصَّدَقَاتِ} — البقرة: 276. ويشمل ربا الفضل وربا النسيئة.", "seed-cat-fiqh", "حرام", "سورة البقرة: 276"],
    ["537", "ما حكم صلاة المسافر؟", "الجواب: قصر الصلاة الرباعية للمسافر جائز — {وَإِذَا ضَرَبْتُمْ فِي الْأَرْضِ} — النساء: 101. ويُشترط أن يكون السفر مسافة القصر.", "seed-cat-salah", "جائز", "سورة النساء: 101"],
    ["538", "ما حكم الوتر؟", "الجواب: الوتر سنة مؤكدة بعد العشاء — {وَأَقِمِ الصَّلَاةَ} — الإسراء: 78. ويُصلّى ركعة أو أكثر.", "seed-cat-salah", "سنة", "سورة الإسراء: 78"],
    ["539", "ما حكم صيام الست من شوال؟", "الجواب: صيام ستّة أيام من شوال سنة — «من صام رمضان ثم أتبعه ستّاً من شوال كان كصيام الدهر». رواه مسلم (1164).", "seed-cat-sawm", "مستحب", "صحيح مسلم 1164"],
    ["540", "ما حكم برّ الجيران؟", "الجواب: برّ الجيران واجب — قال ﷺ: «ما زال جبريل يوصيني بالجار حتى ظننت أنه سيورثه». رواه البخاري (6014) ومسلم (2624).", "seed-cat-adab", "واجب", "صحيح البخاري 6014"],
    ["541", "ما حكم قراءة سورة الملك؟", "الجواب: قراءة سورة الملك قبل النوم سنة — «إن سورة من القرآن ثلاثون آية شفعت لصاحبها». رواه الترمذي (2891) وصححه الألباني.", "seed-cat-quran", "مستحب", "سنن الترمذي 2891"],
    ["542", "ما حكم صلاة الاستسقاء؟", "الجواب: صلاة الاستسقاء سنة عند القحط — قال ﷺ: «استسقوا الله». رواه البخاري (967). وهي ركعتان.", "seed-cat-salah", "مستحب", "صحيح البخاري 967"],
    ["543", "ما حكم الغسل من الجنابة؟", "الجواب: الغسل من الجنابة واجب — {وَإِن كُنتُمْ جُنُبًا فَاطَّهَّرُوا} — المائدة: 6. وهو فرض قبل الصلاة.", "seed-cat-tahara", "واجب", "سورة المائدة: 6"],
    ["544", "ما حكم صيام يوم الاثنين والخميس؟", "الجواب: صيام يومي الاثنين والخميس سنة — «تُعرض الأعمال يوم الاثنين والخميس». رواه الترمذي (747) وصححه الألباني.", "seed-cat-sawm", "مستحب", "سنن الترمذي 747"],
    ["545", "ما حكم إيتاء الزكاة؟", "الجواب: إخراج الزكاة واجب — {خُذْ مِنْ أَمْوَالِهِمْ صَدَقَةً} — التوبة: 103. وتُخرج للمصارف الثمانية.", "seed-cat-zakat", "واجب", "سورة التوبة: 103"],
    ["546", "ما حكم صلاة التراويح؟", "الجواب: صلاة التراويح سنة في رمضان — «من قام رمضان إيماناً واحتساباً غُفر له ما تقدم من ذنبه». رواه البخاري (37).", "seed-cat-salah", "سنة", "صحيح البخاري 37"],
    ["547", "ما حكم قراءة آية الكرسي؟", "الجواب: قراءة آية الكرسي بعد كل صلاة سنة — «من قرأ آية الكرسي دبر كل صلاة لم يمنعه من دخول الجنة إلا أن يموت». رواه النسائي (9928).", "seed-cat-quran", "مستحب", "سنن النسائي 9928"],
    ["548", "ما حكم صيام يوم عرفة للحاج؟", "الجواب: صيام يوم عرفة للحاج مكروه عند جمهور العلماء؛ لأنه من شعائر الحج. ويُستحب للغير الحاج.", "seed-cat-sawm", "مكروه", "المغني — ابن قدامة"],
    ["549", "ما حكم صلاة الجنازة؟", "الجواب: صلاة الجنازة فرض كفاية — «من مات منكم فصلّوا عليه». رواه البخاري (1315). وهي أربع تكبيرات.", "seed-cat-salah", "واجب", "صحيح البخاري 1315"],
  ];
  const cats = {
  ...
  let block = `  /* ───────── جولة ٤٦: إثراء Q&A (525-549) ───────── */\n`;
...
const FAWAID_BLOCK = `  /* ── إضافات جولة ٤٦ ── */
...
const STORIES_BLOCK = `  /* ───────── جولة ٤٦: قصص (104-106) ───────── */
  {
    id: 104,
...
async function applyAll() {
  ...
  if (!quizContent.includes("demo-quiz-1180")) {
  ...
  if (!qaContent.includes("seed-qa-525")) {
  ...
  if (!fawaidContent.includes("إضافات جولة ٤٦")) {
...
async function verify() {
  ...
  return {
    ...
    quizRound46: DEMO_QUIZ_QUESTIONS.filter((q) => q.id?.startsWith("demo-quiz-118") || q.id?.startsWith("demo-quiz-119") || q.id?.startsWith("demo-quiz-120")).length,
    weakSectionCounts: sectionCounts,
    qaTotal: SEED_QA.length,
    qaShortAnswers: SEED_QA.filter((q) => (q.answer || "").length < QA_MIN).length,
    qaRound46: SEED_QA.filter((q) => q.id?.startsWith("seed-qa-5") && parseInt(q.id.split("-").pop()) >= 525).length,
