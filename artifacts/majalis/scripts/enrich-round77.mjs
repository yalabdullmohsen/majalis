#!/usr/bin/env node
/**
 * Round 77 — lesson bodies ≥520, pages meta ≥450 (educational), nations ≥450,
 * updates≥270, circles/mutashabihat≥300, mind-maps/amr≥310, latin corruption scan.
 * Usage: node scripts/enrich-round77.mjs [--apply] [--verify]
 */
...
const PAGE_MIN = 450;
const NATIONS_MIN = 450;
const UPDATE_SUMMARY_MIN = 270;
const QURAN_CIRCLE_MIN = 300;
const MUTA_MIN = 300;
const MIND_MAP_MIN = 310;
const AMR_EXPL_MIN = 310;
...
function lessonBodyReport(verifyMode = false) {
  ...
  const out = execSync(`node scripts/enrich-r77-lesson-bodies.mjs ${flag}`.trim(), {
    cwd: ROOT,
    encoding: "utf8",
  });
...
if (apply) {
  execSync("node scripts/enrich-r77-lesson-bodies.mjs --apply", { cwd: ROOT, stdio: "inherit" });
}
