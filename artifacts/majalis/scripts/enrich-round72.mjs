#!/usr/bin/env node
/**
 * Round 72 — lesson bodies ≥470, pages meta ≥400 (educational), nations ≥400,
 * fiqh summary≥270 description≥300, courses≥310 occasions≥300 landmarks≥520, latin corruption scan.
 * Usage: node scripts/enrich-round72.mjs [--apply] [--verify]
 */
...
const PAGE_MIN = 400;
const NATIONS_MIN = 400;
const FIQH_SUMMARY_MIN = 270;
const FIQH_DESC_MIN = 300;
const LANDMARK_DESC_MIN = 520;
const LANDMARK_SIG_MIN = 520;
const COURSE_SUMMARY_MIN = 310;
const OCCASION_SUMMARY_MIN = 300;
...
function lessonBodyReport(verifyMode = false) {
  ...
  const out = execSync(`node scripts/enrich-r72-lesson-bodies.mjs ${flag}`.trim(), {
    cwd: ROOT,
    encoding: "utf8",
  });
...
if (apply) {
  execSync("node scripts/enrich-r72-lesson-bodies.mjs --apply", { cwd: ROOT, stdio: "inherit" });
}
