#!/usr/bin/env node
/**
 * Round 82 bulk content — quiz/QA/fawaid/stories/PM.
 * Usage: node scripts/enrich-round82-content.mjs [--apply] [--verify]
 */
import fs from "node:fs";
import path from "node:path";
import { fileURLToPath } from "node:url";
import { QUIZ_ITEMS, QA_ITEMS, FAWAID_ITEMS, STORY_ITEMS, PM_ITEMS } from "./r82-content-data.mjs";
...
function addSeeds(apply) {
  ...
  if (!quizContent.includes("demo-quiz-2905")) {
    const block =
      "  /* ───────── جولة ٨٢: محتوى أفقي (2905-2954) ───────── */\n" +
  ...
  if (!qaContent.includes("seed-qa-1900")) {
  ...
  if (!fawaidContent.includes("إضافات جولة ٨٢")) {
    const block = "  /* ── إضافات جولة ٨٢ ── */\n" + FAWAID_ITEMS.map(renderFawaidItem).join(",\n");
    if (apply) {
  ...
  if (!storiesContent.includes("story-r82-1")) {
    const block =
  ...
  if (!pmContent.includes("fever-cooling-r82")) {
    const block =
...
async function verifyCounts() {
  ...
  const fawaid = countRoundFawaid(82);
  ...
  return {
    quizRound82: quizRound.length,
    quizRound82ShortAnswers: quizRound.filter((q) => (q.answer || "").length < QUIZ_ANSWER_MIN).length,
    quizRound82ShortExpl: quizRound.filter((q) => (q.explanation || "").length < QUIZ_EXPL_MIN).length,
    qaRound82: qaRound.length,
    qaRound82ShortAnswers: qaRound.filter((q) => (q.answer || "").length < QA_ANSWER_MIN).length,
    fawaidRound82Block: fawaid.count,
    fawaidRound82Short145: fawaid.short,
    storiesRound82: storiesRound.length,
    storiesRound82ShortContent: storiesRound.filter((s) => (s.full_content || "").length < STORY_CONTENT_MIN).length,
    pmRound82: pmRound.length,
    pmRound82WeakAsFixed: pmRound.filter((x) => /ضعيف/.test(x.hadithSource || "") && !/لا يثبت|ضعف|ضعيف/.test(`${x.body || ""} ${x.benefits?.join(" ") || ""}`)).length,
    ...
    pmRound82Ids: pmRound.map((x) => x.id),
  };
}
...
if (verify) {
  ...
  const fail =
    (a.quizRound82 ?? 0) !== 50 ||
    (a.quizRound82ShortAnswers ?? 1) > 0 ||
    (a.quizRound82ShortExpl ?? 1) > 0 ||
    (a.qaRound82 ?? 0) !== 40 ||
    (a.qaRound82ShortAnswers ?? 1) > 0 ||
    (a.fawaidRound82Block ?? 0) !== 25 ||
    (a.fawaidRound82Short145 ?? 1) > 0 ||
    (a.storiesRound82 ?? 0) !== 5 ||
    (a.storiesRound82ShortContent ?? 1) > 0 ||
    (a.pmRound82 ?? 0) !== 5 ||
    (a.pmRound82WeakAsFixed ?? 1) > 0 ||
    a.lastQuizId !== "demo-quiz-2954" ||
    a.lastQaId !== "seed-qa-1939" ||
    a.lastStorySlug !== "story-r82-5";
