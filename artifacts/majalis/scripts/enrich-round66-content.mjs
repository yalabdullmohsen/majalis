#!/usr/bin/env node
/**
 * Round 66 bulk content — quiz/QA/fawaid/stories/PM + scholars/sheikhs/prophets raises.
 * Usage: node scripts/enrich-round66-content.mjs [--apply] [--verify]
 */
import fs from "node:fs";
import path from "node:path";
import { fileURLToPath } from "node:url";
import { QUIZ_ITEMS, QA_ITEMS, FAWAID_ITEMS } from "./r66-content-data.mjs";
...
const SCHOLAR_BIO_MIN = 520;
const SHEIKH_BIO_MIN = 320;
const PROPHET_BIO_MIN = 480;
const PROPHET_LESSON_MIN = 170;

const PM_R66_IDS = [
  "honey-digestion-r66",
  "olive-oil-nutrition-r66",
  "black-seed-immunity-r66",
  "siwak-dental-r66",
  "cupping-moderation-r66",
];
...
function raiseQuizExplanations(apply) {
  ...
  for (const q of quiz) {
    ...
    if (Number.isFinite(n) && n >= 2115 && (q.explanation || "").length < QUIZ_EXPL_MIN) {
...
function readStoriesBlock() {
  return fs.readFileSync(path.join(__dirname, "r66-original-stories.ts"), "utf8").trim();
}

function readPmBlock() {
  return fs.readFileSync(path.join(__dirname, "r66-prophetic-medicine.ts"), "utf8").trim();
}

function addSeeds(apply) {
  const stats = { quiz: 0, qa: 0, fawaid: 0, stories: 0, pm: 0 };
  const marker = "جولة ٦٦";

  const quizPath = path.join(LIB, "quiz-seed.ts");
  let quizContent = fs.readFileSync(quizPath, "utf8");
  if (!quizContent.includes(marker)) {
    const block = `  /* ───────── ${marker}: أقسام أضعف (2115-2164) ───────── */\n` +
  ...
  if (!qaContent.includes("seed-qa-1270")) {
  ...
  if (!fawaidContent.includes("إضافات جولة ٦٦")) {
    const block = `  /* ── إضافات جولة ٦٦ ── */\n` + FAWAID_ITEMS.map(renderFawaidItem).join(",\n");
    if (apply) {
  ...
  if (!storiesContent.includes("id: 200,")) {
  ...
  if (!pmContent.includes("honey-digestion-r66")) {
...
async function verifyCounts() {
  ...
  const round66Idx = fawaidSrc.indexOf("إضافات جولة ٦٦");
  const fawaidRound66Block =
    round66Idx >= 0 ? fawaidSrc.slice(round66Idx).split("{ text:").length - 1 : 0;
  ...
  return {
    ...
    quizRound66ShortExpl: quiz.filter((q) => {
      const n = parseInt((q.id || "").replace("demo-quiz-", ""), 10);
      return n >= 2015 && (q.explanation || "").length < QUIZ_EXPL_MIN;
    }).length,
    quizRound66: quiz.filter((q) => {
      const n = parseInt((q.id || "").replace("demo-quiz-", ""), 10);
      return n >= 2115 && n <= 2164;
    }).length,
    qaRound66: qa.filter((q) => {
      const n = parseInt((q.id || "").replace("seed-qa-", ""), 10);
      return n >= 1270 && n <= 1309;
    }).length,
    storiesRound66: stories.filter((s) => s.id >= 200 && s.id <= 204).length,
    pmRound66: pm.filter((x) => PM_R66_IDS.includes(x.id)).length,
    fawaidRound66Block,
    fawaidRound66Short145: (() => {
      if (round66Idx < 0) return 999;
      const block = fawaidSrc.slice(round66Idx);
      const texts = [...block.matchAll(/text:\s*"((?:[^"\\]|\\.)*)"/g)].map((m) => m[1]);
      return texts.filter((t) => t.length < FAWAID_MIN).length;
    })(),
    scholarsShort520: scholars.filter((s) => s.bio.length < SCHOLAR_BIO_MIN).length,
    sheikhsShort320: sheikhsMod.SHEIKHS_SEED.filter((s) => (s.bio || "").length < SHEIKH_BIO_MIN).length,
    prophetsBriefShort480: prophets.filter((p) => (p.briefBio || "").length < PROPHET_BIO_MIN).length,
    prophetsLessonShort170: prophets.reduce(
...
if (verify) {
  ...
  const fail =
    ...
    (a.quizRound66ShortExpl ?? 1) > 0 ||
    (a.quizRound66 ?? 0) !== 50 ||
    (a.qaRound66 ?? 0) !== 40 ||
    (a.storiesRound66 ?? 0) !== 5 ||
    (a.pmRound66 ?? 0) !== 5 ||
    (a.fawaidRound66Block ?? 0) !== 25 ||
    (a.fawaidRound66Short145 ?? 1) > 0 ||
    (a.scholarsShort520 ?? 1) > 0 ||
    (a.sheikhsShort320 ?? 1) > 0 ||
    (a.prophetsBriefShort480 ?? 1) > 0 ||
    (a.prophetsLessonShort170 ?? 1) > 0;
