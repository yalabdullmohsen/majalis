...
const OUT = path.join(__dirname, "r74-content-data.mjs");
...
function pickQuiz(existing, need) {
  ...
  const recentQs = new Set(
    existing.filter((q) => parseInt((q.id || "").replace("demo-quiz-", ""), 10) >= 2465).map((q) => q.question),
  );
...
const qaPicked = pickQa(existingQa, 40, 1590);
const fawaidPicked = pickFawaid(curated, 25, 665);
