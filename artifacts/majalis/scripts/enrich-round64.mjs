#!/usr/bin/env node
/**
 * Round 64 — lesson bodies ≥390, pages meta ≥320 (educational), nations ≥320,
 * fiqh summary≥230 description≥260, courses≥270 occasions≥260 landmarks≥440, latin corruption scan.
 * Usage: node scripts/enrich-round64.mjs [--apply] [--verify]
 */
...
const PAGE_MIN = 320;
const NATIONS_MIN = 320;
const FIQH_SUMMARY_MIN = 230;
const FIQH_DESC_MIN = 260;
const LANDMARK_DESC_MIN = 440;
const LANDMARK_SIG_MIN = 440;
const COURSE_SUMMARY_MIN = 270;
const OCCASION_SUMMARY_MIN = 260;
...
function lessonBodyReport(verifyMode = false) {
  ...
  const out = execSync(`node scripts/enrich-r64-lesson-bodies.mjs ${flag}`.trim(), {
    cwd: ROOT,
    encoding: "utf8",
  });
...
if (apply) {
  execSync("node scripts/enrich-r64-lesson-bodies.mjs --apply", { cwd: ROOT, stdio: "inherit" });
}
