#!/usr/bin/env node
/**
 * Round 74 bulk content — quiz/QA/fawaid/stories/PM + scholars/sheikhs/prophets raises.
 * Usage: node scripts/enrich-round74-content.mjs [--apply] [--verify]
 */
import fs from "node:fs";
import path from "node:path";
import { fileURLToPath } from "node:url";
import { QUIZ_ITEMS, QA_ITEMS, FAWAID_ITEMS } from "./r74-content-data.mjs";
...
const SCHOLAR_BIO_MIN = 600;
const SHEIKH_BIO_MIN = 360;
const PROPHET_BIO_MIN = 560;
const PROPHET_LESSON_MIN = 210;
...
function raiseQuizExplanations(apply) {
  ...
  for (const q of quiz) {
    ...
    if (Number.isFinite(n) && n >= 2515 && (q.explanation || "").length < QUIZ_EXPL_MIN) {
...
function readStoriesBlock() {
  return fs.readFileSync(path.join(__dirname, "r74-original-stories.ts"), "utf8").trim();
}

function readPmBlock() {
  return fs.readFileSync(path.join(__dirname, "r74-prophetic-medicine.ts"), "utf8").trim();
}

function addSeeds(apply) {
  const stats = { quiz: 0, qa: 0, fawaid: 0, stories: 0, pm: 0 };
  const marker = "جولة ٧٤";

  const quizPath = path.join(LIB, "quiz-seed.ts");
  let quizContent = fs.readFileSync(quizPath, "utf8");
  if (!quizContent.includes(marker)) {
    const block = `  /* ───────── ${marker}: أقسام أضعف (2515-2564) ───────── */\n` +
  ...
  if (!qaContent.includes("seed-qa-1590")) {
  ...
  if (!fawaidContent.includes("إضافات جولة ٧٤")) {
    const block = `  /* ── إضافات جولة ٧٤ ── */\n` + FAWAID_ITEMS.map(renderFawaidItem).join(",\n");
    if (apply) {
  ...
  if (!storiesContent.includes("id: 240,")) {
  ...
  if (!pmContent.includes("milk-sunnah-r74")) {
...
async function verifyCounts() {
  ...
  const round74Idx = fawaidSrc.indexOf("إضافات جولة ٧٤");
  const fawaidRound74Block =
    round74Idx >= 0 ? fawaidSrc.slice(round74Idx).split("{ text:").length - 1 : 0;
  ...
  return {
    ...
    quizRound74ShortExpl: quiz.filter((q) => {
      const n = parseInt((q.id || "").replace("demo-quiz-", ""), 10);
      return n >= 2515 && (q.explanation || "").length < QUIZ_EXPL_MIN;
    }).length,
    quizRound74: quiz.filter((q) => {
      const n = parseInt((q.id || "").replace("demo-quiz-", ""), 10);
      return n >= 2515 && n <= 2564;
    }).length,
    qaRound74: qa.filter((q) => {
      const n = parseInt((q.id || "").replace("seed-qa-", ""), 10);
      return n >= 1590 && n <= 1629;
    }).length,
    storiesRound74: stories.filter((s) => s.id >= 240 && s.id <= 244).length,
    pmRound74: pm.filter((x) => PM_R74_IDS.includes(x.id)).length,
    fawaidRound74Block,
    fawaidRound74Short145: (() => {
      if (round74Idx < 0) return 999;
      const block = fawaidSrc.slice(round74Idx);
      const texts = [...block.matchAll(/text:\s*"((?:[^"\\]|\\.)*)"/g)].map((m) => m[1]);
      return texts.filter((t) => t.length < FAWAID_MIN).length;
    })(),
    scholarsShort600: scholars.filter((s) => s.bio.length < SCHOLAR_BIO_MIN).length,
    sheikhsShort360: sheikhsMod.SHEIKHS_SEED.filter((s) => (s.bio || "").length < SHEIKH_BIO_MIN).length,
    prophetsBriefShort560: prophets.filter((p) => (p.briefBio || "").length < PROPHET_BIO_MIN).length,
    prophetsLessonShort210: prophets.reduce(
...
if (verify) {
  ...
  const fail =
    ...
    (a.quizRound74ShortExpl ?? 1) > 0 ||
    (a.quizRound74 ?? 0) !== 50 ||
    (a.qaRound74 ?? 0) !== 40 ||
    (a.storiesRound74 ?? 0) !== 5 ||
    (a.pmRound74 ?? 0) !== 5 ||
    (a.fawaidRound74Block ?? 0) !== 25 ||
    (a.fawaidRound74Short145 ?? 1) > 0 ||
    (a.scholarsShort600 ?? 1) > 0 ||
    (a.sheikhsShort360 ?? 1) > 0 ||
    (a.prophetsBriefShort560 ?? 1) > 0 ||
    (a.prophetsLessonShort210 ?? 1) > 0;
