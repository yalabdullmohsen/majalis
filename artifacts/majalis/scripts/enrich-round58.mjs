#!/usr/bin/env node
/**
 * Round 58 — lesson bodies ≥330, pages meta ≥260, nations ≥260,
 * fiqh-issues summary≥200 description≥230, landmarks desc≥380 significance≥260,
 * courses summary≥240, occasions≥230, latin corruption scan.
 * Usage: node scripts/enrich-round58.mjs [--apply] [--verify]
 */
...
const PAGE_MIN = 260;
const NATIONS_MIN = 260;
const FIQH_SUMMARY_MIN = 200;
const FIQH_DESC_MIN = 230;
const LANDMARK_DESC_MIN = 380;
const LANDMARK_SIG_MIN = 260;
const COURSE_SUMMARY_MIN = 240;
const OCCASION_SUMMARY_MIN = 230;
...
function lessonBodyReport(verifyMode = false) {
  ...
  const out = execSync(`node scripts/enrich-r58-lesson-bodies.mjs ${flag}`.trim(), {
    cwd: ROOT,
    encoding: "utf8",
  });
...
if (apply) {
  execSync("node scripts/enrich-r58-lesson-bodies.mjs --apply", { cwd: ROOT, stdio: "inherit" });
}
