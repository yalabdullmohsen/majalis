#!/usr/bin/env node
/**
 * Round 83 bulk content — quiz/QA/fawaid/stories/PM.
 * Usage: node scripts/enrich-round83-content.mjs [--apply] [--verify]
 */
import fs from "node:fs";
import path from "node:path";
import { fileURLToPath } from "node:url";
import { QUIZ_ITEMS, QA_ITEMS, FAWAID_ITEMS, STORY_ITEMS, PM_ITEMS } from "./r83-content-data.mjs";
...
function addSeeds(apply) {
  ...
  if (!quizContent.includes("demo-quiz-2955")) {
    const block =
      `  /* ───────── جولة ${ROUND_AR}: محتوى أفقي (2955-3004) ───────── */\n` +
  ...
  if (!qaContent.includes("seed-qa-1940")) {
  ...
  if (!storiesContent.includes("story-r83-1")) {
    const block =
      `  /* ────────── جولة ${ROUND_AR}: قصص أصلية (284-288) ────────── */\n` +
  ...
  if (!pmContent.includes("contagion-separation-r83")) {
    const block =
...
async function verifyCounts() {
  ...
  return {
    quizRound83: quizRound.length,
    quizRound83ShortAnswers: quizRound.filter((q) => (q.answer || "").length < QUIZ_ANSWER_MIN).length,
    quizRound83ShortExpl: quizRound.filter((q) => (q.explanation || "").length < QUIZ_EXPL_MIN).length,
    qaRound83: qaRound.length,
    qaRound83ShortAnswers: qaRound.filter((q) => (q.answer || "").length < QA_ANSWER_MIN).length,
    fawaidRound83Block: fawaid.count,
    fawaidRound83Short145: fawaid.short,
    storiesRound83: storiesRound.length,
    storiesRound83ShortContent: storiesRound.filter((s) => (s.full_content || "").length < STORY_CONTENT_MIN).length,
    pmRound83: pmRound.length,
    pmRound83WeakAsFixed: pmRound.filter((x) => /ضعيف/.test(x.hadithSource || "") && !/لا يثبت|ضعف|ضعيف/.test(`${x.body || ""} ${x.benefits?.join(" ") || ""}`)).length,
    ...
    pmRound83Ids: pmRound.map((x) => x.id),
  };
}
...
if (verify) {
  ...
  const fail =
    (a.quizRound83 ?? 0) !== 50 ||
    (a.quizRound83ShortAnswers ?? 1) > 0 ||
    (a.quizRound83ShortExpl ?? 1) > 0 ||
    (a.qaRound83 ?? 0) !== 40 ||
    (a.qaRound83ShortAnswers ?? 1) > 0 ||
    (a.fawaidRound83Block ?? 0) !== 25 ||
    (a.fawaidRound83Short145 ?? 1) > 0 ||
    (a.storiesRound83 ?? 0) !== 5 ||
    (a.storiesRound83ShortContent ?? 1) > 0 ||
    (a.pmRound83 ?? 0) !== 5 ||
    (a.pmRound83WeakAsFixed ?? 1) > 0 ||
    a.roundLastQuizId !== "demo-quiz-3004" ||
    a.roundLastQaId !== "seed-qa-1979" ||
    a.roundLastStorySlug !== "story-r83-5";
