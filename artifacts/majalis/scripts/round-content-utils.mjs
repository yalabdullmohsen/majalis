...
export const __dirnameFromMeta = (metaUrl) => path.dirname(fileURLToPath(metaUrl));

export function createRoundRunner({
  metaUrl,
  round,
  roundAr,
  quizStart,
  ...
  const quizAnswerMin = 60;
  const quizExplMin = 80;
  const qaAnswerMin = 90;
  const fawaidMin = 145;
  const storyContentMin = 500;
  ...
  function assertSourceData() {
    ...
    for (const q of quizItems) {
      if ((q.answer || "").length < quizAnswerMin) problems.push(`short quiz answer ${q.id}`);
      if ((q.explanation || "").length < quizExplMin) problems.push(`short quiz explanation ${q.id}`);
    }
    for (const q of qaItems) {
      if ((q.answer || "").length < qaAnswerMin) problems.push(`short QA answer ${q.id}`);
    }
    for (const f of fawaidItems) {
      if ((f.text || "").length < fawaidMin) problems.push(`short fawaid text ${f.category}`);
    }
    for (const s of storyItems) {
      if ((s.full_content || "").length < storyContentMin) problems.push(`short story ${s.slug}`);
    }
    const contentText = [
    ...
    const latinLeaks = contentText.filter((text) => /[A-Za-z]{3,}/.test(text));
    if (latinLeaks.length) problems.push("latin text leak in Arabic content");
  ...
  function addSeeds(apply) {
    ...
    if (!quizContent.includes(`demo-quiz-${quizStart}`)) {
      const block =
        `  /* ───────── جولة ${roundAr}: محتوى أفقي (${quizStart}-${quizEnd}) ───────── */\n` +
    ...
    if (!storiesContent.includes(`story-r${round}-1`)) {
      const block =
        `  /* ────────── جولة ${roundAr}: قصص أصلية (${storyStart}-${storyEnd}) ────────── */\n` +
    ...
    if (!pmContent.includes(pmIds[0])) {
      const block =
        `/* ── إضافات جولة ${roundAr}: طب نبوي بمنهجية التثبت ── */\n` +
  ...
  function countRoundFawaid() {
    ...
    return { count: texts.length, short: texts.filter((t) => t.length < fawaidMin).length };
  }
  ...
  async function verifyCounts() {
    ...
    return {
      ...
      [`quizRound${round}ShortAnswers`]: quizRound.filter((q) => (q.answer || "").length < quizAnswerMin).length,
      [`quizRound${round}ShortExpl`]: quizRound.filter((q) => (q.explanation || "").length < quizExplMin).length,
      [`qaRound${round}`]: qaRound.length,
      [`qaRound${round}ShortAnswers`]: qaRound.filter((q) => (q.answer || "").length < qaAnswerMin).length,
      [`fawaidRound${round}Block`]: fawaid.count,
      [`fawaidRound${round}Short145`]: fawaid.short,
      [`storiesRound${round}`]: storiesRound.length,
      [`storiesRound${round}ShortContent`]: storiesRound.filter((s) => (s.full_content || "").length < storyContentMin).length,
      [`pmRound${round}`]: pmRound.length,
      quizTotal: quiz.length,
      qaTotal: qa.length,
      fawaidCuratedTotal: countFawaidCurated(),
  ...
  async function run() {
    ...
    if (verify) {
      ...
      const fail =
        (a[`quizRound${round}`] ?? 0) !== 50 ||
        (a[`quizRound${round}ShortAnswers`] ?? 1) > 0 ||
        (a[`quizRound${round}ShortExpl`] ?? 1) > 0 ||
        (a[`qaRound${round}`] ?? 0) !== 40 ||
        (a[`qaRound${round}ShortAnswers`] ?? 1) > 0 ||
        (a[`fawaidRound${round}Block`] ?? 0) !== 25 ||
        (a[`fawaidRound${round}Short145`] ?? 1) > 0 ||
        (a[`storiesRound${round}`] ?? 0) !== 5 ||
        (a[`storiesRound${round}ShortContent`] ?? 1) > 0 ||
        (a[`pmRound${round}`] ?? 0) !== 5 ||
