#!/usr/bin/env node
/**
 * Round 78 bulk content — quiz/QA/fawaid/stories/PM.
 * Usage: node scripts/enrich-round78-content.mjs [--apply] [--verify]
 */
import fs from "node:fs";
import path from "node:path";
import { fileURLToPath } from "node:url";
import { QUIZ_ITEMS, QA_ITEMS, FAWAID_ITEMS } from "./r78-content-data.mjs";
...
const PM_R78_IDS = ["aloe-r78", "watermelon-r78", "cinnamon-r78", "colocynth-r78"];
...
function raiseQuizExplanations(apply) {
  ...
  for (const q of quiz) {
    ...
    if (Number.isFinite(n) && n >= 2715 && (q.explanation || "").length < QUIZ_EXPL_MIN) {
...
function addSeeds(apply) {
  ...
  if (!qaContent.includes("seed-qa-1750")) {
  ...
  if (!fawaidContent.includes("إضافات جولة ٧٨")) {
    const block = `  /* ── إضافات جولة ٧٨ ── */\n` + FAWAID_ITEMS.map(renderFawaidItem).join(",\n");
    if (apply) {
  ...
  if (!storiesContent.includes("id: 260,")) {
  ...
  if (!pmContent.includes("aloe-r78")) {
...
async function verifyCounts() {
  ...
  const round78Idx = fawaidSrc.indexOf("إضافات جولة ٧٨");
  const fawaidRound78Block =
    round78Idx >= 0 ? fawaidSrc.slice(round78Idx).split("{ text:").length - 1 : 0;
  ...
  return {
    ...
    quizRound78ShortExpl: quiz.filter((q) => {
      const n = parseInt((q.id || "").replace("demo-quiz-", ""), 10);
      return n >= 2715 && (q.explanation || "").length < QUIZ_EXPL_MIN;
    }).length,
    quizRound78: quiz.filter((q) => {
      const n = parseInt((q.id || "").replace("demo-quiz-", ""), 10);
      return n >= 2715 && n <= 2754;
    }).length,
    qaRound78: qa.filter((q) => {
      const n = parseInt((q.id || "").replace("seed-qa-", ""), 10);
      return n >= 1750 && n <= 1779;
    }).length,
    storiesRound78: stories.filter((s) => s.id >= 260 && s.id <= 263).length,
    pmRound78: pm.filter((x) => PM_R78_IDS.includes(x.id)).length,
    fawaidRound78Block,
    fawaidRound78Short145: (() => {
      if (round78Idx < 0) return 999;
      const block = fawaidSrc.slice(round78Idx);
...
if (verify) {
  ...
  const fail =
    ...
    (a.quizRound78ShortExpl ?? 1) > 0 ||
    (a.quizRound78 ?? 0) !== 40 ||
    (a.qaRound78 ?? 0) !== 30 ||
    (a.storiesRound78 ?? 0) !== 4 ||
    (a.pmRound78 ?? 0) !== 4 ||
    (a.fawaidRound78Block ?? 0) !== 20 ||
    (a.fawaidRound78Short145 ?? 1) > 0;
