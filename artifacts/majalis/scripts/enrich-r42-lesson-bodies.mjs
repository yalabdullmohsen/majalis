#!/usr/bin/env node
/**
 * Round 42 — raise live lesson bodies to ≥200 chars; diversify bridges >40.
 * Usage: node scripts/enrich-r42-lesson-bodies.mjs [--apply] [--verify]
 */
...
const TATBIQ_RE =
  /(?:^|[\.،]\s*)(?:تطبيق:|عمليًا:|خطوة اليوم:|من التطبيق:|في الميدان:|ما يُستحسن:|مناسب لك:|ابدأ بـ:|جرّب:|التزم:|نصيحة عملية:|من أثر المعنى:|من الواجب:|للتطبيق:|خطوة عملية:|في الميدان:|مناسب:|واجب:|خطوة:|ابدأ:|جرّب:|التزم:)/;
...
/** Fresh scholarly bridges for round 42 (not in r40/r41 dominant pools). */
const R42_BRIDGE_POOL = [
  "ويتأكد أن الورع في السر أثبت من حسن الظاهر",
  "فلا يُستبدل التزام الحق بزخرفة العبارة",
  "ويُستحضر أن كل معنى يُسأل عنه يوم القيامة",
...
const TRACKED_PHRASES = [...new Set(R42_BRIDGE_POOL)];
...
function pickBridge(body, counts, idx) {
  const candidates = R42_BRIDGE_POOL.filter((p) => !body.includes(p)).sort(
    (a, b) => (counts[a] || 0) - (counts[b] || 0)
  );
  const underCap = candidates.filter((p) => (counts[p] || 0) < MAX_BRIDGE_FREQ);
  const pool = underCap.length ? underCap : candidates.length ? candidates : R42_BRIDGE_POOL;
...
function expandBody(body, counts, state) {
  ...
  let guard = 0;
  while (out.length < MIN_LEN && guard < 20) {
    guard++;
...
function redistributeBridges(body, counts) {
  ...
  for (const phrase of over) {
    while ((counts[phrase] || 0) > MAX_BRIDGE_FREQ && out.includes(phrase)) {
      const alt = pickReplacement(out, R42_BRIDGE_POOL, counts);
      counts[phrase]--;
...
function processFile(filePath, counts, apply) {
  ...
  const state = {
    bridgeIdx: Math.abs(hashStr(filePath)) % R42_BRIDGE_POOL.length,
