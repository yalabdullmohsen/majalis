#!/usr/bin/env node
/**
 * Round 61 bulk content — quiz/QA/fawaid/stories — quiz/QA/fawaid/stories.
 * Usage: node scripts/enrich-round61-content.mjs [--apply] [--verify]
 */
import fs from "node:fs";
import path from "node:path";
import { fileURLToPath } from "node:url";
import { QUIZ_ITEMS, QA_ITEMS, FAWAID_ITEMS } from "./r61-content-data.mjs";
...
function raiseQuizExplanations(apply) {
  ...
  for (const q of quiz) {
    ...
    if (Number.isFinite(n) && n >= 1865 && (q.explanation || "").length < QUIZ_EXPL_MIN) {
...
function addSeeds(apply) {
  ...
  if (!qaContent.includes("seed-qa-1070")) {
  ...
  if (!fawaidContent.includes("إضافات جولة ٦١")) {
    const block = `  /* ── إضافات جولة ٦١ ── */\n` + FAWAID_ITEMS.map(renderFawaidItem).join(",\n");
    if (apply) {
  ...
  if (!storiesContent.includes("id: 175,")) {
...
async function verifyCounts() {
  ...
  const round61Idx = fawaidSrc.indexOf("إضافات جولة ٦١");
  const fawaidRound61Block =
    round61Idx >= 0 ? fawaidSrc.slice(round61Idx).split("{ text:").length - 1 : 0;
  ...
  return {
    ...
    quizRound61ShortExpl: quiz.filter((q) => {
      const n = parseInt((q.id || "").replace("demo-quiz-", ""), 10);
      return n >= 1865 && (q.explanation || "").length < QUIZ_EXPL_MIN;
    }).length,
    quizRound61: quiz.filter((q) => {
      const n = parseInt((q.id || "").replace("demo-quiz-", ""), 10);
      return n >= 1865 && n <= 1914;
    }).length,
    qaRound61: qa.filter((q) => {
      const n = parseInt((q.id || "").replace("seed-qa-", ""), 10);
      return n >= 1070 && n <= 1109;
    }).length,
    storiesRound61: stories.filter((s) => s.id >= 175 && s.id <= 179).length,
    fawaidRound61Block,
    fawaidRound61Short145: (() => {
      if (round61Idx < 0) return 999;
      const block = fawaidSrc.slice(round61Idx);
...
if (verify) {
  ...
  const fail =
    ...
    (a.quizRound61ShortExpl ?? 1) > 0 ||
    (a.quizRound61 ?? 0) !== 50 ||
    (a.qaRound61 ?? 0) !== 40 ||
    (a.storiesRound61 ?? 0) !== 5 ||
    (a.fawaidRound61Block ?? 0) !== 25 ||
    (a.fawaidRound61Short145 ?? 1) > 0;
