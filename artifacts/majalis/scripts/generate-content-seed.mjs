...
const QUIZ_TO_QA_CAT = {
  ...
  "السيرة|المساجد": "السيرة",
  "السيرة|الغزوات": "السيرة",
  "السيرة|الصلح": "السيرة",
  "السيرة|الفتح": "السيرة",
  ...
  "الصالحون|الفقه": "الحديث",
  "الصالحون|التفسير": "القرآن",
  "الصالحون|الزهد": "الآداب",
  "الصحابة|الحديث": "الصحابة",
  "الألغاز الشرعية|عام": "الآداب",
};
...
// صمام أمان: ملفا البذور أصبحا مصدرًا مُدقَّقًا يدويًا (تصحيحات تخريج وآيات
// وأحكام مذهبية دقيقة أُضيفت عبر دورات تدقيق متعددة) ولم يعودا نسخة طبق الأصل
// من مخرجات هذا المولّد. الكتابة فوقهما بلا انتباه تمحو ذلك التدقيق نهائيًا،
// فيرفض المولّد الكتابة عند اكتشاف أي اختلاف ما لم يُمرَّر --force صراحةً.
const force = process.argv.includes("--force");

function writeSeed(label, filePath, nextContent) {
  const exists = fs.existsSync(filePath);
  const current = exists ? fs.readFileSync(filePath, "utf8") : null;
  if (exists && current !== nextContent && !force) {
    const currentCount = (current.match(/"id": "seed-/g) ?? []).length;
    const nextCount = (nextContent.match(/"id": "seed-/g) ?? []).length;
    console.error(
      "⛔ رُفضت الكتابة على " + label + " (" + filePath + ")\n" +
        "   الملف الحالي يحوي " + currentCount + " عنصرًا، ومخرجات المولّد " + nextCount + " عنصرًا.\n" +
        "   هذا الملف مُدقَّق يدويًا (تخريج أحاديث وآيات محقَّقة) ولم يعد مطابقًا لمخرجات المولّد.\n" +
        "   الكتابة فوقه تمحو كل التدقيق. إن كنت متأكدًا فعلاً فمرّر --force."
    );
    return false;
  }
...
const wroteQa = writeSeed("QA seed", qaSeedPath, generateQaSeedTs(seedQa));
const wroteFawaid = writeSeed("Fawaid seed", fawaidSeedPath, generateFawaidSeedTs(seedFawaid));

console.log("عدد أسئلة QA المولَّدة: " + seedQa.length);
console.log("عدد الفوائد المولَّدة: " + seedFawaid.length);

if (!wroteQa || !wroteFawaid) {
  process.exitCode = 1;
}
for (const cat of QA_CATEGORIES) {
