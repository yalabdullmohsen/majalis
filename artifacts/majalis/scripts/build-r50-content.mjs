...
const OUT = path.join(__dirname, "r50-content-data.mjs");
...
function padAnswer(text, min) {
  ...
  const suffix = " — يُراجع في كتب الفقه المعتمدة عند أهل العلم.";
...
function pickQuiz(existing, need) {
  ...
  const recentQs = new Set(
    existing.filter((q) => parseInt((q.id || "").replace("demo-quiz-", ""), 10) >= 1200).map((q) => q.question),
  );
...
function pickFawaid(curated, need) {
  const pool = curated.filter((x) => x.text && x.text.length >= 100);
  return pool.slice(65, 65 + need).map((f) => ({ ...f, text: padFawaid(f.text, 145) }));
}
...
const qaPicked = pickQa(existingQa, 40, 630);
const fawaidPicked = pickFawaid(curated, 25);
...
const qaLines = qaPicked.map((q) => {
  const answer = padAnswer(q.answer, 90);
...
fs.writeFileSync(OUT, `/** Auto-generated for round 50 */
...
console.log(JSON.stringify({ quiz: quizLines.length, qa: qaLines.length, fawaid: fawaidLines.length }, null, 2));
