#!/usr/bin/env node
/**
 * Round 62 — lesson bodies ≥370, pages meta ≥300, nations ≥300,
 * fiqh summary≥220 description≥250, courses≥260 occasions≥250 landmarks≥420, latin corruption scan.
 * Usage: node scripts/enrich-round62.mjs [--apply] [--verify]
 */
...
const PAGE_MIN = 300;
const NATIONS_MIN = 300;
const UPDATE_SUMMARY_MIN = 190;
const QURAN_CIRCLE_MIN = 220;
const MUTA_MIN = 220;
const MIND_MAP_MIN = 230;
const AMR_EXPL_MIN = 230;
const FIQH_SUMMARY_MIN = 220;
const FIQH_DESC_MIN = 250;
const LANDMARK_DESC_MIN = 420;
const LANDMARK_SIG_MIN = 260;
const COURSE_SUMMARY_MIN = 260;
const OCCASION_SUMMARY_MIN = 250;
...
function lessonBodyReport(verifyMode = false) {
  ...
  const out = execSync(`node scripts/enrich-r62-lesson-bodies.mjs ${flag}`.trim(), {
    cwd: ROOT,
    encoding: "utf8",
  });
...
if (apply) {
  execSync("node scripts/enrich-r62-lesson-bodies.mjs --apply", { cwd: ROOT, stdio: "inherit" });
}
