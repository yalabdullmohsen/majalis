...
const OUT = path.join(__dirname, "r72-content-data.mjs");
...
function pickQuiz(existing, need) {
  ...
  const recentQs = new Set(
    existing.filter((q) => parseInt((q.id || "").replace("demo-quiz-", ""), 10) >= 2365).map((q) => q.question),
  );
...
const qaPicked = pickQa(existingQa, 40, 1510);
const fawaidPicked = pickFawaid(curated, 25, 615);
