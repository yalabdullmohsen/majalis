#!/usr/bin/env node
/**
 * Round 70 — lesson bodies ≥450, pages meta ≥380 (educational), nations ≥380,
 * fiqh summary≥260 description≥290, courses≥300 occasions≥290 landmarks≥500, latin corruption scan.
 * Usage: node scripts/enrich-round70.mjs [--apply] [--verify]
 */
...
const PAGE_MIN = 380;
const NATIONS_MIN = 380;
const FIQH_SUMMARY_MIN = 260;
const FIQH_DESC_MIN = 290;
const LANDMARK_DESC_MIN = 500;
const LANDMARK_SIG_MIN = 500;
const COURSE_SUMMARY_MIN = 300;
const OCCASION_SUMMARY_MIN = 290;
...
function lessonBodyReport(verifyMode = false) {
  ...
  const out = execSync(`node scripts/enrich-r70-lesson-bodies.mjs ${flag}`.trim(), {
    cwd: ROOT,
    encoding: "utf8",
  });
...
if (apply) {
  execSync("node scripts/enrich-r70-lesson-bodies.mjs --apply", { cwd: ROOT, stdio: "inherit" });
}
