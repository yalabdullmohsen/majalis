...
// أسماء الأنبياء القصيرة (صالح، هود، لوط، نوح) تحتاج حدود كلمة عربية صريحة
// (\b في JS لا يتعرّف على الأحرف العربية) وإلا تتصادم مع كلمات تحتويها فعليًا
// كـ"الصالحين" (تحتوي "صالح") أو "شهود"/"معهود" (تحتوي "هود") — اكتُشف عبر
// false positive حقيقي: "من مؤلف كتاب رياض الصالحين؟" صُنِّف خطأً كسؤال أنبياء.
const AR = "\\u0621-\\u064A";
const nameBoundary = (name) => `(?<![${AR}])${name}(?![${AR}])`;
const ANBIYA_NAMES = ["نوح", "إبراهيم", "موسى", "عيسى", "يونس", "هود", "صالح", "لوط", "شعيب", "ذو الكفل"]
  .map(nameBoundary)
  .join("|");
const ANBIYA_RE = new RegExp(
  `(?:من (?:أول|ال)?(?:رسل|نبي|أنبياء)|النبي الذي|إلى قوم (?:عاد|ثمود|لوط)|ابتلعه الحوت|كلمه الله|${ANBIYA_NAMES}|قصص? (?:الأنبياء|المرسلين)|عليه(?:ه)?\\s*السلام)`,
  "i"
);
