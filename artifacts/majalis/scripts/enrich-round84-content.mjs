#!/usr/bin/env node
/**
 * Round 84 bulk content — quiz/QA/fawaid/stories/PM.
 * Usage: node scripts/enrich-round84-content.mjs [--apply] [--verify]
 */
import fs from "node:fs";
import path from "node:path";
import { fileURLToPath } from "node:url";
import { QUIZ_ITEMS, QA_ITEMS, FAWAID_ITEMS, STORY_ITEMS, PM_ITEMS } from "./r84-content-data.mjs";
...
function addSeeds(apply) {
  ...
  if (!quizContent.includes("demo-quiz-3005")) {
    const block =
      `  /* ───────── جولة ${ROUND_AR}: محتوى أفقي (3005-3054) ───────── */\n` +
  ...
  if (!qaContent.includes("seed-qa-1980")) {
  ...
  if (!fawaidContent.includes(`إضافات جولة ${ROUND_AR}`)) {
    const block = `  /* ── إضافات جولة ${ROUND_AR} ── */\n` + FAWAID_ITEMS.map(renderFawaidItem).join(",\n");
    if (apply) {
  ...
  if (!storiesContent.includes("story-r84-1")) {
    const block =
      `  /* ────────── جولة ${ROUND_AR}: قصص أصلية (289-293) ────────── */\n` +
  ...
  if (!pmContent.includes("stagnant-water-cleanliness-r84")) {
    const block =
      `/* ── إضافات جولة ${ROUND_AR}: طب نبوي بمنهجية التثبت ── */\n` +
...
function countRoundFawaid(roundAr) {
  const src = fs.readFileSync(path.join(LIB, "fawaid-curated-seed.ts"), "utf8");
...
async function verifyCounts() {
  ...
  const fawaid = countRoundFawaid(ROUND_AR);
  ...
  return {
    quizRound84: quizRound.length,
    quizRound84ShortAnswers: quizRound.filter((q) => (q.answer || "").length < QUIZ_ANSWER_MIN).length,
    quizRound84ShortExpl: quizRound.filter((q) => (q.explanation || "").length < QUIZ_EXPL_MIN).length,
    qaRound84: qaRound.length,
    qaRound84ShortAnswers: qaRound.filter((q) => (q.answer || "").length < QA_ANSWER_MIN).length,
    fawaidRound84Block: fawaid.count,
    fawaidRound84Short145: fawaid.short,
    storiesRound84: storiesRound.length,
    storiesRound84ShortContent: storiesRound.filter((s) => (s.full_content || "").length < STORY_CONTENT_MIN).length,
    pmRound84: pmRound.length,
    pmRound84WeakAsFixed: pmRound.filter((x) => /ضعيف/.test(x.hadithSource || "") && !/لا يثبت|ضعف|ضعيف/.test(`${x.body || ""} ${x.benefits?.join(" ") || ""}`)).length,
    ...
    pmRound84Ids: pmRound.map((x) => x.id),
  };
}
...
if (verify) {
  ...
  const fail =
    (a.quizRound84 ?? 0) !== 50 ||
    (a.quizRound84ShortAnswers ?? 1) > 0 ||
    (a.quizRound84ShortExpl ?? 1) > 0 ||
    (a.qaRound84 ?? 0) !== 40 ||
    (a.qaRound84ShortAnswers ?? 1) > 0 ||
    (a.fawaidRound84Block ?? 0) !== 25 ||
    (a.fawaidRound84Short145 ?? 1) > 0 ||
    (a.storiesRound84 ?? 0) !== 5 ||
    (a.storiesRound84ShortContent ?? 1) > 0 ||
    (a.pmRound84 ?? 0) !== 5 ||
    (a.pmRound84WeakAsFixed ?? 1) > 0 ||
    a.roundLastQuizId !== "demo-quiz-3054" ||
    a.roundLastQaId !== "seed-qa-2019" ||
    a.roundLastStorySlug !== "story-r84-5";
