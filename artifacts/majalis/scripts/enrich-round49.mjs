...
function pageSuffix(text, field, fileName) {
  ...
  if (text.startsWith("«") || text.includes("»") || text.includes("قال ﷺ")) {
    if (field === "text") {
      return ["ويُستفاد منه في تزكية القلب واستحضار الآخرة", "من مواعظ الرقائق المعتمدة في منهج سُنّة"];
    }
    return ["حديثٌ أو أثرٌ يُعرض بلفظه دون تحريف", "يُراعى ثبوته قبل الاستدلال — من مراجع سُنّة"];
  }
  const map = {
    RaqaiqPage: ["من مواعظ الرقائق والزهد المعتمدة", "يُستحضر في تزكية القلب — مرجع سُنّة"],
    AkhlaqPage: ["من مكارم الأخلاق في الشرع", "يُستفاد في التربية والسلوك — مرجع سُنّة"],
    AmradQalbiyyaPage: ["من أمراض القلوب وعلاجها", "يُستحضر في محاسبة النفس — مرجع سُنّة"],
    PrayerRanksPage: ["من فضائل الصلاة ومراتبها", "يُستحضر في تحسين العبادة — مرجع سُنّة"],
  };
  return map[base] || ["محتوى تعليمي معتمد في منهج سُنّة", "يُستفاد في التعلم والتطبيق — مرجع تربوي شرعي"];
}
