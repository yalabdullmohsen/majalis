...
const OUT = path.join(__dirname, "r75-content-data.mjs");
...
function pickQuiz(existing, need) {
  ...
  const recentQs = new Set(
    existing.filter((q) => parseInt((q.id || "").replace("demo-quiz-", ""), 10) >= 2515).map((q) => q.question),
  );
...
const qaPicked = pickQa(existingQa, 40, 1630);
const fawaidPicked = pickFawaid(curated, 25, 690);
