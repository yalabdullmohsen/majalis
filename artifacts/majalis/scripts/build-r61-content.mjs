...
const OUT = path.join(__dirname, "r61-content-data.mjs");
...
function pickQuiz(existing, need) {
  ...
  const recentQs = new Set(
    existing.filter((q) => parseInt((q.id || "").replace("demo-quiz-", ""), 10) >= 1815).map((q) => q.question),
  );
...
const qaPicked = pickQa(existingQa, 40, 1070);
const fawaidPicked = pickFawaid(curated, 25, 340);
