#!/usr/bin/env node
/**
 * Round 65 — lesson bodies ≥400, pages meta ≥330 (educational), nations ≥330,
 * updates≥210, circles/mutashabihat≥240, mind-maps/amr≥250, latin corruption scan.
 * Usage: node scripts/enrich-round65.mjs [--apply] [--verify]
 */
...
const PAGE_MIN = 330;
const NATIONS_MIN = 330;
const UPDATE_SUMMARY_MIN = 210;
const QURAN_CIRCLE_MIN = 240;
const MUTA_MIN = 240;
const MIND_MAP_MIN = 250;
const AMR_EXPL_MIN = 250;
...
function lessonBodyReport(verifyMode = false) {
  ...
  const out = execSync(`node scripts/enrich-r65-lesson-bodies.mjs ${flag}`.trim(), {
    cwd: ROOT,
    encoding: "utf8",
  });
...
if (apply) {
  execSync("node scripts/enrich-r65-lesson-bodies.mjs --apply", { cwd: ROOT, stdio: "inherit" });
}
