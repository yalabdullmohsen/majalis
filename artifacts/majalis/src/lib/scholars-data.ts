export type Scholar = {
  id: string;
  name: string;
  fullName: string;
  era: string;
  specialty: string[];
  bio: string;
  key_works: string[];
  died: string;
  region: string;
  madhhab?: string;
  quote?: string;
};

export const SCHOLARS: Scholar[] = [
  // الأئمة الأربعة
  {
    id: "abu-hanifa",
    name: "أبو حنيفة النعمان",
    fullName: "النعمان بن ثابت الكوفي",
    era: "الأئمة الأربعة",
    specialty: ["فقه", "أصول"],
    bio: "إمام المذهب الحنفي، أول من دوّن الفقه وصنّفه، عُرف بعمق النظر واستقراء مسائل النوازل",
    key_works: ["المسند", "الفقه الأكبر", "العالم والمتعلم"],
    died: "١٥٠ هـ",
    region: "الكوفة / بغداد",
    madhhab: "حنفي",
    quote: "العلم لا يُعطيك بعضه حتى تُعطيه كلّك",
  },
  {
    id: "malik",
    name: "الإمام مالك",
    fullName: "مالك بن أنس الأصبحي",
    era: "الأئمة الأربعة",
    specialty: ["حديث", "فقه"],
    bio: "إمام دار الهجرة، صاحب الموطأ، أقدم كتب الحديث الموثقة، اشتُهر بعلمه بعمل أهل المدينة",
    key_works: ["الموطأ", "المدونة الكبرى"],
    died: "١٧٩ هـ",
    region: "المدينة المنورة",
    madhhab: "مالكي",
    quote: "لا يُفتي بهذا الأمر حتى تأتيه الآثار من ثلاثة وجوه",
  },
  {
    id: "shafi",
    name: "الإمام الشافعي",
    fullName: "محمد بن إدريس الشافعي",
    era: "الأئمة الأربعة",
    specialty: ["فقه", "أصول", "شعر"],
    bio: "مؤسس علم أصول الفقه، جمع بين المنهج المدني والعراقي، صاحب الرسالة، أول كتاب في الأصول",
    key_works: ["الرسالة", "الأم", "ديوان الشافعي"],
    died: "٢٠٤ هـ",
    region: "فلسطين / مصر",
    madhhab: "شافعي",
    quote: "كلما ازددتُ علماً ازددتُ علماً بجهلي",
  },
  {
    id: "ahmad",
    name: "الإمام أحمد",
    fullName: "أحمد بن محمد بن حنبل الشيباني",
    era: "الأئمة الأربعة",
    specialty: ["حديث", "فقه", "عقيدة"],
    bio: "إمام أهل السنة، ثبت على المحنة وأبى القول بخلق القرآن، جمع المسند الكبير",
    key_works: ["المسند", "الزهد", "العلل"],
    died: "٢٤١ هـ",
    region: "بغداد",
    madhhab: "حنبلي",
    quote: "المحنة ميزان بين المؤمن وأهل النفاق",
  },
  // علماء الحديث
  {
    id: "bukhari",
    name: "الإمام البخاري",
    fullName: "محمد بن إسماعيل البخاري",
    era: "المحدثون",
    specialty: ["حديث"],
    bio: "صاحب أصح كتاب بعد كتاب الله، الجامع الصحيح، حفظ ستمئة ألف حديث",
    key_works: ["صحيح البخاري", "الأدب المفرد", "التاريخ الكبير"],
    died: "٢٥٦ هـ",
    region: "بخارى (أوزبكستان)",
    quote: "ما وضعتُ في الصحيح حديثاً إلا اغتسلتُ وصليتُ ركعتين",
  },
  {
    id: "muslim",
    name: "الإمام مسلم",
    fullName: "مسلم بن الحجاج القشيري النيسابوري",
    era: "المحدثون",
    specialty: ["حديث", "رجال", "علل"],
    bio: "صاحب الصحيح المسمى بالمسند الصحيح المختصر، ثاني الصحيحين، استغرق في تأليفه خمسة عشر عاماً",
    key_works: ["صحيح مسلم (المسند الصحيح)", "الكنى والأسماء", "التمييز"],
    died: "٢٦١ هـ",
    region: "نيسابور / خراسان",
    madhhab: "شافعي",
    quote: "لا أعلم أحداً وصف الحديث في الصحة كما وصفه الإمام مسلم",
  },
  // علماء العقيدة والتفسير
  {
    id: "ibn-taymiyya",
    name: "ابن تيمية",
    fullName: "أحمد بن عبد الحليم بن تيمية الحراني",
    era: "العلماء الكبار",
    specialty: ["عقيدة", "فقه", "تفسير"],
    bio: "شيخ الإسلام، مجدد القرن السابع، صاحب الدعوة إلى السنة ومحاربة البدعة",
    key_works: ["مجموع الفتاوى", "درء تعارض العقل والنقل", "منهاج السنة", "الفتاوى الكبرى"],
    died: "٧٢٨ هـ",
    region: "الشام (سوريا)",
    quote: "ما يصنع أعدائي بي؟ سجني خلوة، ونفيي سياحة، وقتلي شهادة",
  },
  {
    id: "ibn-qayyim",
    name: "ابن القيم",
    fullName: "محمد بن أبي بكر بن قيم الجوزية",
    era: "العلماء الكبار",
    specialty: ["عقيدة", "فقه", "أخلاق", "تربية"],
    bio: "تلميذ ابن تيمية الأبرز، عُرف بعمق التحليل ودقة الاستنباط وجمال الأسلوب",
    key_works: ["زاد المعاد", "إغاثة اللهفان", "مدارج السالكين", "الروح"],
    died: "٧٥١ هـ",
    region: "دمشق",
    quote: "القلوب الحية تُحيي بذكر الله كما تُحيي الأرض الميتة بالمطر",
  },
  {
    id: "nawawi",
    name: "الإمام النووي",
    fullName: "يحيى بن شرف النووي",
    era: "العلماء الكبار",
    specialty: ["حديث", "فقه", "لغة"],
    bio: "إمام الشافعية في عصره، ألّف في الحديث والفقه واللغة، عُرف بالزهد والتقوى",
    key_works: ["الأذكار", "رياض الصالحين", "شرح صحيح مسلم", "الأربعون النووية"],
    died: "٦٧٦ هـ",
    region: "نوى (سوريا)",
  },
  {
    id: "ibn-hajar",
    name: "ابن حجر العسقلاني",
    fullName: "أحمد بن علي بن حجر العسقلاني",
    era: "العلماء الكبار",
    specialty: ["حديث", "تاريخ", "رجال"],
    bio: "حافظ العصر، شيخ الإسلام في الحديث، أعظم شراح صحيح البخاري",
    key_works: ["فتح الباري", "الإصابة في تمييز الصحابة", "تهذيب التهذيب"],
    died: "٨٥٢ هـ",
    region: "القاهرة",
  },
  {
    id: "shawkani",
    name: "الشوكاني",
    fullName: "محمد بن علي بن محمد الشوكاني",
    era: "المجددون",
    specialty: ["حديث", "فقه", "أصول"],
    bio: "إمام اليمن في عصره، صاحب نيل الأوطار، جمع بين الحديث والفقه الاستدلالي",
    key_works: ["نيل الأوطار", "إرشاد الفحول", "فتح القدير"],
    died: "١٢٥٠ هـ",
    region: "اليمن",
    quote: "الدليل لا يُعارَض بكثرة القائلين ولا بقِدَم المذاهب",
  },
  // العلماء المعاصرون
  {
    id: "ibn-baz",
    name: "الشيخ ابن باز",
    fullName: "عبد العزيز بن عبد الله بن باز",
    era: "المعاصرون",
    specialty: ["فقه", "عقيدة", "حديث"],
    bio: "مفتي المملكة العربية السعودية السابق، رئيس هيئة كبار العلماء، إمام وقته في الفتوى",
    key_works: ["مجموع الفتاوى", "التحذير من البدع", "نقد القومية العربية"],
    died: "١٤٢٠ هـ",
    region: "السعودية",
    quote: "العلم النافع يورث الخشية والإنابة والتواضع",
  },
  {
    id: "ibn-uthaymeen",
    name: "الشيخ ابن عثيمين",
    fullName: "محمد بن صالح بن محمد العثيمين",
    era: "المعاصرون",
    specialty: ["فقه", "عقيدة", "تفسير"],
    bio: "من أبرز علماء المملكة، عُرف بوضوح الشرح وسعة الاطلاع وربط الفقه بالواقع",
    key_works: ["الشرح الممتع", "القول المفيد", "مجموع الفتاوى"],
    died: "١٤٢١ هـ",
    region: "عنيزة، السعودية",
    quote: "أنعم الله علينا بنعمة الإسلام فاشكروا الله عليها",
  },
  {
    id: "albani",
    name: "الشيخ الألباني",
    fullName: "محمد ناصر الدين الألباني",
    era: "المعاصرون",
    specialty: ["حديث", "فقه"],
    bio: "محدّث العصر، عمل طول حياته على تمييز الصحيح من الضعيف في السنة النبوية",
    key_works: ["سلسلة الأحاديث الصحيحة", "سلسلة الأحاديث الضعيفة", "إرواء الغليل"],
    died: "١٤٢٠ هـ",
    region: "ألبانيا / الأردن",
    quote: "التمسك بالسنة هو الطريق الصحيح المأمون",
  },
  {
    id: "qaradawi",
    name: "الشيخ القرضاوي",
    fullName: "يوسف بن عبد الله القرضاوي",
    era: "المعاصرون",
    specialty: ["فقه", "دعوة", "أصول"],
    bio: "من أبرز العلماء في الفقه الإسلامي المعاصر، اشتُهر بالمنهج الوسطي وفقه الأولويات",
    key_works: ["الحلال والحرام", "فقه الزكاة", "فقه الأولويات", "فتاوى معاصرة"],
    died: "١٤٤٣ هـ",
    region: "مصر / قطر",
    quote: "الإسلام هو الحل، لأنه دين ودنيا وعبادة وسياسة",
  },
  // محدثون بارزون
  {
    id: "tirmidhi",
    name: "الإمام الترمذي",
    fullName: "محمد بن عيسى بن سَوْرة الترمذي",
    era: "المحدثون",
    specialty: ["حديث", "علل", "رجال"],
    bio: "صاحب الجامع الصحيح (سنن الترمذي)، أحد الكتب الستة، اشتُهر بذكر علل الأحاديث وبيان درجاتها",
    key_works: ["الجامع الصحيح", "الشمائل المحمدية", "العلل الكبير"],
    died: "٢٧٩ هـ",
    region: "ترمذ / خراسان",
    madhhab: "شافعي",
    quote: "ما وضعتُ في كتابي هذا حديثاً إلا وقد أخرجتُه إلى بغداد",
  },
  {
    id: "ibn-majah",
    name: "ابن ماجه",
    fullName: "محمد بن يزيد القزويني",
    era: "المحدثون",
    specialty: ["حديث", "تفسير", "تاريخ"],
    bio: "صاحب سنن ابن ماجه، السادس من الكتب الستة، رحل في طلب الحديث إلى الحجاز والعراق ومصر والشام",
    key_works: ["سنن ابن ماجه", "التاريخ", "تفسير القرآن"],
    died: "٢٧٣ هـ",
    region: "قزوين / إيران",
    quote: "كتبتُ عن رسول الله ﷺ ما لم يكتبه أحد قبلي",
  },
  {
    id: "ghazali",
    name: "الإمام الغزالي",
    fullName: "محمد بن محمد الغزالي الطوسي",
    era: "العلماء الكبار",
    specialty: ["فقه", "أصول", "تصوف", "عقيدة"],
    bio: "حجة الإسلام، أعاد ترتيب أولويات الإسلام في كتابه إحياء علوم الدين، وكان إماماً في الفقه الشافعي والكلام والفلسفة",
    key_works: ["إحياء علوم الدين", "تهافت الفلاسفة", "المستصفى", "المنقذ من الضلال"],
    died: "٥٠٥ هـ",
    region: "طوس / خراسان",
    madhhab: "شافعي",
    quote: "العلم بلا عمل جنون، والعمل بلا علم لا يكون",
  },
  {
    id: "ibn-kathir",
    name: "ابن كثير",
    fullName: "إسماعيل بن عمر بن كثير الدمشقي",
    era: "العلماء الكبار",
    specialty: ["تفسير", "حديث", "تاريخ"],
    bio: "المفسر المحدث المؤرخ، صاحب أشهر تفسير للقرآن الكريم حتى اليوم، تلميذ ابن تيمية",
    key_works: ["تفسير القرآن العظيم", "البداية والنهاية", "الباعث الحثيث"],
    died: "٧٧٤ هـ",
    region: "الشام / دمشق",
    madhhab: "شافعي",
    quote: "أصحّ ما يُفسَّر به القرآن القرآنُ ثم السنة ثم أقوال الصحابة",
  },
  {
    id: "ibn-khaldun",
    name: "ابن خلدون",
    fullName: "عبد الرحمن بن محمد بن خلدون",
    era: "العلماء الكبار",
    specialty: ["تاريخ", "فقه", "أصول"],
    bio: "مؤسس علم الاجتماع والتاريخ الحضاري، صاحب المقدمة الشهيرة",
    key_works: ["المقدمة", "العبر وديوان المبتدأ والخبر", "التعريف بابن خلدون"],
    died: "٨٠٨ هـ",
    region: "تونس / الأندلس / مصر",
    madhhab: "مالكي",
    quote: "التاريخ في ظاهره لا يزيد على الإخبار عن الأيام والدول، وفي باطنه نظر وتحقيق",
  },
  {
    id: "ibn-ashur",
    name: "ابن عاشور",
    fullName: "محمد الطاهر بن محمد ابن عاشور",
    era: "المجددون",
    specialty: ["تفسير", "أصول", "مقاصد"],
    bio: "عَلَم التفسير والمقاصد في القرن العشرين، صاحب التحرير والتنوير، أضخم تفسير عربي حديث",
    key_works: ["التحرير والتنوير", "مقاصد الشريعة الإسلامية", "أصول النظام الاجتماعي في الإسلام"],
    died: "١٣٩٣ هـ",
    region: "تونس",
    madhhab: "مالكي",
    quote: "الشريعة كلها مبنية على تحقيق مصالح العباد في العاجل والآجل",
  },
  {
    id: "fawzan",
    name: "الشيخ صالح الفوزان",
    fullName: "صالح بن فوزان بن عبد الله الفوزان",
    era: "المعاصرون",
    specialty: ["فقه", "عقيدة", "فتوى"],
    bio: "عضو هيئة كبار العلماء في المملكة العربية السعودية، من أبرز مراجع الفتوى المعاصرة في العقيدة والفقه",
    key_works: ["الملخص الفقهي", "إعانة المستفيد بشرح كتاب التوحيد", "الأطعمة وأحكام الصيد والذبائح"],
    died: "—",
    region: "المملكة العربية السعودية",
    quote: "العلم الشرعي فريضة، والجهل به خطر على الفرد والمجتمع",
  },
  {
    id: "al-suyuti",
    name: "الإمام السيوطي",
    fullName: "جلال الدين عبد الرحمن السيوطي",
    era: "العلماء الكبار",
    specialty: ["تفسير", "حديث", "فقه", "لغة"],
    bio: "من أغزر العلماء تأليفاً في تاريخ الإسلام، صنّف في كل فنون الإسلام ما يتجاوز ستمائة مصنف",
    key_works: ["الإتقان في علوم القرآن", "الجامع الصغير", "الدر المنثور", "جمع الجوامع"],
    died: "٩١١ هـ",
    region: "مصر / القاهرة",
    madhhab: "شافعي",
    quote: "أهل الحديث حملة الإسلام والمدافعون عنه إلى يوم القيامة",
  },
  {
    id: "abu-dawud",
    name: "أبو داود السجستاني",
    fullName: "سليمان بن الأشعث السجستاني",
    era: "المحدثون",
    specialty: ["حديث", "فقه"],
    bio: "صاحب السنن، الكتاب الثالث من الكتب الستة، عُني بأحاديث الأحكام",
    key_works: ["سنن أبي داود", "المراسيل", "الزهد"],
    died: "٢٧٥ هـ",
    region: "سجستان / البصرة",
    madhhab: "حنبلي",
    quote: "انظروا كيف كانت أفعاله ﷺ فاتبعوه",
  },
  {
    id: "nasai",
    name: "الإمام النسائي",
    fullName: "أحمد بن شعيب بن علي النسائي",
    era: "المحدثون",
    specialty: ["حديث", "رجال", "علل"],
    bio: "صاحب السنن الصغرى (المجتبى)، المعروف بأشد الكتب الستة انتقاداً للرواة",
    key_works: ["السنن الصغرى", "السنن الكبرى", "الضعفاء والمتروكون"],
    died: "٣٠٣ هـ",
    region: "نسا (تركمانستان) / مصر",
    quote: "الكذاب على رسول الله ﷺ يجب أن يُرد حديثه ويُكذَّب",
  },
  {
    id: "daraqutni",
    name: "الإمام الدارقطني",
    fullName: "علي بن عمر الدارقطني البغدادي",
    era: "العلماء الكبار",
    specialty: ["حديث", "قراءات", "علل"],
    bio: "إمام الحفاظ في عصره، صاحب السنن والعلل، اشتُهر بمعرفة علل الحديث",
    key_works: ["العلل الواردة في الأحاديث", "سنن الدارقطني", "المختلف والمؤتلف"],
    died: "٣٨٥ هـ",
    region: "بغداد / دار القطن",
    madhhab: "شافعي",
    quote: "السنن ثلاث: سنة واجبة، وسنة فاضلة، وسنة مباحة",
  },
  {
    id: "bayhaki",
    name: "الإمام البيهقي",
    fullName: "أحمد بن الحسين بن علي البيهقي",
    era: "العلماء الكبار",
    specialty: ["حديث", "فقه", "عقيدة"],
    bio: "حافظ خراسان، صاحب السنن الكبرى أضخم موسوعة فقهية حديثية في التراث",
    key_works: ["السنن الكبرى", "شعب الإيمان", "الاعتقاد والهداية"],
    died: "٤٥٨ هـ",
    region: "بيهق / نيسابور",
    madhhab: "شافعي",
    quote: "من أراد أن يتفقه في الدين فليحفظ السنة",
  },
  {
    id: "ibn-juzayy",
    name: "ابن جزي الكلبي",
    fullName: "محمد بن أحمد بن محمد بن جزي الكلبي الغرناطي",
    era: "العلماء الكبار",
    specialty: ["تفسير", "فقه", "أصول"],
    bio: "فقيه وأصولي أندلسي، صاحب القوانين الفقهية أحد أشهر المختصرات المالكية",
    key_works: ["التسهيل لعلوم التنزيل", "القوانين الفقهية", "الكافي في الفقه"],
    died: "٧٤١ هـ",
    region: "غرناطة / الأندلس",
    madhhab: "مالكي",
    quote: "الفقه علم بالأحكام الشرعية العملية المستنبطة من أدلتها التفصيلية",
  },
  {
    id: "shatbi",
    name: "الإمام الشاطبي",
    fullName: "إبراهيم بن موسى اللخمي الغرناطي الشاطبي",
    era: "المجددون",
    specialty: ["أصول", "مقاصد", "فقه"],
    bio: "أبرع من كتب في مقاصد الشريعة وأسس لها تأسيساً منهجياً محكماً",
    key_works: ["الموافقات في أصول الشريعة", "الاعتصام", "الإفادات والإنشادات"],
    died: "٧٩٠ هـ",
    region: "غرناطة / الأندلس",
    madhhab: "مالكي",
    quote: "المقاصد الشرعية جاءت لحفظ الدين والنفس والعقل والنسل والمال",
  },
  {
    id: "shanqiti",
    name: "الشنقيطي",
    fullName: "محمد الأمين بن محمد المختار الشنقيطي",
    era: "المعاصرون",
    specialty: ["تفسير", "أصول", "حديث"],
    bio: "علم التفسير في القرن العشرين، قدم من موريتانيا إلى المدينة المنورة، ودرّس في الجامعة الإسلامية",
    key_works: ["أضواء البيان في إيضاح القرآن بالقرآن", "مذكرة أصول الفقه", "دفع إيهام الاضطراب"],
    died: "١٣٩٣ هـ",
    region: "موريتانيا / المدينة المنورة",
    quote: "أعظم ما يُفسَّر به القرآن القرآنُ ذاته",
  },
  {
    id: "bint-al-shati",
    name: "بنت الشاطئ",
    fullName: "عائشة عبد الرحمن المعروفة ببنت الشاطئ",
    era: "المعاصرون",
    specialty: ["تفسير", "لغة", "أدب"],
    bio: "رائدة التفسير البياني الأدبي للقرآن الكريم، أستاذة جامعية وكاتبة وأديبة",
    key_works: ["التفسير البياني للقرآن الكريم", "الإعجاز البياني للقرآن", "في حياة الشعراء"],
    died: "١٤١٩ هـ",
    region: "مصر",
    quote: "القرآن معجز ببيانه لا بمفرداته وحدها، والإعجاز البياني أعمق من الإعجاز اللغوي",
  },
  {
    id: "zuhayli",
    name: "الشيخ وهبة الزحيلي",
    fullName: "وهبة مصطفى الزحيلي",
    era: "المعاصرون",
    specialty: ["فقه", "تفسير", "أصول"],
    bio: "فقيه موسوعي سوري، أستاذ الفقه الإسلامي وأصوله بجامعة دمشق، ألّف أشمل موسوعة فقهية مقارنة في العصر الحديث",
    key_works: ["الفقه الإسلامي وأدلته (11 مجلداً)", "التفسير المنير (16 مجلداً)", "أصول الفقه الإسلامي"],
    died: "١٤٣٦ هـ",
    region: "دير عطية، سوريا",
    madhhab: "حنفي",
    quote: "الفقه الإسلامي ثروة بشرية ضخمة وتراث إنساني فريد لا نظير له في أي حضارة",
  },
  {
    id: "abu-zayd",
    name: "الشيخ بكر أبو زيد",
    fullName: "بكر بن عبد الله أبو زيد",
    era: "المعاصرون",
    specialty: ["فقه", "عقيدة", "لغة"],
    bio: "فقيه سعودي وعضو هيئة كبار العلماء، اشتُهر بمحاربة الألفاظ الدخيلة وبعنايته بآداب طالب العلم",
    key_works: ["معجم المناهي اللفظية", "حلية طالب العلم", "لا جديد في أحكام الصلاة"],
    died: "١٤٢٩ هـ",
    region: "القصيم، المملكة العربية السعودية",
    quote: "اللغة وعاء الهوية، والتهاون بها تهاون بالدين وبالأمة",
  },
  {
    id: "muhammad-ghazali",
    name: "الشيخ محمد الغزالي",
    fullName: "محمد الغزالي السقا",
    era: "المعاصرون",
    specialty: ["دعوة", "عقيدة", "فقه"],
    bio: "داعية مصري من أبرز رموز الإسلام الوسطي في القرن العشرين",
    key_works: ["فقه السيرة", "الإسلام والأوضاع الاقتصادية", "كيف نفهم الإسلام", "خلق المسلم"],
    died: "١٤١٦ هـ",
    region: "مصر / السعودية",
    madhhab: "شافعي",
    quote: "الإسلام في أمة تجهله غريب كما كان في أول يوم جاء",
  },
  {
    id: "mubarakfuri",
    name: "المباركفوري",
    fullName: "محمد بن عبد الرحمن بن عبد الرحيم المباركفوري",
    era: "المجددون",
    specialty: ["حديث", "سيرة", "شروح"],
    bio: "محدث هندي، صاحب أشهر شروح سنن الترمذي تحفة الأحوذي، كما كتب الرحيق المختوم",
    key_works: ["تحفة الأحوذي شرح جامع الترمذي", "الرحيق المختوم"],
    died: "١٣٥٣ هـ",
    region: "الهند",
    madhhab: "أهل الحديث",
    quote: "السيرة النبوية مدرسة كاملة للحياة في كل أبعادها",
  },
  {
    id: "tabari",
    name: "الطبري",
    fullName: "محمد بن جرير الطبري",
    era: "العلماء الكبار",
    specialty: ["تفسير", "حديث", "فقه"],
    bio: "إمام المفسرين وأمير المؤرخين، جمع بين التفسير والحديث والفقه والتاريخ",
    key_works: ["جامع البيان في تأويل القرآن", "تاريخ الرسل والملوك"],
    died: "٣١٠ هـ",
    region: "بغداد",
    madhhab: "اجتهادي",
    quote: "العالم لا يكون عالماً حتى يجمع بين الرواية والدراية",
  },
  {
    id: "ibn-jawzi",
    name: "ابن الجوزي",
    fullName: "عبد الرحمن بن علي بن الجوزي",
    era: "العلماء الكبار",
    specialty: ["حديث", "وعظ", "رجال"],
    bio: "وَاعِظ بغداد وإمام عصره في الحنابلة، ألّف في التصوف الصحيح والأخلاق والوعظ",
    key_works: ["تلبيس إبليس", "صيد الخاطر", "المنتظم في التاريخ"],
    died: "٥٩٧ هـ",
    region: "بغداد",
    madhhab: "حنبلي",
    quote: "الوقت أعزّ الأشياء على المرء فلا يُضيّعه في ما لا ينفع",
  },
  {
    id: "ibn-rajab",
    name: "ابن رجب الحنبلي",
    fullName: "عبد الرحمن بن أحمد بن رجب الحنبلي",
    era: "المحدثون",
    specialty: ["حديث", "فقه"],
    bio: "محدث حنبلي دمشقي، أحد أبرز شراح الأحاديث النبوية",
    key_works: ["جامع العلوم والحكم شرح الأربعين", "فتح الباري شرح البخاري", "لطائف المعارف"],
    died: "٧٩٥ هـ",
    region: "دمشق",
    madhhab: "حنبلي",
    quote: "طوبى لمن أخمل نفسه وأشغلها بمعرفة عيوبها",
  },
  {
    id: "ibn-abd-al-barr",
    name: "ابن عبد البر",
    fullName: "يوسف بن عبد الله بن عبد البر النمري القرطبي",
    era: "العلماء الكبار",
    specialty: ["حديث", "رجال", "فقه"],
    bio: "حافظ المغرب وإمام المالكية في الأندلس، صاحب الاستيعاب في أسماء الصحابة",
    key_works: ["الاستيعاب في معرفة الأصحاب", "التمهيد لما في الموطأ من المعاني", "جامع بيان العلم"],
    died: "٤٦٣ هـ",
    region: "الأندلس",
    madhhab: "مالكي",
    quote: "العلم يزيد بالبحث ويُضعف الشك ويقوى باليقين",
  },
  {
    id: "dhahabi",
    name: "الإمام الذهبي",
    fullName: "محمد بن أحمد بن عثمان الذهبي",
    era: "العلماء الكبار",
    specialty: ["حديث", "رجال", "تاريخ"],
    bio: "حافظ الإسلام في القرن الثامن الهجري، أكثر العلماء تأليفاً في علوم الرجال والتراجم",
    key_works: ["سير أعلام النبلاء", "ميزان الاعتدال", "تذكرة الحفاظ", "الكاشف", "تاريخ الإسلام"],
    died: "٧٤٨ هـ",
    region: "دمشق",
    madhhab: "شافعي",
    quote: "الرجل الثقة يُعدل بألف سواه، وتعديله للراوي يوجب خشية الله في نقل الحديث",
  },
  {
    id: "izz-ibn-abd-al-salam",
    name: "العز بن عبد السلام",
    fullName: "عبد العزيز بن عبد السلام السلمي الدمشقي",
    era: "العلماء الكبار",
    specialty: ["فقه", "أصول", "مقاصد"],
    bio: "سلطان العلماء، صاحب قواعد الأحكام في مصالح الأنام، وضع أسس الفقه المقاصدي",
    key_works: ["قواعد الأحكام في مصالح الأنام", "الفوائد في مشكل القرآن", "شجرة المعارف والأحوال"],
    died: "٦٦٠ هـ",
    region: "دمشق / مصر",
    madhhab: "شافعي",
    quote: "درء المفاسد مقدَّم على جلب المصالح عند التعارض",
  },
  {
    id: "ibn-abd-al-wahhab",
    name: "الشيخ محمد بن عبد الوهاب",
    fullName: "محمد بن عبد الوهاب بن سليمان التميمي النجدي",
    era: "المجددون",
    specialty: ["عقيدة", "فقه", "دعوة"],
    bio: "إمام الدعوة الإصلاحية في الجزيرة العربية، جدّد دعوة التوحيد في القرن الثاني عشر الهجري",
    key_works: ["كتاب التوحيد", "كشف الشبهات", "الأصول الثلاثة", "الواجبات المتحتمات"],
    died: "١٢٠٦ هـ",
    region: "نجد، الجزيرة العربية",
    madhhab: "حنبلي",
    quote: "توحيد الله هو أعظم واجب وأجل مطلوب",
  },
  {
    id: "ibn-abidin",
    name: "ابن عابدين",
    fullName: "محمد أمين بن عمر بن عبد العزيز ابن عابدين الشامي",
    era: "المجددون",
    specialty: ["فقه", "أصول"],
    bio: "فقيه الشام وخاتمة أئمة الحنفية، صاحب حاشية ردّ المحتار المرجع الأول للفتوى الحنفية",
    key_works: ["ردّ المحتار على الدر المختار (حاشية ابن عابدين)", "العقود الدرية", "منحة الخالق"],
    died: "١٢٥٢ هـ",
    region: "دمشق",
    madhhab: "حنفي",
    quote: "التقليد لا يجوز في المسائل الاعتقادية، وإنما يجوز في الفروع الفقهية",
  },
  {
    id: "rashid-rida",
    name: "رشيد رضا",
    fullName: "محمد رشيد بن علي رضا القلموني",
    era: "المجددون",
    specialty: ["تفسير", "دعوة", "أصول"],
    bio: "مجدد القرن الرابع عشر الهجري، تلميذ محمد عبده وصاحب مجلة المنار",
    key_works: ["تفسير المنار", "الخلافة أو الإمامة العظمى", "الوحي المحمدي"],
    died: "١٣٥٤ هـ",
    region: "سوريا / مصر",
    madhhab: "حنفي / اجتهادي",
    quote: "الإسلام دين العقل والعلم، وهو أصلح نظام للبشر في كل زمان",
  },
  {
    id: "muhammad-abdu",
    name: "الشيخ محمد عبده",
    fullName: "محمد بن عبده بن حسن خيرالله",
    era: "المجددون",
    specialty: ["عقيدة", "تفسير", "أصول"],
    bio: "مجدد مصري وإمام الإصلاح الديني، مفتي مصر وشيخ الأزهر",
    key_works: ["رسالة التوحيد", "تفسير القرآن الحكيم (المنار)", "الإسلام والنصرانية مع العلم والمدنية"],
    died: "١٣٢٣ هـ",
    region: "مصر",
    madhhab: "اجتهادي",
    quote: "جئتُ لأُحرّر العقل من قيد التقليد وأفهم الدين على طريقة السلف قبل ظهور الخلاف",
  },
  {
    id: "ibn-badis",
    name: "عبد الحميد بن باديس",
    fullName: "عبد الحميد بن محمد المصطفى بن مكي بن باديس",
    era: "المجددون",
    specialty: ["تفسير", "دعوة", "لغة"],
    bio: "إمام التجديد الإسلامي في الجزائر، مؤسس جمعية العلماء المسلمين الجزائريين",
    key_works: ["مجالس التذكير من كلام الحكيم الخبير", "العقائد الإسلامية من الآيات القرآنية"],
    died: "١٣٥٩ هـ",
    region: "قسنطينة، الجزائر",
    madhhab: "مالكي",
    quote: "الإسلام ديننا والعربية لغتنا والجزائر وطننا",
  },
  // ── علماء مضافون من IslamicScholarsPage ──
  {
    id: "abu-yusuf",
    name: "الإمام أبو يوسف",
    fullName: "يعقوب بن إبراهيم بن حبيب الأنصاري",
    era: "المحدثون",
    specialty: ["فقه", "أصول", "قضاء"],
    bio: "أول من لُقِّب بـ«قاضي القضاة» في الإسلام، تلميذ أبي حنيفة النعمان الأبرز، نشر المذهب الحنفي وهذَّبه وأثرى أصوله، وأسهم في وضع نظام القضاء الإسلامي",
    key_works: ["كتاب الخراج", "الآثار", "الرد على سير الأوزاعي"],
    died: "١٨٢ هـ",
    region: "الكوفة / بغداد",
    madhhab: "حنفي",
    quote: "ما من شيء أبعد عن العلماء من صحبة السلطان",
  },
  {
    id: "shaybani",
    name: "الإمام محمد الشيباني",
    fullName: "محمد بن الحسن بن فرقد الشيباني",
    era: "المحدثون",
    specialty: ["فقه", "أصول"],
    bio: "ثالث أعلام الحنفية بعد أبي حنيفة وأبي يوسف، وضع مؤلفات المذهب الحنفي الأصلية المعروفة بـ«ظاهر الرواية» الستة، والتقى بالإمام الشافعي فأفاد كل منهما الآخر",
    key_works: ["المبسوط", "الجامع الكبير", "الجامع الصغير", "السير الكبير"],
    died: "١٨٩ هـ",
    region: "الكوفة / بغداد / الري",
    madhhab: "حنفي",
    quote: "من لا يعرف الخلاف لا تشمّ أنفه رائحة الفقه",
  },
  {
    id: "muzani",
    name: "الإمام المزني",
    fullName: "إسماعيل بن يحيى المزني",
    era: "المحدثون",
    specialty: ["فقه", "أصول"],
    bio: "من أبرز أصحاب الإمام الشافعي ونابهيهم، قال عنه الشافعي: «المزني ناصر مذهبي». كتابه «المختصر» من أشهر مختصرات الفقه وعليه شروح لا تحصى",
    key_works: ["المختصر في الفقه الشافعي", "المنثور", "شرح السنة للشافعي"],
    died: "٢٦٤ هـ",
    region: "مصر",
    madhhab: "شافعي",
    quote: "فقه الشافعي محيط وأنا ناقل لا مستنبط",
  },
  {
    id: "ibn-hibban",
    name: "الإمام ابن حبان",
    fullName: "محمد بن حبان بن أحمد التميمي البُستي",
    era: "العلماء الكبار",
    specialty: ["حديث", "رجال", "فقه"],
    bio: "محدث موسوعي وفقيه شافعي، اشتُهر بكتابيه الصحيح والضعفاء، وطريقته في تصحيح الحديث مستقلة تُعدّ تساهلاً عند الجمهور وهو يراها تحقيقاً",
    key_works: ["صحيح ابن حبان (الإحسان)", "كتاب المجروحين", "الثقات"],
    died: "٣٥٤ هـ",
    region: "بُست / خراسان",
    madhhab: "شافعي",
    quote: "أقل ما يجب على طالب العلم أن يبحث حتى ييأس",
  },
  {
    id: "ibn-khuzayma",
    name: "الإمام ابن خزيمة",
    fullName: "محمد بن إسحاق بن خزيمة النيسابوري",
    era: "العلماء الكبار",
    specialty: ["حديث", "فقه", "عقيدة"],
    bio: "إمام الأئمة كما لقّبه العلماء، جمع بين الحديث والفقه والعقيدة، وكتابه التوحيد من أنفس كتب إثبات الصفات، وصحيحه متقن الانتقاء",
    key_works: ["صحيح ابن خزيمة", "كتاب التوحيد وإثبات صفات الرب", "مختصر المختصر"],
    died: "٣١١ هـ",
    region: "نيسابور / خراسان",
    madhhab: "شافعي",
    quote: "ليس تحت أديم السماء أعلم بحديث رسول الله من أبي بكر بن خزيمة",
  },
  {
    id: "hakim",
    name: "الإمام الحاكم",
    fullName: "محمد بن عبد الله بن محمد النيسابوري",
    era: "العلماء الكبار",
    specialty: ["حديث", "رجال"],
    bio: "أمير المحدثين في عصره بخراسان، شهير بكتابه «المستدرك على الصحيحين»، وإن أُخذ عليه التساهل في الحكم على الأحاديث، وكتابه «معرفة علوم الحديث» من أصول المصطلح",
    key_works: ["المستدرك على الصحيحين", "معرفة علوم الحديث", "تاريخ نيسابور"],
    died: "٤٠٥ هـ",
    region: "نيسابور / خراسان",
    madhhab: "شافعي",
    quote: "كم من صحيح لم يُخرَّج وضعيف قد وُضِع",
  },
  {
    id: "ibn-mubarak",
    name: "الإمام عبد الله بن المبارك",
    fullName: "عبد الله بن المبارك المروزي",
    era: "المحدثون",
    specialty: ["حديث", "زهد", "جهاد"],
    bio: "جمع بين الحديث والفقه والجهاد والزهد حتى كانوا يقولون إنه جمع فيه كل خصلة محمودة. اشتُهر بنشر الحديث وتحفيز الهمم نحو الجهاد والعلم",
    key_works: ["كتاب الزهد والرقائق", "المسند", "كتاب الجهاد"],
    died: "١٨١ هـ",
    region: "مرو / خراسان",
    madhhab: "اجتهادي",
    quote: "رُبَّ عمل صغير تُكثِّره النية، ورُبَّ عمل كبير تُصغِّره النية",
  },
  {
    id: "sufyan-thawri",
    name: "الإمام سفيان الثوري",
    fullName: "سفيان بن سعيد بن مسروق الثوري الكوفي",
    era: "المحدثون",
    specialty: ["حديث", "فقه", "زهد"],
    bio: "أمير المؤمنين في الحديث، أحد أوعية السنة ومحيطيها، كان مذهبه الفقهي مشهوراً قبل أن ينقرض. قال شعبة: «سفيان أمير المؤمنين في الحديث»",
    key_works: ["الجامع الكبير", "الجامع الصغير", "كتاب الفرائض"],
    died: "١٦١ هـ",
    region: "الكوفة / مكة",
    madhhab: "ثوري",
    quote: "ما أعددتُ للجنة عُدَّتها، ولا للنار جُنَّتها",
  },
  {
    id: "awzai",
    name: "الإمام الأوزاعي",
    fullName: "عبد الرحمن بن عمرو الأوزاعي",
    era: "المحدثون",
    specialty: ["فقه", "حديث"],
    bio: "إمام أهل الشام في عصره وصاحب المذهب الأوزاعي الذي سادَ الشام والأندلس قروناً ثم اضمحل. كان رائداً في فقه السياسة الشرعية وأحكام الجهاد",
    key_works: ["السير (في أحكام الجهاد)", "المسائل", "مسائل في الصيام"],
    died: "١٥٧ هـ",
    region: "الشام / بيروت",
    madhhab: "أوزاعي",
    quote: "تمسَّك بآثار من سلف وإن رفضك الناس",
  },
  {
    id: "laith-ibn-sad",
    name: "الإمام الليث بن سعد",
    fullName: "الليث بن سعد بن عبد الرحمن الفهمي",
    era: "المحدثون",
    specialty: ["حديث", "فقه"],
    bio: "إمام مصر بلا منازع في عصره، قال عنه الشافعي: «الليث أفقه من مالك إلا أن أصحابه لم يقوموا به». تميَّز باستقلالية الرأي وسعة الاطلاع وكرم النفس",
    key_works: ["الأجوبة على فقه مالك", "مسائل في الفقه مروية عنه"],
    died: "١٧٥ هـ",
    region: "مصر",
    madhhab: "اجتهادي",
    quote: "الفقه في الأصول والمصالح لا في كثرة النقل",
  },
  {
    id: "ibn-al-mundhir",
    name: "الإمام ابن المنذر",
    fullName: "محمد بن إبراهيم بن المنذر النيسابوري",
    era: "العلماء الكبار",
    specialty: ["فقه", "أصول"],
    bio: "إمام الفقه المقارن في القرنين الثالث والرابع، صنَّف الإجماع والخلاف بدقة متناهية، كتابه «الأوسط» في الفقه المقارن من أنفس ما كتب في هذا الباب",
    key_works: ["الأوسط في السنن والإجماع والاختلاف", "الإشراف على مذاهب العلماء", "كتاب الإجماع"],
    died: "٣٢٠ هـ",
    region: "مكة المكرمة",
    madhhab: "شافعي",
    quote: "الإجماع حجة والخلاف رحمة",
  },
  {
    id: "khattabi",
    name: "الإمام الخطابي",
    fullName: "حمد بن محمد بن إبراهيم الخطابي البُستي",
    era: "العلماء الكبار",
    specialty: ["حديث", "فقه", "لغة"],
    bio: "محدث وفقيه وأديب، جمع بين علم الحديث واللغة، وكتابه «معالم السنن» من أجود شروح سنن أبي داود في جمعه بين الفقه واللغة ونكت الحديث",
    key_works: ["معالم السنن (شرح أبي داود)", "غريب الحديث", "شأن الدعاء"],
    died: "٣٨٨ هـ",
    region: "بُست / خراسان",
    madhhab: "شافعي",
    quote: "لا يُنتفَع بعلم بلا عمل كما لا يُنتفَع بسيف بلا يد",
  },
  {
    id: "ibn-hajar-haytami",
    name: "ابن حجر الهيتمي",
    fullName: "أحمد بن محمد بن علي بن حجر الهيتمي",
    era: "العلماء الكبار",
    specialty: ["فقه", "فتوى"],
    bio: "كبير فقهاء الشافعية في عصره، مرجع الفتوى في مصر والحجاز وكثير من بلاد الإسلام، كتابه «تحفة المحتاج» من أمهات المذهب الشافعي المتأخر",
    key_works: ["تحفة المحتاج بشرح المنهاج", "الزواجر عن اقتراف الكبائر", "الفتاوى الفقهية الكبرى"],
    died: "٩٧٤ هـ",
    region: "مصر / مكة المكرمة",
    madhhab: "شافعي",
    quote: "الفقه علم يُحيي القلوب ويُميت الهوى",
  },
  {
    id: "ali-tantawi",
    name: "الشيخ علي الطنطاوي",
    fullName: "علي بن مصطفى بن أحمد الطنطاوي",
    era: "المعاصرون",
    specialty: ["دعوة", "أدب", "فقه"],
    bio: "عالم وأديب وداعية سوري، اشتُهر بأسلوبه العذب في الكتابة الإسلامية وسهولة تقريب الأحكام، وكانت مقالاته ومذكراته مرجعاً لأجيال من القراء العرب",
    key_works: ["مذكرات علي الطنطاوي", "رجال من التاريخ", "هكذا علَّمتني الحياة", "فكر ومباحث"],
    died: "١٤٢٠ هـ",
    region: "دمشق / جدة",
    madhhab: "اجتهادي",
    quote: "لا أحسد الناس على شيء إلا على العلم الذي يُنفَع به",
  },
  {
    id: "shaarawi",
    name: "الشيخ الشعراوي",
    fullName: "محمد متولي الشعراوي",
    era: "المعاصرون",
    specialty: ["تفسير", "دعوة", "لغة"],
    bio: "إمام الدعاة في القرن العشرين، اشتُهر ببرنامج «خواطر» التفسيري الذي حوَّل القرآن إلى خطاب شعبي مؤثر، وتفسيره المبسَّط بلغ عشرات المجلدات",
    key_works: ["تفسير الشعراوي (25 مجلداً)", "الإسلام والمرأة", "الجنة والنار"],
    died: "١٤١٩ هـ",
    region: "مصر",
    madhhab: "شافعي",
    quote: "القرآن ليس للحفظ فقط، بل للفهم والعمل",
  },
  {
    id: "ibn-ghudiyan",
    name: "الشيخ عبد الله بن غديان",
    fullName: "عبد الله بن عبد الرحمن بن غديان الغامدي",
    era: "المعاصرون",
    specialty: ["فقه", "عقيدة", "فتوى"],
    bio: "عضو هيئة كبار العلماء بالمملكة العربية السعودية وعضو اللجنة الدائمة للبحوث العلمية والإفتاء لأكثر من أربعين سنة، اشتُهر بالتحقيق والتدقيق في المسائل",
    key_works: ["فتاوى اللجنة الدائمة (مشاركاً)", "تصحيح بعض المفاهيم"],
    died: "١٤٣٤ هـ",
    region: "الرياض، المملكة العربية السعودية",
    madhhab: "حنبلي",
    quote: "الفتوى أمانة والورع قبلها شرط",
  },
  {
    id: "afifi",
    name: "الشيخ عبد الرزاق عفيفي",
    fullName: "عبد الرزاق بن عفيفي بن عطية بن سالم",
    era: "المعاصرون",
    specialty: ["عقيدة", "فقه", "حديث"],
    bio: "نائب رئيس اللجنة الدائمة للبحوث العلمية والإفتاء، شيخ ابن باز المفضَّل في علوم الحديث، اشتُهر بتوسعه في علم العلل والتصحيح والتضعيف",
    key_works: ["الفتاوى (مشاركاً)", "مذكرات في أصول الفقه"],
    died: "١٤١٥ هـ",
    region: "مصر / الرياض",
    madhhab: "حنبلي",
    quote: "صحيح السنة دستور المسلمين وحكم لا يُرَدّ",
  },
  {
    id: "sufyan-uyayna",
    name: "الإمام سفيان بن عيينة",
    fullName: "سفيان بن عيينة بن أبي عمران الهلالي",
    era: "المحدثون",
    specialty: ["حديث", "تفسير"],
    bio: "إمام الحرمين في الحديث والتفسير في عصره، أقام بمكة وأخذ عنه الشافعي وأحمد ويحيى بن معين وغيرهم، شيخ مكة المكرمة ومحدِّثها الأول في عصره",
    key_works: ["الجامع (مسند، مفقود)", "تفسير مجالسه (مروية في الكتب)"],
    died: "١٩٨ هـ",
    region: "مكة المكرمة",
    madhhab: "اجتهادي",
    quote: "الزهد في الدنيا اليقين بالله وقِصَر الأمل",
  },
  {
    id: "muqbil",
    name: "الشيخ مقبل بن هادي الوادعي",
    fullName: "مقبل بن هادي بن مقبل الوادعي الحمداني",
    era: "المعاصرون",
    specialty: ["حديث", "عقيدة"],
    bio: "أبرز محدثي اليمن في القرن العشرين، أسَّس مركز دار الحديث بدماج، نشر السنة وعلومها في اليمن والجزيرة العربية، وتتلمذ على يديه الآلاف",
    key_works: ["الصحيح المسند مما ليس في الصحيحين", "الجامع الصحيح مما ليس في الصحيحين", "رجال الحاكم في المستدرك"],
    died: "١٤٢٢ هـ",
    region: "صعدة، اليمن",
    madhhab: "حنبلي",
    quote: "السنة واجبة الاتباع وإن خالفت القياس",
  },
  {
    id: "ibn-mufli",
    name: "الإمام ابن مفلح",
    fullName: "محمد بن مفلح بن محمد المقدسي",
    era: "العلماء الكبار",
    specialty: ["فقه", "أصول", "عقيدة"],
    bio: "من أبرز أصحاب ابن تيمية وابن القيم، تميَّز بالدقة والتحرير الفقهي، وكتابه «الفروع» من أمهات الحنابلة ومرجع المتقدمين والمتأخرين",
    key_works: ["الفروع (9 مجلدات)", "الآداب الشرعية والمنح المرعية", "أصول الفقه"],
    died: "٧٦٣ هـ",
    region: "الشام / مصر",
    madhhab: "حنبلي",
    quote: "العلم بلا عمل كالشجر بلا ثمر",
  },
  {
    id: "ibn-rushd",
    name: "ابن رشد",
    fullName: "محمد بن أحمد بن محمد بن رشد القرطبي",
    era: "العلماء الكبار",
    specialty: ["فقه", "فلسفة", "طب"],
    bio: "فيلسوف الإسلام الأكبر وشارح أرسطو، فقيه مالكي وطبيب، وضع بداية المجتهد في الفقه المقارن بين المذاهب",
    key_works: ["بداية المجتهد ونهاية المقتصد", "تهافت التهافت", "الكليات في الطب"],
    died: "٥٩٥ هـ",
    region: "قرطبة، الأندلس",
    madhhab: "مالكي",
    quote: "الحق لا يضاده إلا الباطل",
  },
  {
    id: "ibn-hazm",
    name: "ابن حزم الأندلسي",
    fullName: "علي بن أحمد بن سعيد بن حزم القرطبي",
    era: "العلماء الكبار",
    specialty: ["فقه", "حديث", "أصول", "عقيدة"],
    bio: "إمام الظاهرية، موسوعي في الفقه والحديث والتاريخ والفلسفة، اشتُهر بجرأة الرأي ودقة التحليل، وله إلمام بعلوم عصره",
    key_works: ["المحلى بالآثار", "الإحكام في أصول الأحكام", "الفصل في الملل والأهواء والنحل"],
    died: "٤٥٦ هـ",
    region: "قرطبة، الأندلس",
    madhhab: "ظاهري",
    quote: "من يصبر على علم ولم يصبر على عمل فليرجع إلى علمه",
  },
  {
    id: "ibn-al-qayyim-alt",
    name: "السخاوي",
    fullName: "محمد بن عبد الرحمن بن محمد السخاوي",
    era: "العلماء الكبار",
    specialty: ["حديث", "رجال", "تاريخ"],
    bio: "حافظ الإسلام في عصره، تلميذ ابن حجر العسقلاني، صاحب الضوء اللامع في أعلام القرن التاسع وأكثر من خمسمائة كتاب",
    key_works: ["الضوء اللامع لأهل القرن التاسع", "فتح المغيث شرح ألفية الحديث", "المقاصد الحسنة"],
    died: "٩٠٢ هـ",
    region: "القاهرة / المدينة المنورة",
    madhhab: "شافعي",
    quote: "العلم نور والجهل ظلمة",
  },
  {
    id: "ibn-asakir",
    name: "ابن عساكر",
    fullName: "علي بن الحسن بن هبة الله بن عساكر",
    era: "العلماء الكبار",
    specialty: ["حديث", "تاريخ", "رجال"],
    bio: "حافظ الشام وإمامها، صاحب تاريخ دمشق الكبير في ثمانين مجلداً، أحد أكبر موسوعات التراجم والتاريخ في الإسلام",
    key_works: ["تاريخ دمشق الكبير (80 مجلداً)", "تبيين كذب المفتري على الأشعري"],
    died: "٥٧١ هـ",
    region: "دمشق، الشام",
    madhhab: "شافعي",
    quote: "علم الإسناد ميزان الأمة",
  },
  {
    id: "ibn-nujaym",
    name: "ابن نجيم",
    fullName: "زين الدين بن إبراهيم بن محمد بن نجيم المصري",
    era: "العلماء الكبار",
    specialty: ["فقه", "أصول"],
    bio: "فقيه حنفي بارز، أعلم أهل زمانه بالمذهب الحنفي، وضع الأشباه والنظائر الذي يعدّ مرجعاً أساسياً للقواعد الفقهية الحنفية",
    key_works: ["البحر الرائق شرح كنز الدقائق", "الأشباه والنظائر", "رسائل ابن نجيم"],
    died: "٩٧٠ هـ",
    region: "القاهرة، مصر",
    madhhab: "حنفي",
    quote: "الأمور بمقاصدها",
  },
  {
    id: "khatib-baghdadi",
    name: "الخطيب البغدادي",
    fullName: "أحمد بن علي بن ثابت، أبو بكر الخطيب البغدادي",
    era: "المحدثون",
    specialty: ["حديث", "رجال", "تاريخ"],
    bio: "حافظ العراق في عصره، صاحب تاريخ بغداد في أربعة عشر مجلداً، وضع كتاباً في اشتراطات الرواية وآداب الحديث يُعدّ مرجعاً فريداً",
    key_works: ["تاريخ بغداد (14 مجلداً)", "الجامع لأخلاق الراوي وآداب السامع", "الكفاية في علم الرواية"],
    died: "٤٦٣ هـ",
    region: "بغداد، العراق",
    madhhab: "شافعي",
    quote: "أحسن الروايات ما أُتقنت فيه الأسانيد",
  },
  {
    id: "amir-al-san'ani",
    name: "الأمير الصنعاني",
    fullName: "محمد بن إسماعيل بن صلاح الأمير الصنعاني",
    era: "المجددون",
    specialty: ["حديث", "فقه", "أصول"],
    bio: "عالم اليمن ومحدثها في القرن الثاني عشر، صاحب سبل السلام شرح بلوغ المرام، ناضل عن الاجتهاد ومحاربة التقليد الأعمى",
    key_works: ["سبل السلام شرح بلوغ المرام", "إرشاد النقاد إلى تيسير الاجتهاد", "توضيح الأفكار"],
    died: "١١٨٢ هـ",
    region: "صنعاء، اليمن",
    madhhab: "اجتهادي",
    quote: "العلماء ورثة الأنبياء، فمن كان أكثرهم علماً كان أكثرهم ورثةً",
  },
  {
    id: "ibn-faris",
    name: "ابن فارس",
    fullName: "أحمد بن فارس بن زكريا القزويني الرازي",
    era: "العلماء الكبار",
    specialty: ["لغة", "فقه"],
    bio: "إمام اللغة وعلم الاشتقاق، صاحب معجم مقاييس اللغة الذي يرد الألفاظ إلى أصولها ثلاثية الاشتقاق، وكان مع ذلك فقيهاً مالكياً بارعاً",
    key_works: ["معجم مقاييس اللغة", "المجمل في اللغة", "الصاحبي في فقه اللغة"],
    died: "٣٩٥ هـ",
    region: "همذان / قزوين",
    madhhab: "مالكي",
    quote: "أصول العربية كلها ترجع إلى ثلاثة",
  },
  {
    id: "zarkashi",
    name: "الزركشي",
    fullName: "بدر الدين محمد بن عبد الله الزركشي",
    era: "العلماء الكبار",
    specialty: ["أصول", "فقه", "علوم القرآن"],
    bio: "موسوعي في الفقه وأصوله وعلوم القرآن، صاحب البرهان في علوم القرآن الكتاب الأكثر شمولاً في تاريخ علوم القرآن",
    key_works: ["البرهان في علوم القرآن (4 مجلدات)", "البحر المحيط في أصول الفقه", "المنثور في القواعد"],
    died: "٧٩٤ هـ",
    region: "القاهرة، مصر",
    madhhab: "شافعي",
    quote: "العلم بعلوم القرآن يفتح أبواب التفسير من غير حصر",
  },
  {
    id: "ibn-uthaymin-ext",
    name: "الشيخ ابن جبرين",
    fullName: "عبد الله بن عبد الرحمن بن جبرين",
    era: "المعاصرون",
    specialty: ["عقيدة", "فقه", "فتوى"],
    bio: "عضو هيئة كبار العلماء في المملكة العربية السعودية، اشتُهر بسهولة الإفتاء وتقريب المسائل للعامة وغزارة العطاء العلمي في الدروس والمحاضرات",
    key_works: ["فتح القريب المجيب شرح كتاب التقريب", "التوضيح والبيان لشجرة الإيمان"],
    died: "١٤٣٠ هـ",
    region: "الرياض، المملكة العربية السعودية",
    madhhab: "حنبلي",
    quote: "التعلم في الصغر كالنقش في الحجر",
  },
  {
    id: "syed-saabiq",
    name: "الشيخ سيد سابق",
    fullName: "سيد سابق محمد التهامي",
    era: "المعاصرون",
    specialty: ["فقه", "دعوة"],
    bio: "فقيه مصري من علماء الأزهر، صاحب فقه السنة الذي يُعدّ من أكثر كتب الفقه المقارن انتشاراً في القرن العشرين",
    key_works: ["فقه السنة (3 مجلدات)", "العقائد الإسلامية"],
    died: "١٤٢٠ هـ",
    region: "القاهرة، مصر",
    madhhab: "اجتهادي",
    quote: "الفقه نور والجهل به ظلام",
  },
  {
    id: "ibn-uthaymeen-older",
    name: "الشيخ عبد الكريم الخضير",
    fullName: "عبد الكريم بن عبد الله بن عبد الرحمن الخضير",
    era: "المعاصرون",
    specialty: ["حديث", "فقه", "عقيدة"],
    bio: "عضو هيئة كبار العلماء في المملكة العربية السعودية، اشتُهر بعمق التحقيق في الحديث وعلله وتفرد كثير من المسائل باستنباطات دقيقة",
    key_works: ["شرح الموقظة في علم مصطلح الحديث", "شرح الفية العراقي", "التعليق على البلوغ"],
    died: "معاصر",
    region: "الرياض، المملكة العربية السعودية",
    madhhab: "حنبلي",
    quote: "العلم الحقيقي هو الذي يخشع به الله",
  },
  {
    id: "ibn-qudama",
    name: "ابن قدامة المقدسي",
    fullName: "أبو محمد موفق الدين عبد الله بن قدامة المقدسي",
    era: "العلماء الكبار",
    specialty: ["فقه", "عقيدة", "حديث"],
    bio: "إمام الحنابلة في زمانه وفقيه المذهب غير المنازع، نشأ في المقدس وأقام في دمشق، موسوعي في الفقه والعقيدة والزهد، يُعدّ المغني من أعظم كتب الفقه الإسلامي المقارن",
    key_works: ["المغني (10 مجلدات)", "الكافي في فقه الإمام أحمد", "المقنع", "لمعة الاعتقاد", "روضة الناظر"],
    died: "٦٢٠ هـ",
    region: "دمشق، الشام",
    madhhab: "حنبلي",
    quote: "العلم الذي لا يُعمل به وبالٌ على صاحبه",
  },
  {
    id: "juwayny",
    name: "إمام الحرمين الجويني",
    fullName: "أبو المعالي عبد الملك بن عبد الله الجويني",
    era: "العلماء الكبار",
    specialty: ["أصول", "عقيدة", "فقه"],
    bio: "إمام الحرمين لقب لقبه أهل عصره، شافعي المذهب، أستاذ الغزالي، من أبرز علماء أصول الفقه وعلم الكلام في تاريخ الإسلام، وضع البرهان في أصول الفقه الذي يُعدّ مرجعاً متقدماً",
    key_works: ["البرهان في أصول الفقه", "الإرشاد إلى قواطع الأدلة", "نهاية المطلب", "الشامل"],
    died: "٤٧٨ هـ",
    region: "نيسابور، خراسان",
    madhhab: "شافعي",
    quote: "من لم يتقن الأصول حُرم الوصول",
  },
  {
    id: "ibn-salah",
    name: "ابن الصلاح",
    fullName: "أبو عمرو عثمان بن عبد الرحمن ابن الصلاح الشهرزوري",
    era: "العلماء الكبار",
    specialty: ["حديث", "فقه"],
    bio: "علامة محدثو زمانه وإمام دار الحديث في دمشق، مقدمته في علوم الحديث أصّلت هذا الفن وصارت مرجعاً لا غنى عنه لكل طالب علم الحديث حتى اليوم",
    key_works: ["مقدمة ابن الصلاح في علوم الحديث", "فتاوى ابن الصلاح", "أدب المفتي والمستفتي"],
    died: "٦٤٣ هـ",
    region: "دمشق، الشام",
    madhhab: "شافعي",
    quote: "الحديث شعبة من شعب الوحي",
  },
  {
    id: "al-qarafi",
    name: "الإمام القرافي",
    fullName: "أبو العباس شهاب الدين أحمد بن إدريس القرافي",
    era: "العلماء الكبار",
    specialty: ["فقه", "أصول", "قواعد"],
    bio: "مجدد الفقه المالكي في القرن السابع، مصري مغربي الأصل، فقيه أصولي بارع قعّد القواعد وميّز بين المسائل في الذخيرة بأسلوب يجمع النقل والتحليل، ووضع الفروق التي تُعدّ من أنفس المصنفات",
    key_works: ["الفروق (4 مجلدات)", "الذخيرة في الفقه المالكي", "نفائس الأصول", "شرح تنقيح الفصول"],
    died: "٦٨٤ هـ",
    region: "القاهرة، مصر",
    madhhab: "مالكي",
    quote: "الفرق بين المسائل هو الفقه الحقيقي",
  },
  {
    id: "fakhr-razi",
    name: "الفخر الرازي",
    fullName: "أبو عبد الله محمد بن عمر فخر الدين الرازي",
    era: "العلماء الكبار",
    specialty: ["تفسير", "عقيدة", "أصول"],
    bio: "إمام المتكلمين والمفسرين في زمانه، موسوعي الثقافة جمع بين الفلسفة والكلام والتفسير، تفسيره مفاتيح الغيب يُعدّ أشمل التفاسير العقلية، استوطن هراة وانتشر تلاميذه في المشرق",
    key_works: ["مفاتيح الغيب (التفسير الكبير)", "المحصول في أصول الفقه", "المطالب العالية", "الأربعين في أصول الدين"],
    died: "٦٠٦ هـ",
    region: "الري، خراسان",
    madhhab: "شافعي",
    quote: "الجهل داء والعلم دواء",
  },
  {
    id: "al-mizzi",
    name: "الإمام المزي",
    fullName: "أبو الحجاج جمال الدين يوسف بن عبد الرحمن المزي",
    era: "العلماء الكبار",
    specialty: ["حديث", "رجال"],
    bio: "حافظ الشام وإمام الجرح والتعديل في القرن الثامن، جمع تهذيب الكمال في أسماء الرجال على مدى سنوات طويلة، وهو إتقان لا يُجارى في علم الرجال حتى اليوم، كان ابن تيمية يُثني عليه كثيراً",
    key_works: ["تهذيب الكمال في أسماء الرجال (35 مجلداً)", "تحفة الأشراف بمعرفة الأطراف"],
    died: "٧٤٢ هـ",
    region: "دمشق، الشام",
    madhhab: "شافعي",
    quote: "لا تثق بحديث لم تعرف رواته",
  },
  {
    id: "al-saadi",
    name: "الشيخ عبد الرحمن السعدي",
    fullName: "عبد الرحمن بن ناصر بن عبد الله السعدي",
    era: "المجددون",
    specialty: ["تفسير", "فقه", "عقيدة"],
    bio: "علامة القصيم وشيخ ابن عثيمين، عُرف بالوضوح والأسلوب السهل الميسّر في بيان المعاني القرآنية والأحكام الشرعية، تفسيره من أكثر تفاسير القرن العشرين انتشاراً بين عامة المسلمين",
    key_works: ["تيسير الكريم الرحمن في تفسير القرآن", "القواعد الحسان للتفسير", "المنهج السديد في شرح كتاب التوحيد", "بهجة قلوب الأبرار"],
    died: "١٣٧٦ هـ",
    region: "عُنيزة، القصيم، المملكة العربية السعودية",
    madhhab: "حنبلي",
    quote: "إذا أردت أن تعرف الرجل فانظر إلى عمله لا إلى قوله",
  },
  {
    id: "ibn-aqil",
    name: "ابن عقيل الحنبلي",
    fullName: "أبو الوفاء علي بن عقيل البغدادي",
    era: "العلماء الكبار",
    specialty: ["فقه", "أصول", "عقيدة"],
    bio: "فقيه الحنابلة وأصولييهم في عصر بغداد الزاهر، صاحب كتاب الفنون الذي يُقال إنه أضخم كتاب في الإسلام إذ تجاوز ثمانمائة مجلد، تميّز بالجرأة الفكرية والقدرة الجدلية الفائقة",
    key_works: ["الفنون", "الواضح في أصول الفقه", "الفصول"],
    died: "٥١٣ هـ",
    region: "بغداد، العراق",
    madhhab: "حنبلي",
    quote: "الزهد في العلم أشد من الزهد في الدنيا",
  },
  {
    id: "ahmad-shakir",
    name: "الشيخ أحمد محمد شاكر",
    fullName: "أحمد محمد شاكر العرابي",
    era: "المجددون",
    specialty: ["حديث", "تحقيق", "فقه"],
    bio: "محدث مصري من كبار علماء تحقيق الحديث في القرن العشرين، أمضى عمره في خدمة السنة النبوية، قاضٍ وعالم وناشر، حقق مسند الإمام أحمد وكثيراً من أمهات كتب السنة والتراث",
    key_works: ["تحقيق مسند الإمام أحمد", "تحقيق جامع الترمذي", "الباعث الحثيث شرح اختصار علوم الحديث", "عمدة التفسير"],
    died: "١٣٧٧ هـ",
    region: "القاهرة، مصر",
    madhhab: "اجتهادي",
    quote: "السنة هي الأساس بعد القرآن",
  },
  {
    id: "al-mundhiri",
    name: "الحافظ المنذري",
    fullName: "أبو محمد زكي الدين عبد العظيم المنذري",
    era: "العلماء الكبار",
    specialty: ["حديث", "فقه"],
    bio: "حافظ مصر وشيخ دار الحديث الكاملية، اشتُهر بمختصر صحيح مسلم وكتابه الترغيب والترهيب الذي يجمع الأحاديث الواردة في التشويق للطاعات والتخويف من المعاصي",
    key_works: ["الترغيب والترهيب", "مختصر صحيح مسلم", "الجواب المفيد في الرد على المعترضين"],
    died: "٦٥٦ هـ",
    region: "القاهرة، مصر",
    madhhab: "شافعي",
    quote: "رغّب في الخير تحبُّه النفوس، وخوِّف من الشر تتركه",
  },
  {
    id: "ibn-daqiq",
    name: "ابن دقيق العيد",
    fullName: "أبو الفتح تقي الدين محمد بن علي ابن دقيق العيد",
    era: "العلماء الكبار",
    specialty: ["حديث", "فقه", "أصول"],
    bio: "علّامة محدث فقيه جمع المذهبين المالكي والشافعي بلا تعصب، يُعدّ شرحه على الأربعين النووية من أجمع شروحها، اشتُهر بالتحقيق الدقيق والنظر العميق في مسائل الحديث والفقه",
    key_works: ["إحكام الأحكام شرح عمدة الأحكام", "شرح الأربعين النووية", "الإلمام بأحاديث الأحكام"],
    died: "٧٠٢ هـ",
    region: "القاهرة، مصر",
    madhhab: "شافعي/مالكي",
    quote: "التحقيق في الحديث منهج لا يُتهاون فيه",
  },
  {
    id: "ibn-rajab-2",
    name: "ابن رجب الحنبلي",
    fullName: "زين الدين عبد الرحمن بن أحمد بن رجب البغدادي الدمشقي",
    era: "العلماء الكبار",
    specialty: ["حديث", "فقه", "تربية"],
    bio: "علّامة حنبلي متبحّر في علوم الحديث والفقه والزهد والرقائق، اشتهر بعمق فهمه ودقة استنباطه وأسلوبه التربوي السلس، يُعدّ جامع العلوم والحكم من أنفس شروح الأربعين النووية",
    key_works: ["جامع العلوم والحكم", "شرح علل الترمذي", "فتح الباري شرح صحيح البخاري", "لطائف المعارف"],
    died: "٧٩٥ هـ",
    region: "دمشق، الشام",
    madhhab: "حنبلي",
    quote: "قيمة كل امرئ ما يُحسنه",
  },
  {
    id: "al-izz-ibn-abdes-salam",
    name: "العز بن عبد السلام",
    fullName: "أبو محمد عز الدين عبد العزيز بن عبد السلام السلمي الدمشقي",
    era: "العلماء الكبار",
    specialty: ["فقه", "أصول", "مقاصد"],
    bio: "سلطان العلماء وبائع الملوك، فقيه شافعي واسع الأفق في المقاصد والاجتهاد، وصفه ابن دقيق العيد بأنه أحد مجتهدي العصر، عُرف بجرأته في الحق وقوله للملوك ما لا يقوله غيره",
    key_works: ["قواعد الأحكام في مصالح الأنام", "الفوائد في اختصار المقاصد", "شجرة المعارف"],
    died: "٦٦٠ هـ",
    region: "دمشق ثم القاهرة",
    madhhab: "شافعي",
    quote: "لا يُباع الحق بشيء من الدنيا",
  },
  {
    id: "ibn-battuta",
    name: "ابن بطوطة",
    fullName: "أبو عبد الله محمد بن عبد الله اللواتي الطنجي ابن بطوطة",
    era: "العلماء الكبار",
    specialty: ["سيرة", "تاريخ"],
    bio: "رحّالة مسلم مغربي يُعدّ من أكثر المسافرين في التاريخ؛ طاف معظم العالم الإسلامي وسجّل رحلاته في كتاب تُهفة النظار، مما جعله مرجعاً تاريخياً نفيساً لأحوال البلاد والعباد في القرن الرابع عشر الميلادي",
    key_works: ["تحفة النظار في غرائب الأمصار وعجائب الأسفار (رحلة ابن بطوطة)"],
    died: "٧٧٠ هـ تقريباً",
    region: "طنجة، المغرب",
    madhhab: "مالكي",
    quote: "السفر يُعلّمك ما لا تعلّمه الكتب",
  },
  {
    id: "al-mawardi",
    name: "الماوردي",
    fullName: "أبو الحسن علي بن محمد بن حبيب البصري الماوردي",
    era: "العلماء الكبار",
    specialty: ["فقه", "أصول", "سياسة شرعية"],
    bio: "فقيه شافعي وسياسي إسلامي بارز؛ تمتّع بمكانة رفيعة في الدولة العباسية وكان قاضياً. يُعدّ كتابه الأحكام السلطانية من أوائل المؤلفات في الفقه الدستوري الإسلامي",
    key_works: ["الأحكام السلطانية والولايات الدينية", "أدب الدنيا والدين", "الحاوي الكبير"],
    died: "٤٥٠ هـ",
    region: "البصرة وبغداد، العراق",
    madhhab: "شافعي",
    quote: "السياسة العادلة من الدين لا خارج عنه",
  },
  {
    id: "al-bayhaqi",
    name: "البيهقي",
    fullName: "أبو بكر أحمد بن الحسين بن علي البيهقي الخراساني",
    era: "المحدثون",
    specialty: ["حديث", "فقه", "أصول"],
    bio: "محدث شافعي حافظ من كبار المحدثين؛ جمع بين علوم الحديث والفقه جمعاً نادراً، واشتُهر بموسوعاته الحديثية الضخمة التي تُعدّ من أوسع المؤلفات في علوم السنة",
    key_works: ["السنن الكبرى", "شعب الإيمان", "دلائل النبوة", "معرفة السنن والآثار"],
    died: "٤٥٨ هـ",
    region: "خراسان، إيران",
    madhhab: "شافعي",
    quote: "الجمع بين الحديث والفقه كمال العلم",
  },
  {
    id: "al-khatib-baghdadi",
    name: "الخطيب البغدادي",
    fullName: "أبو بكر أحمد بن علي بن ثابت الخطيب البغدادي",
    era: "المحدثون",
    specialty: ["حديث", "تاريخ", "رجال"],
    bio: "محدث ومؤرخ إسلامي بغدادي بارز؛ أُطلق عليه 'حافظ الشرق'، تاريخه 'تاريخ بغداد' في 14 جزءاً مرجع لا غنى عنه في تراجم العلماء ورجال الحديث، وكتابه 'الكفاية في علم الرواية' أسس لعلم مصطلح الحديث تأصيلاً منهجياً",
    key_works: ["تاريخ بغداد", "الكفاية في علم الرواية", "الجامع لأخلاق الراوي وآداب السامع"],
    died: "٤٦٣ هـ",
    region: "بغداد، العراق",
    madhhab: "شافعي",
    quote: "الرواية لا تُقبل إلا بعد معرفة أحوال الراوي",
  },
  {
    id: "al-ghazali-junior",
    name: "ابن عطاء الله السكندري",
    fullName: "أحمد بن محمد بن عبد الكريم ابن عطاء الله الشاذلي الإسكندري",
    era: "العلماء الكبار",
    specialty: ["تربية", "سلوك", "عقيدة"],
    bio: "عالم وشيخ صوفي من أعلام الطريقة الشاذلية؛ اشتُهر بالحكم العطائية التي تُعدّ من أرقى المؤلفات في أدب السلوك والتربية الروحية، جمع بين الفقه والزهد والمعرفة بالله",
    key_works: ["الحكم العطائية", "مفتاح الفلاح ومصباح الأرواح", "لطائف المنن"],
    died: "٧٠٩ هـ",
    region: "الإسكندرية ثم القاهرة، مصر",
    madhhab: "مالكي",
    quote: "لا تطلب من الله ما تريده واطلب منه ما يريده",
  },
  {
    id: "ibn-hazm-2",
    name: "ابن حزم الأندلسي",
    fullName: "أبو محمد علي بن أحمد بن سعيد بن حزم الأندلسي القرطبي",
    era: "العلماء الكبار",
    specialty: ["فقه", "حديث", "أصول", "أدب"],
    bio: "فقيه ظاهري أندلسي موسوعي الثقافة؛ كان شاعراً وأديباً وفيلسوفاً وناقداً للأديان، يُعدّ كتابه 'المحلى' من أضخم كتب الفقه الحديثي وأجرأها في الاستدلال بالدليل دون تعصب للمذاهب",
    key_works: ["المحلى بالآثار", "الإحكام في أصول الأحكام", "طوق الحمامة", "الفصل في الملل والأهواء والنحل"],
    died: "٤٥٦ هـ",
    region: "قرطبة، الأندلس",
    madhhab: "ظاهري",
    quote: "لا حجة إلا القرآن والسنة الصحيحة وما أجمع عليه العلماء",
  },
  {
    id: "al-nawawi",
    name: "النووي",
    fullName: "أبو زكريا يحيى بن شرف النووي الدمشقي",
    era: "العلماء الكبار",
    specialty: ["فقه", "حديث", "زهد"],
    bio: "فقيه شافعي محدث زاهد من أبرز علماء الإسلام؛ وصل إلى درجة الاجتهاد في المذهب الشافعي، عُرف بورعه الشديد وزهده رغم صغر سنه عند وفاته",
    key_works: ["شرح صحيح مسلم", "رياض الصالحين", "المجموع شرح المهذب", "روضة الطالبين", "الأذكار", "الأربعون النووية"],
    died: "٦٧٦ هـ",
    region: "نوى ودمشق، بلاد الشام",
    madhhab: "شافعي",
    quote: "من زهد في الدنيا أضاءت له الحكمة في قلبه وأجرى الله الحكمة على لسانه",
  },
  {
    id: "ibn-taymiyyah",
    name: "ابن تيمية",
    fullName: "أبو العباس أحمد بن عبد الحليم بن عبد السلام بن تيمية الحراني",
    era: "العلماء الكبار",
    specialty: ["عقيدة", "فقه", "أصول", "حديث"],
    bio: "فقيه ومجدد حنبلي من أعلام الإسلام؛ جمع بين العلم الشرعي والمنطق والرد على المخالفين. توفي في سجن القلعة بدمشق بسبب فتاواه الجريئة",
    key_works: ["درء تعارض العقل والنقل", "الفتاوى الكبرى", "منهاج السنة النبوية", "اقتضاء الصراط المستقيم"],
    died: "٧٢٨ هـ",
    region: "حران ودمشق، بلاد الشام",
    madhhab: "حنبلي",
    quote: "العلم بالدليل شرف والعمل به سعادة",
  },
  {
    id: "ibn-qayyim-2",
    name: "ابن القيم",
    fullName: "أبو عبد الله محمد بن أبي بكر ابن قيم الجوزية الدمشقي",
    era: "العلماء الكبار",
    specialty: ["فقه", "عقيدة", "تربية", "سلوك"],
    bio: "فقيه حنبلي وتلميذ ابن تيمية الأوفى الذي نشر علمه وهذّبه. تميّز بعمق التأمل في مسائل السلوك والقلب وغزارة الإنتاج العلمي",
    key_works: ["مدارج السالكين", "إعلام الموقعين", "زاد المعاد", "الداء والدواء", "طريق الهجرتين"],
    died: "٧٥١ هـ",
    region: "دمشق، بلاد الشام",
    madhhab: "حنبلي",
    quote: "القلب في سيره إلى الله كالطائر: المحبة رأسه، والخوف والرجاء جناحاه",
  },
  {
    id: "al-sakhawi",
    name: "السخاوي",
    fullName: "أبو الخير محمد بن عبد الرحمن بن محمد السخاوي",
    era: "العلماء الكبار",
    specialty: ["حديث", "تاريخ", "رجال"],
    bio: "محدث مؤرخ شافعي من أكابر الحفاظ في القرن التاسع الهجري؛ تلميذ الحافظ ابن حجر العسقلاني المباشر، اشتُهر بترجمة شيوخه وعصره في موسوعته الضخمة «الضوء اللامع» التي لا غنى عنها في معرفة علماء القرن التاسع",
    key_works: ["الضوء اللامع لأهل القرن التاسع", "فتح المغيث شرح ألفية الحديث", "المقاصد الحسنة في الأحاديث المشتهرة"],
    died: "٩٠٢ هـ",
    region: "القاهرة، ثم المدينة المنورة",
    madhhab: "شافعي",
    quote: "من لم يتقن التاريخ لم يتقن العلم",
  },
  {
    id: "ibn-kathir-2",
    name: "ابن كثير",
    fullName: "أبو الفداء إسماعيل بن عمر بن كثير القرشي الدمشقي",
    era: "العلماء الكبار",
    specialty: ["تفسير", "حديث", "فقه", "تاريخ"],
    bio: "مفسر محدث مؤرخ شافعي تلميذ ابن تيمية؛ تفسيره يُعدّ من أشهر التفاسير بالمأثور حتى اليوم، وكتابه «البداية والنهاية» من أهم موسوعات التاريخ الإسلامي، وله كتاب في السيرة النبوية من أوثق ما كُتب فيها",
    key_works: ["تفسير القرآن العظيم", "البداية والنهاية", "السيرة النبوية", "الباعث الحثيث"],
    died: "٧٧٤ هـ",
    region: "بصرى، ثم دمشق، بلاد الشام",
    madhhab: "شافعي",
    quote: "أصح التفسير: تفسير القرآن بالقرآن، ثم بالسنة، ثم بأقوال الصحابة",
  },
  {
    id: "al-tirmidhi",
    name: "الترمذي",
    fullName: "أبو عيسى محمد بن عيسى بن سَوْرة الترمذي",
    era: "المحدثون",
    specialty: ["حديث", "علل"],
    bio: "محدث من أصحاب الكتب الستة ومن تلاميذ البخاري؛ جامعه «السنن» يتميز بعلله وبيانه لأقوال الفقهاء على الأحاديث، وكتابه «العلل الكبير» من أدق الكتب في نقد الأحاديث، وقد أفقد بصره في آخر حياته",
    key_works: ["الجامع الترمذي (السنن)", "العلل الكبير", "الشمائل المحمدية"],
    died: "٢٧٩ هـ",
    region: "ترمذ، آسيا الوسطى",
    madhhab: "شافعي",
    quote: "كل حديث بيّنتُ علّته فلا يُعمَل به",
  },
  {
    id: "al-nasai",
    name: "النسائي",
    fullName: "أبو عبد الرحمن أحمد بن شعيب بن علي النسائي",
    era: "المحدثون",
    specialty: ["حديث", "رجال"],
    bio: "محدث من أصحاب الكتب الستة؛ اشتُهر بشدة نقده للرواة ودقته في الجرح والتعديل حتى قيل إنه لم يترك من الرجال إلا القليل. سننه من أصح كتب الحديث، وتوفي في فلسطين بعد أن استُشهد على ولايه الصالحة",
    key_works: ["السنن الصغرى (المجتبى)", "السنن الكبرى", "الضعفاء والمتروكون"],
    died: "٣٠٣ هـ",
    region: "نسا، خراسان (تركمانستان حالياً)، ووفاته بالرملة",
    madhhab: "شافعي",
    quote: "الجرح والتعديل أمانة والتساهل فيه خيانة",
  },
  {
    id: "ibn-juzayy-2",
    name: "ابن جزي",
    fullName: "أبو القاسم محمد بن أحمد بن محمد بن جزي الكلبي الغرناطي",
    era: "العلماء الكبار",
    specialty: ["فقه", "تفسير", "أصول"],
    bio: "فقيه مالكي أندلسي؛ اشتُهر بكتابه «القوانين الفقهية» الذي لخّص فيه الفقه المالكي بأسلوب مقارن مركّز، وفي التفسير له «التسهيل لعلوم التنزيل»، وهو من نظّم رحلة ابن بطوطة كتاباً",
    key_works: ["القوانين الفقهية", "التسهيل لعلوم التنزيل", "تقريب الوصول إلى علم الأصول"],
    died: "٧٤١ هـ",
    region: "غرناطة، الأندلس",
    madhhab: "مالكي",
    quote: "الفقه حياة الأحكام والأصول أُمهاتها",
  },
  {
    id: "ibn-al-jawzi",
    name: "ابن الجوزي",
    fullName: "أبو الفرج عبد الرحمن بن علي بن الجوزي",
    era: "العلماء الكبار",
    specialty: ["وعظ", "تفسير", "حديث", "فقه"],
    bio: "واعظ حنبلي وموسوعي القرن السادس الهجري؛ صنّف في أكثر من ثلاثمائة مصنّف بين التفسير والحديث والتاريخ والوعظ، وكتابه «تلبيس إبليس» و«صيد الخاطر» من أشهر مؤلفاته التي لا تزال تُقرأ",
    key_works: ["تلبيس إبليس", "صيد الخاطر", "المنتظم في التاريخ", "زاد المسير في علم التفسير"],
    died: "٥٩٧ هـ",
    region: "بغداد، العراق",
    madhhab: "حنبلي",
    quote: "إذا فاتتك الفرص فلا تفتك الأيام بالأسف على ما مضى",
  },
  {
    id: "al-haytami",
    name: "ابن حجر الهيتمي",
    fullName: "أبو العباس أحمد بن محمد بن علي بن حجر الهيتمي",
    era: "العلماء الكبار",
    specialty: ["فقه", "حديث", "فتوى"],
    bio: "فقيه شافعي مصري يُعدّ مرجعاً أساسياً للمذهب الشافعي المتأخر في المشرق والجزيرة العربية وجنوب شرق آسيا، وفتاواه الكبرى من أبرز موارد الفقه الشافعي",
    key_works: ["تحفة المحتاج بشرح المنهاج", "الفتاوى الكبرى الفقهية", "الزواجر عن اقتراف الكبائر", "الصواعق المحرقة"],
    died: "٩٧٤ هـ",
    region: "مصر ثم مكة المكرمة",
    madhhab: "شافعي",
    quote: "الفقيه الحق من يُفتي بما يُفضي إلى مصلحة الدارين",
  },
  {
    id: "al-shawkani",
    name: "الشوكاني",
    fullName: "أبو علي محمد بن علي بن محمد الشوكاني",
    era: "المجددون",
    specialty: ["حديث", "فقه", "أصول", "تفسير"],
    bio: "محدث فقيه يمني مجتهد؛ ناقش مسألة الاجتهاد والتقليد، وتفسيره «فتح القدير» جمع التفسير بالمأثور والرأي، وكتابه «نيل الأوطار» في شرح الأحاديث الفقهية من أوسع المصادر في موضوعه",
    key_works: ["نيل الأوطار شرح منتقى الأخبار", "فتح القدير", "إرشاد الفحول إلى تحقيق الحق من علم الأصول", "الدراري المضية"],
    died: "١٢٥٠ هـ",
    region: "صنعاء، اليمن",
    madhhab: "مجتهد",
    quote: "التقليد الأعمى يُضيّق العقل ويُسكت الدليل",
  },
  {
    id: "al-ajurri",
    name: "الآجري",
    fullName: "أبو بكر محمد بن الحسين الآجري البغدادي",
    era: "العلماء الكبار",
    specialty: ["عقيدة", "حديث", "أخلاق"],
    bio: "محدث حنبلي بغدادي سكن مكة المكرمة؛ اشتُهر بكتابه «الشريعة» في بيان عقيدة أهل السنة والرد على الفرق المخالفة، وكتابه «أخلاق العلماء» من أبرز ما كُتب في آداب أهل العلم",
    key_works: ["الشريعة", "أخلاق العلماء", "أخلاق حاملي القرآن", "الأربعون حديثاً"],
    died: "٣٦٠ هـ",
    region: "بغداد، ثم مكة المكرمة",
    madhhab: "حنبلي",
    quote: "العالم إذا لم يعمل بعلمه أفسد بجهله",
  },
  {
    id: "al-mizzi-2",
    name: "المزي",
    fullName: "أبو الحجاج يوسف بن الزكي عبد الرحمن المزي",
    era: "العلماء الكبار",
    specialty: ["رجال", "حديث"],
    bio: "حافظ دمشقي من أئمة علم الرجال في القرن الثامن الهجري؛ موسوعته «تهذيب الكمال في أسماء الرجال» من أضخم وأتقن كتب التراجم في رواة الكتب الستة، وتلميذه الذهبي هذّبها وزاد عليها",
    key_works: ["تهذيب الكمال في أسماء الرجال", "الأطراف", "تحفة الأشراف بمعرفة الأطراف"],
    died: "٧٤٢ هـ",
    region: "دمشق، بلاد الشام",
    madhhab: "شافعي",
    quote: "معرفة الرجال نصف العلم",
  },
  {
    id: "ibn-al-wazir",
    name: "ابن الوزير",
    fullName: "أبو عبدالله محمد بن إبراهيم بن علي بن المرتضى ابن الوزير اليماني",
    era: "العلماء الكبار",
    specialty: ["أصول", "حديث", "عقيدة"],
    bio: "عالم يماني زيدي تحوّل إلى الانتصار للحديث النبوي ومنهج المحدثين؛ كتابه «إيثار الحق على الخلق» دعوة صريحة لتقديم الدليل على التقليد، وكان سلفياً في منهجه الحديثي رغم أصله الزيدي، وأثّر تأثيراً بالغاً في الشوكاني",
    key_works: ["إيثار الحق على الخلق", "الروض الباسم في الذب عن سنة أبي القاسم", "العواصم والقواصم"],
    died: "٨٤٠ هـ",
    region: "اليمن",
    madhhab: "مجتهد",
    quote: "الحق أحق أن يُتبع من الرجال",
  },
  {
    id: "al-mubarakfuri-2",
    name: "المباركفوري",
    fullName: "أبو العلا محمد عبد الرحمن بن عبد الرحيم المباركفوري",
    era: "المجددون",
    specialty: ["حديث", "شروح"],
    bio: "محدث هندي من أبرز علماء الحديث في شبه القارة الهندية في العصر الحديث؛ اشتُهر بشرحه الضخم على جامع الترمذي «تحفة الأحوذي» الذي أصبح مرجعاً أساسياً في شرح سنن الترمذي وبيان درجات أحاديثه",
    key_works: ["تحفة الأحوذي شرح جامع الترمذي", "مختصر تحفة الأحوذي", "سيرة النبي ﷺ"],
    died: "١٣٥٣ هـ",
    region: "مباركفور، الهند",
    madhhab: "أهل الحديث",
    quote: "خير ما شُغل به العمر تقييد السنة وخدمة الحديث",
  },
  {
    id: "ibn-al-mubarak-senior",
    name: "ابن المبارك",
    fullName: "أبو عبد الرحمن عبد الله بن المبارك المروزي",
    era: "المحدثون",
    specialty: ["حديث", "زهد", "فقه"],
    bio: "إمام جليل من أئمة القرن الثاني الهجري؛ جمع بين العلم والزهد والجهاد في سبيل الله، وكان من أوسع الناس رواية في عصره مع تمييزه الفائق للرجال. قال فيه الإمام أحمد: «ما رأيت رجلاً أجمع لكل خير من ابن المبارك»",
    key_works: ["الزهد والرقائق", "الجهاد", "المسند"],
    died: "١٨١ هـ",
    region: "مرو، خراسان",
    madhhab: "مجتهد",
    quote: "ربّ عمل صغير تُعظّمه النية، وربّ عمل كبير تُصغّره النية",
  },
  {
    id: "ibn-abi-shaybah",
    name: "ابن أبي شيبة",
    fullName: "أبو بكر عبد الله بن محمد بن إبراهيم ابن أبي شيبة العبسي الكوفي",
    era: "المحدثون",
    specialty: ["حديث", "فقه"],
    bio: "حافظ كوفي من أئمة الحديث في الطبقة الوسطى؛ موسوعته «المصنف» من أجمع كتب الآثار التي تضم الأحاديث المرفوعة والموقوفة والمقطوعة في أبواب فقهية، وهو شيخ البخاري ومسلم وأبي داود",
    key_works: ["المصنف في الأحاديث والآثار", "المسند", "الأدب"],
    died: "٢٣٥ هـ",
    region: "الكوفة، العراق",
    madhhab: "مجتهد",
    quote: "من طلب العلم لله كفاه الله مؤونة الدنيا",
  },
  {
    id: "al-qurtubi-scholar",
    name: "القرطبي",
    fullName: "أبو عبد الله محمد بن أحمد بن أبي بكر الأنصاري القرطبي",
    era: "العلماء الكبار",
    specialty: ["تفسير", "فقه", "أصول"],
    bio: "مفسر أندلسي مالكي من أئمة القرن السابع؛ تفسيره «الجامع لأحكام القرآن» يعدّ من أنفع التفاسير في استنباط الأحكام الفقهية، وانتقل من الأندلس إلى مصر وتوفي في المنيا. تميّز بغزارة الاستدلال ودقة الترجيح بين المذاهب",
    key_works: ["الجامع لأحكام القرآن (تفسير القرطبي)", "التذكرة بأحوال الموتى وأمور الآخرة", "الأسنى في شرح أسماء الله الحسنى"],
    died: "٦٧١ هـ",
    region: "قرطبة، الأندلس — ثم مصر",
    madhhab: "مالكي",
    quote: "كتاب الله تبارك وتعالى لا تنقضي عجائبه ولا تتناهى كنوزه",
  },
  {
    id: "ibn-rajab-al-hanbali",
    name: "ابن رجب الحنبلي",
    fullName: "زين الدين عبد الرحمن بن أحمد بن رجب الحنبلي",
    era: "العلماء الكبار",
    specialty: ["حديث", "فقه", "رقائق"],
    bio: "محدث حنبلي من القرن الثامن الهجري؛ جمع بين العلم والعبادة والزهد والتحقيق. شرحه على صحيح البخاري «فتح الباري» (مختلف عن شرح ابن حجر) من أنفس الشروح الحديثية، وكتابه «جامع العلوم والحكم» شرح لخمسين حديثاً نبوياً يعدّ مرجعاً أصيلاً",
    key_works: ["جامع العلوم والحكم", "فتح الباري شرح البخاري", "لطائف المعارف", "القواعد الفقهية"],
    died: "٧٩٥ هـ",
    region: "بغداد، ثم دمشق",
    madhhab: "حنبلي",
    quote: "العلم الحقيقي هو الذي يدعو صاحبه إلى الخشية من الله",
  },
  {
    id: "al-nawawi-imam",
    name: "النووي",
    fullName: "أبو زكريا يحيى بن شرف النووي",
    era: "العلماء الكبار",
    specialty: ["فقه", "حديث", "أصول"],
    bio: "إمام شافعي من أئمة القرن السابع الهجري؛ أحد أعظم محققي المذهب الشافعي وعلماء الحديث. عاش قصيراً (٤٥ سنة) لكنه أنجز تراثاً علمياً ضخماً يُدرَّس حتى اليوم في كل بقاع العالم الإسلامي. مرجع في الفقه والحديث وعلوم الأخلاق",
    key_works: ["المجموع شرح المهذب", "رياض الصالحين", "الأذكار", "شرح صحيح مسلم", "الأربعون النووية"],
    died: "٦٧٦ هـ",
    region: "نوى، الشام",
    madhhab: "شافعي",
    quote: "أعلم الناس بالله أخشاهم له",
  },
  {
    id: "al-bukhari-imam",
    name: "الإمام البخاري",
    fullName: "أبو عبد الله محمد بن إسماعيل البخاري",
    era: "المحدثون",
    specialty: ["حديث", "رجال"],
    bio: "أمير المؤمنين في الحديث؛ صاحب الجامع الصحيح الذي أجمع العلماء على أنه أصح كتاب بعد كتاب الله. رحل في طلب الحديث إلى بلاد شتى منذ صغره، وبلغ عدد شيوخه ألفاً وثمانين شيخاً. انتقد بشدة مَن خالفه في مسألة لفظه بالقرآن وابتُلي بالمحنة في آخر حياته",
    key_works: ["الجامع الصحيح (صحيح البخاري)", "التاريخ الكبير", "التاريخ الأوسط", "الأدب المفرد"],
    died: "٢٥٦ هـ",
    region: "بخارى، خراسان",
    madhhab: "مجتهد",
    quote: "ما وضعت في الصحيح حديثاً إلا اغتسلت وصليت ركعتين",
  },
  {
    id: "ibn-hajar-asqalani",
    name: "ابن حجر العسقلاني",
    fullName: "أبو الفضل أحمد بن علي بن محمد بن حجر العسقلاني",
    era: "العلماء الكبار",
    specialty: ["حديث", "رجال", "تفسير"],
    bio: "حافظ العصر وشيخ الإسلام في علم الحديث؛ صاحب فتح الباري شرح صحيح البخاري الذي يعدّ أعظم شروح البخاري وأجلّها على الإطلاق. ولي قضاء القضاة في مصر. تجاوزت مصنفاته مائة وخمسين كتاباً في الحديث والرجال والتاريخ والتفسير",
    key_works: ["فتح الباري شرح صحيح البخاري", "الإصابة في تمييز الصحابة", "تهذيب التهذيب", "لسان الميزان", "بلوغ المرام"],
    died: "٨٥٢ هـ",
    region: "عسقلان، ثم القاهرة",
    madhhab: "شافعي",
    quote: "كل ما في الوجود فمن النبي ﷺ أو بسببه أو بدعوته",
  },
  {
    id: "ibn-kathir-imam",
    name: "ابن كثير",
    fullName: "أبو الفداء إسماعيل بن عمر بن كثير القرشي الدمشقي",
    era: "العلماء الكبار",
    specialty: ["تفسير", "حديث", "تاريخ"],
    bio: "مفسر محدث مؤرخ شافعي من القرن الثامن الهجري؛ تلميذ ابن تيمية وابن القيم. تفسيره «تفسير القرآن العظيم» من أشهر التفاسير بالمأثور وأوسع انتشاراً. وكتابه «البداية والنهاية» موسوعة تاريخية لا غنى عنها",
    key_works: ["تفسير القرآن العظيم (تفسير ابن كثير)", "البداية والنهاية", "الفصول في السيرة النبوية", "اختصار علوم الحديث"],
    died: "٧٧٤ هـ",
    region: "بصرى، الشام — ثم دمشق",
    madhhab: "شافعي",
    quote: "المؤمن يرى ذنبه كالجبل فوق رأسه يخاف أن يقع عليه",
  },
  {
    id: "al-mawardi-2",
    name: "الماوردي",
    fullName: "أبو الحسن علي بن محمد بن حبيب الماوردي",
    era: "العلماء الكبار",
    specialty: ["فقه", "أصول", "سياسة شرعية"],
    bio: "فقيه شافعي وسياسي إسلامي من القرن الخامس الهجري؛ يُعدّ من أبرز علماء الفقه السياسي في الإسلام. كتابه «الأحكام السلطانية» مرجع كلاسيكي في نظام الحكم الإسلامي. كان قاضي القضاة في عهد الخليفة العباسي القادر",
    key_works: ["الأحكام السلطانية والولايات الدينية", "الحاوي الكبير في الفقه", "أدب الدنيا والدين"],
    died: "٤٥٠ هـ",
    region: "البصرة، ثم بغداد",
    madhhab: "شافعي",
    quote: "الدين أصل الدولة وحارسها، والدولة حارسة الدين ومؤيدتها",
  },
  {
    id: "al-suyuti-2",
    name: "السيوطي",
    fullName: "جلال الدين عبد الرحمن بن أبي بكر السيوطي",
    era: "العلماء الكبار",
    specialty: ["تفسير", "حديث", "أصول", "لغة"],
    bio: "موسوعي مصري من القرن التاسع الهجري؛ ادّعى بلوغ درجة الاجتهاد المطلق. صنّف ما يزيد على خمسمائة مؤلف في علوم القرآن والحديث والفقه واللغة والتاريخ. كتابه «الإتقان في علوم القرآن» مرجع لا غنى عنه لكل باحث في علوم القرآن",
    key_works: ["الإتقان في علوم القرآن", "الجامع الصغير", "تدريب الراوي", "المزهر في اللغة", "الدر المنثور"],
    died: "٩١١ هـ",
    region: "القاهرة، مصر",
    madhhab: "شافعي",
    quote: "تلقّينا علومنا عن الكتب والمشايخ — والحمد لله على نعمة العلم",
  },
];

export const SCHOLAR_ERAS = ["الكل", "الأئمة الأربعة", "المحدثون", "العلماء الكبار", "المجددون", "المعاصرون"];
export const SCHOLAR_SPECIALTIES = ["الكل", "فقه", "حديث", "عقيدة", "تفسير", "أصول", "مقاصد", "لغة", "سيرة", "رجال"];

export function findScholarById(id: string): Scholar | undefined {
  return SCHOLARS.find(s => s.id === id);
}
