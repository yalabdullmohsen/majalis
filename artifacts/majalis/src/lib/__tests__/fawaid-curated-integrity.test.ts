 ...
 *
 * فالبوابةُ تُقابلُ **كلَّ لفظٍ بين «»** في سجلٍّ مصدرُه أحدُ الصحيحَينِ
 * بنصِّ النسختَينِ — إلّا ما صرَّحَ السجلُّ نفسُه بضعفِه أو وضعِه (فذِكرُه
 * للتحذيرِ منه لا للاحتجاجِ به).
 *
 * ⚠️ **قصٌّ مقصودٌ في التغطيةِ يُصرَّحُ به ولا يُسكَتُ عنه**: البوابةُ على
 * `FAWAID_CURATED_SEED` وحدَه. وتشغيلُها على الشقيقِ `fawaid-seed.ts` يُخرِج
 * **٢٠ سجلًّا** (`seed-fawaid-` ٩، ٣٤١، ٣٤٤، ٣٤٦، ٣٤٨، ٣٥٣، ٣٦٤، ٣٧٨، ٣٨٧،
 * ٣٩٥، ٣٩٨، ٤٠٧، ٤٦٤، ٤٧٣، ٤٧٩، ٤٨٧، ٤٩٥، ٥٠٦، ٥١٠، ٥٣٢) — وعيّنتُها
 * **ليست عزوًا كاذبًا** بل نقلٌ بغيرِ ضبطِه الحرفيّ («من سرّه أن يُبسط له في
 * رزقه **ويُنسأ** له في أثره» ولفظُ البخاريِّ ٥٩٨٥ «**وأن يُنسأ**»)، أو «»
 * مستعملةٌ لإبرازِ مصطلحٍ لا لنقلِ لفظٍ («قاعدة «الأمور بمقاصدها»»). وتصحيحُ
 * كلِّ لفظٍ منها يقتضي نقلًا حرفيًّا سجلًّا سجلًّا ⇒ دفعةٌ مستقلّةٌ لاحقة،
 * ولم تُضَمَّ هنا حتى لا تُغلَق البوابةُ على عملٍ لم يُقابَل بعدُ.
 */
...
const unmatchedQuote = FAWAID_CURATED_SEED.filter((f) => {
  ...
  const quotes = [...f.text.matchAll(/«([^»]{12,})»/g)].map((m) => normHadith(m[1]));
  return quotes.some((q) => !corpus.some((c) => c.includes(q)));
});
assert(
