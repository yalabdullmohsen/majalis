...
assert.deepEqual(
  ...
  ["القرآن", "الدروس", "الرئيسية", "الصلاة", "الأقسام"],
);
...
const home =
  read("src/pages/account/ui/HomeView.tsx") +
  read("src/pages/account/ui/HomeBelowFold.tsx") +
  read("src/components/home/HomePrimaryPortals.tsx");
...
for (const [from, to] of Object.entries(IA_REDIRECTS)) {
  ...
  if (from === "/more") {
    assert.match(app, /path="\/more"[^>]*>\s*<Redirect\s+to="\/sections"/);
    continue;
  }
...
for (const source of Object.keys(IA_REDIRECTS)) {
  ...
  if (source === "/more") {
    assert.match(vercel, /"source"\s*:\s*"\/more"[\s\S]{0,120}"destination"\s*:\s*"\/sections"/);
    continue;
  }
  const re = new RegExp(
