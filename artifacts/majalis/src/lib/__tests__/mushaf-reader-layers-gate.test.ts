...
assert.match(controls, /enterKeyHint=["']search["']/);
...
assert.doesNotMatch(controls, /inputMode=["']numeric["']/, "لوحة الأرقام تخفي زر البحث على iOS");
...
assert.match(search, /enterKeyHint=["']search["']/, "زر بحث على لوحة المفاتيح");
assert.match(search, /inputMode=["']search["']/);
assert.doesNotMatch(search, /inputMode=\{[^}]*numeric/, "لا numeric في بحث المصحف — يخفي زر البحث");
assert.doesNotMatch(search, /enterKeyHint=\{[^}]*go/, "بحث وليس go للأرقام والكلمات");
