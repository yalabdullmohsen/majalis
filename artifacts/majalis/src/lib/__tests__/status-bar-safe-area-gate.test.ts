...
assert.match(cap, /StatusBar:\s*\{[^}]*backgroundColor:\s*"#F7F3EB"/s);
assert.doesNotMatch(cap, /StatusBar:\s*\{[^}]*backgroundColor:\s*"#0E1A15"/s);
assert.match(cap, /ios:\s*\{[\s\S]*?backgroundColor:\s*"#F7F3EB"/);

const utils = read("src/lib/capacitor-utils.ts");
assert.match(utils, /STATUS_BAR_BG_LIGHT\s*=\s*"#F7F3EB"/);
