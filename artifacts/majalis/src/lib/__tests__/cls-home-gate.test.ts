/**
 * بوابة CLS — حجز ارتفاعات شاشة البداية الجديدة.
...
assert.match(critical, /\.mj-home-start__brand[\s\S]*min-height:\s*7\.5rem/, "حجز الشعار");
assert.match(critical, /\.mj-prayer-summary[\s\S]*min-height:\s*11\.5rem/, "حجز مواقيت الصلاة");
assert.match(critical, /\.mj-dhikr-summary[\s\S]*min-height:\s*8\.5rem/, "حجز الأذكار");
assert.match(critical, /\.hus-field[\s\S]*min-height:\s*52px/, "حجز شريط البحث");
assert.match(critical, /\.mj-home-featured[\s\S]*min-height:\s*9\.5rem/, "حجز الأقسام");
