...
console.log("\n=== TopSectionBar — مساحات موحّدة ===");
{
  ...
  assert(hrefs.includes("/mushaf") && labels.includes("قرآن"), "مساحة قرآن تفتح المصحف");
  ...
  assert(!hrefs.includes("/quran-knowledge"), "علوم القرآن ليست تبويبًا سفليًا منفصلًا");
}

console.log("\n=== isTabActive ===");
{
  assert(isTabActive("/mushaf", "/mushaf") === true, "المصحف يفعّل تبويب قرآن");
  ...
  assert(isTabActive("/library/book-1", "/mushaf") === false, "مسار كتاب لا يفعّل قرآن");
}
...
console.log("\n=== nav-visibility تنظيف ===");
{
  ...
  assert(!isComingSoonPath("/mushaf"), "المصحف مفتوح");
