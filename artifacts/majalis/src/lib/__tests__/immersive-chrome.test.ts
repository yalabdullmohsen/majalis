...
assert.equal(isPinnedChromePath("/prophets"), true, "كروم قائمة الأنبياء مثبَّت بلا إخفاء transform");
assert.equal(isPinnedChromePath("/prophets/musa"), false, "تفاصيل النبي قراءة مركّزة — ليست pinned");
assert.equal(isImmersiveChromePath("/prophets/musa"), true, "تفاصيل النبي غمرية لإخفاء Header/BottomNav");
