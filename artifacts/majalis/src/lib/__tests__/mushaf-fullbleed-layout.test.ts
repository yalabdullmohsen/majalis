...
assert.match(
  quranCss,
  /\.mpv-body\.mpv-body--ayah\s*\{[\s\S]*?padding-top:\s*max\(\s*11\.8vh\s*,\s*var\(--inset-top/,
  "بداية كتلة النص عند ١١٫٨٪",
);
assert.match(
  quranCss,
  /\.mpv-body\.mpv-body--ayah\s*\{[\s\S]*?padding-bottom:\s*max\(\s*7\.5vh\s*,\s*var\(--inset-bottom/,
