...
console.log("\n=== فقه — صفحة المحور من books.json ===");
{
  ...
  assert(pageSrc.includes("FIQH_SUPPORTING_TOPICS"), "صفحة الفقه تعرض المباحث المساندة");
  assert(pageSrc.includes("/fiqh/usul"), "مدخل أصول الفقه ظاهر في بوابة الفقه");
  assert(pageSrc.includes("المباحث المساندة"), "عنوان المباحث المساندة");
