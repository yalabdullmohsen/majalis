...
assert.match(css, /\.ayah-sheet\s*\{[^}]*background:\s*var\(--surface/);
assert.match(css, /font-size:\s*17px/);
assert.match(css, /line-height:\s*1\.9/);
assert.doesNotMatch(actions, /المزيد/);
assert.doesNotMatch(actions, /previewLines|slice\(0,\s*2\)/);
assert.doesNotMatch(actions, /تجويد/);
assert.match(actions, /role="tablist"/);
assert.match(actions, /تفسير/);
assert.match(actions, /تلاوة/);
assert.match(actions, /معاني الكلمات/);
assert.match(actions, /ayahMeaningsTab|QURAN_DATA_FEATURES/);
assert.match(actions, /font-size: 22px|ayah-action-sheet__ayah-preview/);
assert.match(css, /font-size:\s*22px/);
assert.match(actions, /ayah-sheet__scrim/);
assert.match(actions, /className="mm-ayah-bar__panel ayah-sheet"/);
