...
assert.match(css, /\.mm-ayah-bar__dismiss\s*\{[^}]*background:\s*rgba\(0,\s*0,\s*0,\s*0\.(14|28)\)/);
...
assert.match(actions, /useState<SheetHeight>\("collapsed"\)/);
assert.match(actions, /data-sheet-mode=\{expanded \? "details" : "quick"\}/);
assert.match(actions, /quran-quick-bar/);
...
assert.match(actions, /وضع الحفظ|mushaf-hifz-toggle/);
assert.match(actions, /ayah-action-sheet__player|mushaf-compact-player/);
assert.match(actions, /isAiTarteelEnabled|تسميع بالذكاء الاصطناعي|تلاوة بالذكاء/);
assert.match(actions, /recitation-test-ai\?surah=.*ayah=/);
