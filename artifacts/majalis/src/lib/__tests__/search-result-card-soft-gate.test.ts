...
const cards = readFileSync(resolve(root, "src/components/search/SearchResultCards.tsx"), "utf8");
const css = readFileSync(resolve(root, "src/styles/pages/search.css"), "utf8");

assert.match(
  view + "\n" + cards,
  /srch-result-card soft-card soft-card--on-light/,
  "نتيجة البحث تستخدم soft-card على سطح فاتح",
);
