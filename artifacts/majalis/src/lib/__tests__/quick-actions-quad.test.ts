...
import { pluralAr, pluralArBucket, NOUN_DURUS, NOUN_HALAQAT } from "@/lib/arabic-count";
...
assert.match(cmp, /pluralArBucket/);
...
assert.match(view, /activeLessons\.length/);
assert.match(view, /archivedLessons\.length/);
...
assert.equal(lessons.quad?.[1]?.count, QURAN_CIRCLES_SEED.length);
assert.equal(lessons.quad?.[3]?.count, ISLAMIC_OCCASIONS.length);
