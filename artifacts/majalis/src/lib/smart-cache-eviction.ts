    ...
    if (typeof caches !== "undefined") {
      ...
      for (const name of names) {
        ...
        entries.push({
          ...
          protected: name === "majalis-version" || name === "ssunnah-version" || name === "majlisilm-version",
        });
      }
    }
  } catch {
  ...
  const candidates = report.entries
    ...
    .filter((e) => {
      ...
      if (e.layer === "cacheStorage") {
        ...
        return (
          (/majalis-(offline|data)-/.test(e.key) ||
            /^ssunnah-v.+-(offline|data)$/.test(e.key) ||
            /^majlisilm-v.+-(offline|data)$/.test(e.key)) &&
          e.key !== "majalis-version" &&
          e.key !== "ssunnah-version" &&
    ...
    try {
      if (entry.layer === "localStorage") {
        ...
        const currentish = names
          .filter(
            (n) =>
              n.startsWith("ssunnah-v") ||
              n.startsWith("majlisilm-v") ||
              n.startsWith("majalis-offline-") ||
              n.startsWith("majalis-data-"),
          )
          .sort();
        const newest = currentish[currentish.length - 1];
        if (
          entry.key !== newest &&
          entry.key !== "majalis-version" &&
          entry.key !== "ssunnah-version" &&
          entry.key !== "majlisilm-version"
        ) {
