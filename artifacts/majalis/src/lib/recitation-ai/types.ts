 ...
 * إعادة")، تتوقف الجلسة فعليًا (المايكروفون يُغلَق) وتُعرض بطاقة تصحيح حي
 * إلزامية حتى يضغط المستخدم "أعد من هذه الكلمة"، فتُستأنَف الجلسة من نفس
 * موضع الخطأ تحديدًا (لا من حيث توقفت فقط) — تحقّق حقيقي، لا مجرد إقرار.
 * مختلف تمامًا عن RecitationMode "teacher_test" (ذاك يبدأ من موضع عشوائي
 * لقياس الاسترجاع، بلا أي علاقة بالتوقف عند الخطأ).
 */
export type AlertLevel = "gentle" | "medium" | "immediate" | "teacher";
...
export type ReferenceWord = {
  ...
  /** رقم صفحة مصحف المدينة (1-604) — من حقل Ayah.page الحقيقي في البيانات المحلية؛ يُستخدَم للانتقال التلقائي بين صفحات المصحف في النطاقات متعددة الصفحات (جزء/حزب/ربع). */
  page: number;
};
