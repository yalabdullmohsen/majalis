...
console.log("═══ إضافي — بسملة قبل سورة قصيرة (كلمات تكفي حسم البسملة تُغذَّى واحدة تلو الأخرى) ═══");
{
  // خلل حقيقي مُكتشَف عبر تحقّق حي لوضع "التسميع الحر": حين تُغذَّى كلمات
  // البسملة الأربع للمحرك **واحدة تلو الأخرى** (بث لحظي حقيقي، لا دفعة
  // واحدة)، كانت حلقة الحسم الرئيسية تُحاذي أول كلمتين معلَّقتين ("بسم"،
  // "الله") ضد أول كلمتين مرجعيتين ("قل"، "هو") فور بلوغ COMMIT_LAG، قبل
  // أن تصل كلمة البسملة الرابعة أصلاً — فيُصنَّف "قل" ناقصًا و"هو" خطأً
  // (سُمع "بسم") زورًا، رغم أن المستخدم لم يُخطئ إطلاقًا. الإصلاح: حجب
  // الحسم عند cursor=0 (لسور غير الفاتحة/التوبة) حتى تصل 4 كلمات كاملة
  // تكفي فحص البسملة أولاً.
  const ref = refWordsFor(112); // الإخلاص — أول آية 4 كلمات بالضبط بعد فصل البسملة
  const heard = ["بسم", "الله", "الرحمن", "الرحيم", ...ref.map((w) => w.normalized)];

  // ⚠️ تحقّق **منتصف الجلسة** (بلا finalize) — حاسم لإثبات الإصلاح فعليًا:
  // finalize() (المُستخدَم في runSession أدناه) يُحاذي كل ما تبقى في الـ
  // buffer دفعة واحدة (finalFlush=true)، فيُخفي هذا الخلل تحديدًا (يُحسَم
  // "قل"/"هو" بشكل صحيح فور نهاية الجلسة بصرف النظر عن الخلل أثناءها) —
  // هذا بالضبط سبب عدم اكتشاف الاختبارات الآلية للخلل أول مرة؛ التحقّق
  // الحي عبر Playwright (لا ينهي الجلسة فورًا) هو ما كشفه فعليًا.
  const engineMidStream = new VerseAlignmentEngine({ referenceWords: ref });
  const midEvents: AlignmentEvent[] = [];
  let tMid = 0;
  for (const w of heard.slice(0, 6)) { // بسم/الله/الرحمن/الرحيم/قل/هو فقط — بلا finalize
    tMid += 300;
    midEvents.push(...engineMidStream.feedWord(w, tMid));
  }
  const cMid = counts(midEvents);
  assert(cMid.correct === 2, `منتصف الجلسة (بلا finalize): "قل" و"هو" مُحتسَبتان صحيحتين فورًا (${JSON.stringify(cMid)})`);
  assert(cMid.missing_word === 0 && cMid.wrong_word === 0, `منتصف الجلسة: لا "قل ناقصة" ولا "هو خطأ" زورًا حتى قبل finalize (${JSON.stringify(cMid)})`);

  const events = runSession(ref, heard, 300);
  const c = counts(events);
  assert(c.correct === ref.length, `كل كلمات الإخلاص (${ref.length}) صحيحة رغم تغذية البسملة قبلها كلمة كلمة (${JSON.stringify(c)})`);
  assert(c.missing_word === 0 && c.wrong_word === 0, `لا "قل ناقصة" ولا "هو خطأ (سُمع بسم)" زورًا — البسملة تُتجاوَز بصمت كما يجب (${JSON.stringify(c)})`);
}
