...
export async function fetchAboutPlatformStats(): Promise<AboutPlatformStats> {
  ...
  try {
    const [
      ...
      supabase.from("lessons").select("id", { count: "exact", head: true }).eq("status", "approved").or("category.ilike.%قرآن%,category.ilike.%قران%,title.ilike.%حلقة%"),
      supabase.from("library_items").select("id", { count: "exact", head: true }).eq("status", "approved").or("type.eq.متن,title.ilike.%متن%,title.ilike.%منظوم%,title.ilike.%أرجوز%"),
    ]);
