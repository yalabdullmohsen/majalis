...
const SOURCES = [
  ...
  { name: "adhan-js (محلي)", desc: "حساب مواقيت الصلاة أوفلاين داخل التطبيق؛ الطريقة الافتراضية للكويت: وزارة الأوقاف (Kuwait)، مع إبقاء اختيار الطريقة للمستخدم." },
...
const STEPS = [
  ...
  {
    Icon: CheckCircle2,
    title: "التوثيق بالمصدر",
    desc: "تُوسم المادة «محتوى موثّق» عند وجود مصدر أو مرجع معروف (كتاب، حديث، فتوى، نقل) — بلا اشتراط مراجِع بشري مُسمّى.",
  },
  {
    Icon: Users,
    title: "مراجعة اختيارية إضافية",
    desc: "قد تُسجَّل مراجعة بشرية إضافية عند توفرها، لكنها ليست شرطًا لعرض المحتوى الموثّق.",
  },
  {
    Icon: Shield,
    title: "الشارة تتبع البيانات",
    desc: "شارة «محتوى موثّق» تظهر تلقائيًا عند وجود مصدر خارجي أو درجة أو مرجع. المحتوى بلا مصدر يُعرض بياناته دون وسم «قيد المراجعة».",
  },
  {
    Icon: Bot,
...
const LIMITS = [
  ...
  "مواقيت الصلاة نتيجة حساب فلكي وفق الطريقة المختارة (محليًا عبر adhan-js) وليست فتوى؛ وتنبيهات الأذان تخضع لقيود نظام التشغيل.",
  "نُخطئ ونُصحِّح: زر «الإبلاغ عن خطأ» أسفل كل مادة، وما يثبت خطؤه يُصحَّح أو يُسحب.",
];

/** درجات التوثيق الخمس — مزامنة مع docs/methodology-page-content.md */
const TRUST_GRADES = [
  {
    name: "نص أصلي",
    code: "primary_text",
  ...
  return (
    ...
    <div className="mtd-page ds-section" dir="rtl">
      ...
      <section className="mtd-section">
        ...
        <p className="mtd-section__body">
          في عصر الذكاء الاصطناعي وتدفق المعلومات، يصعب التمييز بين المحتوى الموثّق والمحتوى المولَّد
          آلياً. فالقاعدة عندنا:{" "}
          <strong>
            تُوسم المادة «محتوى موثّق» عند وجود مصدر أو مرجع معروف
          </strong>
          . والمحتوى المولَّد آلياً يُوسم صراحةً ولا يُوثَّق تلقائياً.
        </p>
        <p className="mtd-section__body">
          نعرض المصدر والدرجة حيث وُجدتا، ولا نُخفي المحتوى خلف وسم «قيد المراجعة». الشارة تتبع
          البيانات لا النية.
        </p>
      </section>
      ...
      <section className="mtd-section mtd-section--highlight">
        ...
        <p className="mtd-section__body">
          واقع التوثيق يتغيّر مع دفعات المراجعة الأسبوعية: كثير من أسئلة الاختبار بلا مرجع
          منفصل، وجزء من المسائل الفقهية استدلال عام موسوم كذلك دون إخفاء المسألة من العرض.
          الجرد التفصيلي للأرقام في مستودع المشروع (`documentation-inventory`) وطابور
          المراجعة (`citation-backlog`). للإبلاغ عن خطأ:{" "}
      ...
      <section className="mtd-section">
        <h2 className="mtd-section__title">مواقيت الصلاة والتنبيهات</h2>
        <ul className="mtd-guarantees">
          <li>
            تُحسب المواقيت محليًا أوفلاين عبر مكتبة adhan-js؛ الطريقة الافتراضية للكويت
            توافق معاملات وزارة الأوقاف، مع إبقاء اختيار طريقة الحساب للمستخدم.
          </li>
          <li>المواقيت نتيجة حساب فلكي وفق الإعدادات — ليست فتوى ولا حكمًا شرعيًا مستقلًا.</li>
          <li>
            تنبيهات الأذان تُجدول عبر إشعارات نظام التشغيل؛ التسليم النهائي يخضع لإذن
            الإشعارات وإعدادات الصوت والتركيز وملخص الإشعارات على الجهاز.
          </li>
          <li>لا ندّعي تجاوز الوضع الصامت أو Focus بلا صلاحية رسمية من المنصة.</li>
        </ul>
      </section>
      ...
      <section className="mtd-section mtd-section--highlight">
        ...
        <ul className="mtd-guarantees">
          ...
          <li>نمنح شارة «محتوى موثّق» عند وجود مصدر أو مرجع معروف — بلا اشتراط مراجع بشري مُسمّى</li>
        </ul>
      </section>
