  ...
  return (
    <div className="isp-detail">
      ...
      <div className="isp-detail__share">
        <ShareButtons title={story.title} url={`https://www.ssunnah.com/stories?slug=${encodeURIComponent(story.slug)}`} />
      </div>
    </div>
  );
}
      ...
      jsonLd: [
        {
          ...
          description: "قصص الصحابة والفتوحات والتاريخ الإسلامي عبر العصور؛ عرض موثّق ضمن منهج سُنّة",
          itemListElement: CATEGORY_LABELS.filter(c => c !== "الكل").map((cat, i) => ({
            "@type": "ListItem",
            position: i + 1,
            name: cat,
            url: `https://www.ssunnah.com/stories?cat=${encodeURIComponent(cat)}`,
          })),
        },
      ],
    });
