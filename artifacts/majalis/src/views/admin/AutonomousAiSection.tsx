          ...
          {(obs?.cronJobs || []).map((job) => (
            <div key={job.name} className="aai-cron-item">
              ...
              <span
                ...
                style={{ "--aai-status-color": job.status === "healthy" ? "var(--emerald-deep)" : "#dc2626" } as React.CSSProperties}
              >{job.status}</span>
            </div>
          ))}
        </Panel>
            ...
            {events.map((ev) => (
              <div key={ev.id || ev.created_at + ev.stage} className="aai-event">
                <span
                  ...
                  style={{ "--aai-ev-color": ev.success ? "var(--emerald-deep)" : "#dc2626" } as React.CSSProperties}
                >{ev.success ? "✓" : "✗"}</span>
                {" "}
