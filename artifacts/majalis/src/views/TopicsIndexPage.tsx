...
const CATEGORY_META: Record<string, { label: string; Icon: LucideIcon; color: string }> = {
  fiqh:    { label: "الفقه الإسلامي",  Icon: Scale,      color: "var(--majalis-emerald-deep, var(--mj-brand))" },
  aqeedah: { label: "العقيدة",          Icon: Shapes,     color: "var(--majalis-emerald, var(--mj-brand-deep))" },
  akhlaq:  { label: "الأخلاق",          Icon: Leaf,       color: "#2d6a4f" },
  quran:   { label: "القرآن الكريم",   Icon: BookMarked, color: "#0A3D2E" },
  hadith:  { label: "الحديث النبوي",  Icon: ScrollText,  color: "var(--mj-brand-deep, #0d4a39)" },
  seerah:  { label: "السيرة النبوية",  Icon: Moon,       color: "var(--majalis-emerald-deep, var(--mj-brand-deep))" },
...
export default function TopicsIndexPage() {
  ...
  useEffect(() => {
    applyPageSeo({
      ...
      description: "استعرض المحتوى الشرعي مجمّعاً حسب الموضوع: فقه، عقيدة، أخلاق، قرآن، حديث، وسيرة — آيات وأحاديث ودروس ومواد تعليمية.",
  ...
  return (
    <PageShell variant="narrow" className="tip-page">
      <PageHeader
        ...
        subtitle="استعرض المحتوى الشرعي مجمّعاً حسب الموضوع: آيات وأحاديث ودروس وكتب ومواد تعليمية."
      />
