...
assert.ok(RECITERS.some((r) => r.id === "balilah" && r.everyayahFolder === null));
...
assert.match(url, /Yasser_Ad-Dussary_128kbps/);

assert.equal(getReciter("ali_jaber").nameAr, "علي جابر");
assert.equal(getReciter("alafasy").qualityLabel, "128kbps");
