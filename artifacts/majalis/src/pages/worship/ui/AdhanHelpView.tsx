...
const SECTIONS: Array<{ id: string; title: string; body: string }> = [
  {
    ...
    body:
      "تأكد من تفعيل الإشعارات من إعدادات النظام: الإعدادات > التطبيقات > سُنّة > الإشعارات > تفعيل جميع الخيارات. وتأكد من تفعيل التنبيهات من إعدادات التطبيق.",
  },
  {
    id: "silent",
  ...
  },
  {
    ...
    body:
      "فعّل تحديث التطبيقات في الخلفية من: الإعدادات > عام > تحديث التطبيقات في الخلفية > سُنّة.",
  },
  {
    id: "times",
...
export default function AdhanHelpView() {
  useEffect(() => {
    applyPageSeo({
      path: "/adhan-help",
      title: "مساعدة الأذان والتنبيهات | سُنّة",
