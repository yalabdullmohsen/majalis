...
const FAWAID_SESSION_CACHE_KEY = "sunnah.fawaid.list.v1";

function readFawaidSessionCache(): any[] | null {
  if (typeof sessionStorage === "undefined") return null;
  try {
    const raw = sessionStorage.getItem(FAWAID_SESSION_CACHE_KEY);
    if (!raw) return null;
...
function writeFawaidSessionCache(items: any[]): void {
  ...
  try {
    sessionStorage.setItem(FAWAID_SESSION_CACHE_KEY, JSON.stringify(items.slice(0, 800)));
  } catch {
    /* حصة التخزين ممتلئة — تجاهل */
  }
}
...
export default function FawaidPage({
  ...
  const sessionCached = useMemo(() => (initialFawaid ? null : readFawaidSessionCache()), [initialFawaid]);
  ...
  /** خلط ثابت لليوم — لا يُعاد عند hydrate الشبكة فيُغيّر ترتيب البطاقات. */
  ...
  useEffect(() => {
    applyPageSeo({
      ...
      description: "منصة لنشر ومشاركة الفوائد الشرعية، فوائد قرآنية وحديثية وعقدية وفقهية وتربوية. عرض موثّق ضمن منهج سُنّة",
      ...
      jsonLd: [
        {
          ...
          description: "أقسام الفوائد الشرعية على المنصة؛ عرض موثّق ضمن منهج سُنّة",
  ...
  useEffect(() => {
    ...
    // رسم فوري من البذرة — بلا انتظار شبكة أو loadSeedData الكامل
