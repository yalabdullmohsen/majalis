...
const DEFAULT_IGNORE =
  ".mm-controls, .mm-audio-dock, .mm-ayah-bar, .mm-ayah-hit, .mm-ayah-run, .mm-page-edge, .mm-reciter-sheet, .mm-search-sheet, .ayah-action-sheet, .mm-basmala";
...
export function MushafPager({
  ...
  const onPointerUp = (e: ReactPointerEvent<HTMLDivElement>) => {
    ...
    if (Math.abs(dx) < 12 && Math.abs(dy) < 12) {
      if (t.closest(".mm-ayah-hit, .mm-ayah-run, .mm-basmala")) return;
